...
Although such algorithms exhibit exponential
    ...
    designs discussed in this appendix.
\end{itemize}

\Gls{optimal.ps} \gls{pattern selection} on \glspl{block DAG}, however,
...
%
In 1995, \textcite{Proebsting1995b} gave a very concise proof for optimal
\gls{pattern selection}, and a longer, more detailed proof was given by
\textcite{Koes2008} in~2008.
%
In this
...
First, we transform an instance~$S$ of the \gls{SAT} problem into a \gls{block DAG}.
%
...
along with an \gls{edge}~$\mEdge{n'_1}{n'_2}$.
%
...
%
Hence, in the resulting \gls{block DAG},
only \glspl{box node} will have more than one \gls{outgoing.e} \gls{edge}.
%
An
example of such a \gls{DAG} is shown in \refFigure{sat-example}, which can
...
for a \gls{node}~\mbox{$n \in N$} in the \gls{block DAG}~\mbox{$G = \mTuple{N,
    E}$}, $\mMatchSet[n]$ is the set of \glspl{pattern} in $\mPSat$ that match
at~$n$; and for a \gls{node}~\mbox{$n_p \in N_p$} in the selected \gls{match} of
pattern~\mbox{$\mTuple{N_p, E_p}$}, \mbox{$\mMatched(n, n_p)$} is the
\gls{node}~\mbox{$n \in N$} that is matched by~$n_p$.
%
Lastly, we say that $G$ is \emph{covered} by a function~\mbox{$f : N
  \rightarrow 2^{\mPSat}$}, which maps \glspl{node} in the \gls{block DAG} to a
set of \glspl{pattern}, if and only if, for each \mbox{$n \in N$},
\begin{enumerate}
...
%
The
second \gls{constraint} enforces that some \gls{match} has been selected to cover the
\gls{stop node}, and the third \gls{constraint} enforces that \glspl{match} have been
...
improvements.
%
First, \citeauthor{Yu1994a}'s design supports \glsshort{block DAG} and \glspl{pattern DAG} whereas those by~\citeauthor{Newcomer1975} and
Cattell~\etal are both limited to \glspl{tree}.
%
Second, it combines \gls{means-end analysis} with \gls{hierarchical
  planning}~\cite{Sacerdoti1973}, which is a \gls{search} strategy that relies
...
implementing the latter when the former appears in the \gls{block DAG}, as the
\glspl{pattern} are syntactically different from one another.

Their design works as follows.
%
First, the \gls{frontend} emits \glspl{block DAG} consisting of \glspl{semantic primitive}, a kind of \gls{IR} code also used
...
effective implementation of the \gls{block DAG}.
%
...
\gls{block DAG}, and only ``trivial amounts'' of the \gls{block DAG} (for
...
transforming the \glspl{block DAG} into \glspl{tree}, or by generalizing the
...
\gls{block DAG} must be forcibly stored in specific locations, which can be
...
$n_{\text{dup}}$ is the number of \glspl{node} in the \gls{block DAG} (a rough
...
how efficient this technique is compared to a design where the \glspl{block DAG} have been transformed into \glspl{expression tree} using just one method.
...
\gls{register allocation}\footnote{Another register-aware \gls{instruction
  ...
  aim of reducing the number of writes to a nonvolatile \gls{register} file.
%
...
%
This
\gls{pattern tree} is traversed in tandem with the \gls{block DAG}, and for
...
version of the \gls{DP}~algorithm, but the paper does not explain how this is done
exactly.
%
Moreover, the algorithm is only applied on the data flow of the
\gls{block DAG} -- control flow is covered separately using a simple
...
the \gls{block DAG} and compute the costs using the conventional \gls{DP}~algorithm
as discussed in \refAppendix{tree-covering}.
%
Each \gls{node} is thus labeled with
the same costs, as if the \gls{block DAG} had first been transformed into a
...
heuristic that splits the \gls{block DAG} at points where \gls{node
  duplication} is estimated to have a detrimental effect on code quality.
%
Like \citeauthor{Ertl1999}'s algorithm, \citeauthor{Koes2008}'s first selects
...
bottom-up \gls{DP}~pass is performed on the \gls{block DAG}, but this time no
\glspl{pattern} are allowed to span across \gls{fixed.n} \glspl{node}, which
...
support more complex \glspl{instruction} such as \gls{multi-output.ic} \glspl{instruction}.
...
\gls{root}~\glspl{node}.
%
But in a design by \citeauthor{Arnold1999b}~\cite{Arnold1999b, Arnold2001}
(originally introduced in a technical report by \textcite{Arnold1999a}), the
...
The \gls{partial.p} \glspl{pattern} are then matched over the \gls{block DAG} using an
...
\gls{pattern DAG} to the covered \glspl{node} in the \gls{block DAG}, and then
checking whether two \gls{partial.p} \glspl{pattern} belong to the same original \gls{pattern
  DAG} and have compatible mappings.
%
This means that no pair of \gls{pattern}
\glspl{node} belonging to different \gls{partial.p} \glspl{pattern} but corresponding to the
same \gls{node} in the original \gls{pattern DAG} may cover different
\glspl{node} in the \gls{block DAG}.
%
For \gls{pattern selection} \citeauthor{Arnold1999b} applied a variant of the
...
the \gls{block DAG} is covered by exactly one \gls{match}.
...
extensions to \glspl{machine grammar} to support \gls{multi-output.ic} \glspl{instruction}.
...
\glspl{symbol} appearing in \gls{complex.r} \glspl{rule} as \gls!{split.p}[
  \glspl{pattern}]\footnote{In their paper, \citeauthor{Scharwaechter2007} call
  these \gls!{simple.r}[ \glspl{rule}] and \gls!{split.r}[ \glspl{rule}],
  respectively, but to conform with the terminology established
  on \refPage{rule-terms}, we will refer to them as \glspl{pattern}.}.
%
...
for the \gls{complex.p} \glspl{pattern} are found by combining the \glspl{match} of
\gls{split.p} \glspl{pattern} into \glspl{match} of \gls{complex.p} \glspl{pattern} where appropriate.
%
...
decisions have been taken, the next step is to perform \gls{pattern
  selection}.
%
For this, \citeauthor{Scharwaechter2007} solve the corresponding \gls{MWIS}
...
dependency conflicts between \gls{complex.p} \glspl{pattern}, and incorporating a feedback
...
\citeauthor{Ahn2009} is that \gls{complex.r} \glspl{rule} can only consist of disconnected
\glspl{pattern tree} (hence there can be no sharing of \glspl{node} between the
\gls{split.p} \glspl{pattern}).
%
\textcite{Youn2011} address this problem in a 2011~paper -- which is a revised
and extended version of the original paper by \citeauthor{Scharwaechter2007} --
...
\glspl{block DAG}, and then match each \gls{subgraph} over the \glspl{block DAG}.
%
...
\gls{subgraph}, the covered \glspl{node} in the \glspl{block DAG} are
...
designs discussed in this dissertation have only been applied on \glspl{block DAG}.
...
represents a \gls{match} covering one or more \glspl{node} in the \gls{block DAG}.
%
...
heuristically (see for example~\cite{Cordone2000, Goldberg2006} for an
overview).
...
\glspl{constraint} of \gls{pattern selection} since some \glspl{pattern} require -- and
...
%
The
Boolean matrix in \refFigure{unate-covering-example-matrix} can thus be
rewritten as
\begin{displaymath}
  f =
...
latter.
%
Hence, the aforementioned \gls{constraint} regarding the compulsory selection
of $p_6$ if $p_4$ is selected can now be expressed as
\begin{displaymath}
...
According to \citeauthor{Liao1995}~\cite{Liao1995, Liao1998} and
...
linear equations (see for example~\cite{Wolsey1998} for an overview), and a
\gls{solution} to an \gls{IP}~model is an assignment to all variables such that all
equations are fulfilled.
%
In general,
solving an \gls{IP}~model is NP-complete, but extensive research in this field
...
variable\footnote{The more common \gls{constraint} is that \emph{exactly one}
  \gls{pattern} must be selected, but in the design by \citeauthor{Wilson1994},
  \glspl{node} are allowed to become inactive and thus need not be covered.}.
%
...
%
In fact, any \gls{constraint} that can be formulated in this way
can be added to an existing \gls{IP}~model, making this approach a suitable \gls{code
...
main focus of \citeauthor{Wilson1994}).

Solving this monolithic \gls{IP}~model, however, typically requires considerably more
...
extend the model with additional \glspl{constraint} in order to support complicated
...
\gls{complex.p} \glspl{pattern}\unskip% Removes unwanted space
%
...
%
Since
\gls{multi-output.ic} and \gls{disjoint-output.ic} \glspl{instruction}
...
addressed this problem by applying a modified version of the \gls{IP}~model by
...
\textcite{Leupers2000b} later made a more direct extension of the \gls{IP}~model by
\citeauthor{Wilson1994} in order to support \gls{SIMD.i} \glspl{instruction}, which
belong to the class of \gls{disjoint-output.ic} \glspl{instruction}.
%
Described in a paper from~2000, \citeauthor{Leupers2000b}'s design assumes every
...
\citeauthor{Leupers2000b} then extended the \gls{IP}~model with linear equations for
combining \glspl{SIMD pair} into \gls{SIMD.i} \glspl{instruction} and defined the
objective function so as to maximize the use of \gls{SIMD.i} \glspl{instruction}.

In the paper, \citeauthor{Leupers2000b} reports experiments where the use of
\gls{SIMD.i} \glspl{instruction} reduced code size by up to 75\% for the selected test
cases and \glspl{target machine}.
%
But since this technique assumes that each
individual operation of the \gls{SIMD.i} \glspl{instruction} is expressed as a single
\gls{node} in the \gls{block DAG}, it is unclear whether the method can be
extended to more complex \gls{SIMD.i} \glspl{instruction}, and whether it scales to
larger \glspl{program}.
%
\textcite{Tanaka2003} later expanded
\citeauthor{Leupers2000b}'s work for selecting \gls{SIMD.i} \glspl{instruction} while
also taking the cost of data transfers into account by introducing auxiliary
transfer \glspl{node} and transfer \glspl{pattern} into the \gls{block DAG}.


\subsection{Modeling the Pattern Matching Problem with IP}

In 2006, \textcite{Bednarski2006} developed an \gls{integrated.cg} \gls{code
  generation} design where both \gls{pattern matching} and \gls{pattern
...
%
For each
match~$m$, the \gls{IP}~model contains integer \glspl{variable} that:
\begin{itemize}
...
enforcing coverage, this \gls{IP}~model also includes equations to ensure that the
selected \glspl{match} are valid \glspl{match}.

Implemented in a framework called \gls!{Optimist}, \citeauthor{Bednarski2006}
compared their \gls{IP}~model against another \gls{integrated.cg}
\gls{code generation} design based on \glsdesc{DP}, which was developed by the
same authors (see \cite{Kessler2001}) and has nothing to do with the
conventional \gls{DP}~algorithm by \textcite{Aho1989}).
%
\citeauthor{Bednarski2006} found that
\gls{Optimist} substantially reduced compilation time while retaining
code quality, but for several test cases -- the largest \gls{block DAG}
...
could be that the \gls{IP}~model also attempts to solve \gls{pattern matching} -- a
...
Although \glsdesc{IP} allows auxiliary \glspl{constraint} to be included into the
\gls{IP}~model, they may be cumbersome to express as linear equations.
%
This issue can
...
also include \gls!{virtual.sl}[ \glspl{storage location}] which convey that
the value is produced as an intermediate result in a chain of operations (for
...
Then, for every pair of operations that are \gls{adjacent.n} in the \gls{block
  DAG}, a set of \glspl{constraint} is added to ensure that there exists a valid data
transfer between the \glspl{storage location} of $\mVar{d}$ and $\mVar{u}_i$ if
these are assigned to different \glspl{register}, or that both are identical if
one is a \gls{virtual.sl} \gls{storage location}.
%
...
%
The last entity, $\mathit{CS}$, is the set of \glspl{constraint} for defining the range
of values for the \glspl{variable} and the dependencies between $\mVar{d}$
and~$\mVar{u}_i$, as well as other auxiliary \glspl{constraint} that may be required
for the \gls{target machine}.
%
...
Although the \glspl{constraint} in \citeauthor{Bashford1999}'s \gls{CP}~model appear to
be limited to involving only a single~\gls{FRT} at a time -- thus hindering
support for \gls{interdependent.ic} \glspl{instruction} -- \glsdesc{CP} in
general seems like a promising tool for performing \gls{instruction selection}.
%
...
restrictions of the \gls{target machine} to be included in the \gls{CP}~model, but
without the need of expressing these \glspl{constraint} as linear equations.
%
At the
time of writing, however, the existing techniques for solving \gls{IP}~models are
more mature than those for solving \gls{CP}~models, which potentially makes
...
\citeauthor{Arslan2013}~\cite{Arslan2013, Arslan2014, Arslan:2016} to
...
\glspl{match} of \gls{partial.p} \glspl{pattern} are recombined into \glspl{match} of
\gls{complex.p} \glspl{pattern} prior to \gls{pattern selection} (see for example
...
accepting multiple, disconnected \glspl{block DAG} as a single input.
...
in \refAppendix{tree-covering}), \citeauthor{Lorenz2001}~\cite{Lorenz2001,
  Lorenz2004} introduced in 2001 another technique where \glsdesc{GA}s are
applied to \gls{code generation}.
%
But unlike the design by \citeauthor{Shu1996}, the one by
\citeauthor{Lorenz2001} takes \glspl{block DAG} instead of \glspl{tree} as
input and also incorporates \gls{instruction scheduling} and \gls{register
  allocation}.
%
\citeauthor{Lorenz2001} recognized that contemporary \glspl{compiler} struggled
...
\gls{block DAG}, \gls{complex.p} \glspl{pattern} can still be supported through an
...
takes a \gls{block DAG} as input and duplicates each operation \gls{node}
...
finding a path from the \gls{leaf} \glspl{node} in the \gls{block DAG} to its
\gls{root} \gls{node}.
%
...
\gls!{MSS} for \gls{microcode generation}\footnote{\Gls{microcode} is
  essentially the hardware language that processors use internally for executing
  \glspl{instruction}.
%
For example, \gls{microcode} controls how the \glspl{register} and program
counter should be updated for a given \gls{instruction}.}, where a \gls{machine
  description} written in \gls!{MIMOLA}~\cite{Zimmermann1979} is used for
modeling the entire data path of the processor, instead of just the
...
\gls{instruction selection}.
%
\gls{MSSQ} was developed by \textcite{Leupers1998} as a faster version of
\gls!{MSSC}~\cite{Nowak1989}, which in turn is an extension of the
\gls{tree}-based \gls!{MSSV}~\cite{Marwedel1993}.
...
\glsshort{bundling}[les] for parallel execution.
...
\gls{disjoint-output.ic} \glspl{instruction} -- can be supported and exploited during
\gls{instruction selection}, leading to improved performance and reduced code
size.
%
