...
\begin{itemize}
  \item First split the \glspl{pattern DAG} into \glspl{tree}, then match these
    ...
    NP-complete~\cite{Garey1979}, but designs applying this technique typically
...
%
Proofs were given in 1976 by \textcite{Bruno1976} and \textcite{Aho1976b}, but
...
improvements.
%
First, \citeauthor{Yu1994a}'s design supports \glsshort{block DAG} and
\glspl{pattern DAG} whereas those by~\citeauthor{Newcomer1975} and Cattell~\etal
...
(\citeauthor{Newcomer1975} and Cattell~\etal both circumvented this problem by
...
%
Semantic \glspl{match} -- which \citeauthor{Hoover1996} call \glspl!{toe print}
...
developed by \textcite{Keutzer1987}, which maps technology-independent
...
\footnote{%
  ...
  2014 by \textcite{Xie2014}, with the aim of reducing the number of writes to a
...
%
For \gls{pattern matching}, \citeauthor{Liem1994} applied the same technique as
...
\citeauthor{Ertl1999} recognized that if several \glspl{pattern} reduce the same
...
%
\citeauthor{Ertl1999} therefore devised a checker, called \gls{DBurg}, that
...
%
To do this efficiently, \citeauthor{Ertl1999} implemented \gls{DBurg} using the
...
%
But like \citeauthor{Ertl1999}'s design, \citeauthor{Koes2008}'s is limited to
...
\gls{pattern tree}, which \citeauthor{Arnold1999b} call \gls!{partial.p}[
  \glspl{pattern}]
%
...
%
In the general case, finding such a solution is NP-complete~\cite{Garey1979},
and the \gls{pattern selection} problem is transformed into an \gls{MIS} problem
as follows.
%
...
%
\footnote{%
  In their paper, \citeauthor{Scharwaechter2007} call these \gls!{simple.r}[
...
%
\textcite{Ahn2009} later broadened this work by including scheduling dependency
...
\citeauthor{Ahn2009} is that \gls{complex.r} \glspl{rule} can only consist of
...
idea that is applied in the LZ77 algorithm~\cite{Ziv1977}.\!%
%
\footnote{%
...
%
To find these cases in a given \gls{program}, \citeauthor{Brisk2004} first
...
%
\citeauthor{Brisk2004} addressed this by first solving the \gls{MIS} problem on
...
%
\citeauthor{Brisk2004} performed experiments on a prototype using a selected set
...
\gls{assembly code}.
%
\citeauthor{Wilson1994} accomplished this by using \gls{IP}, which is a method
...
\footnote{%
  ...
  be selected, but in the design by \citeauthor{Wilson1994}, \glspl{node} are
...
Similar linear equations can be formulated for modeling \gls{instruction
  scheduling} and \gls{register allocation} -- which \citeauthor{Wilson1994}
...
\citeauthor{Wilson1994} reported that experiments showed that the generated
...
%
\citeauthor{Leupers1996} addressed this problem by applying a modified version
of the \gls{IP}~model by \citeauthor{Wilson1994}.
...
\textcite{Leupers2000b} later made a more direct extension of the \gls{IP}~model
by \citeauthor{Wilson1994} in order to support \gls{SIMD.i} \glspl{instruction},
...
%
This is collectively called a \gls!{SIMD pair}, and \citeauthor{Leupers2000b}
...
\glspl{program}.
%
\textcite{Tanaka2003} later expanded \citeauthor{Leupers2000b}'s work for
selecting \gls{SIMD.i} \glspl{instruction} while also taking the cost of data
transfers into account by introducing auxiliary transfer \glspl{node} and
...
%
The scheme -- which later was applied by \textcite{Eriksson2008}, and is also
described in an article by \textcite{Eriksson2012} -- is an extension of their
earlier work where \gls{instruction selection} had previously more or less been
ignored (see~\cite{Kessler2001, Kessler2002}).
...
compared their \gls{IP}~model against another \gls{integrated.cg} \gls{code
...
\gls{DP}~algorithm by \textcite{Aho1989}).
%
\citeauthor{Bednarski2006} found that \gls{Optimist} substantially reduced
...
solving combinatorial optimization problems (see for example~\cite{Rossi2006}
for an overview) but has more flexible modeling capabilities compared to
\gls{IP}.
...
%
Like \citeauthor{Leupers1996}'s \gls{IP}-based design,
\citeauthor{Bashford1999}'s first breaks down the \gls{instruction set} of the
...
Although the \glspl{constraint} in \citeauthor{Bashford1999}'s \gls{CP}~model
...
%
\citeauthor{Arslan2013}'s design therefore differs from the previous techniques
...
\gls{pattern selection} (see for example \textcite{Scharwaechter2007},
\textcite{Ahn2009}, \citeauthor{Arnold1999b}~\cite{Arnold1999a, Arnold1999b,
  Arnold2001}), as it allows these two problems to be solved in tandem.
%
...
\citeauthor{Lorenz2001} takes \glspl{block DAG} instead of \glspl{tree} as input
and also incorporates \gls{instruction scheduling} and \gls{register
  allocation}.
%
...
%
According to \citeauthor{Lorenz2001}, the major contribution to this reduction
...
%
Implemented in a system called \gls{Aviv}, \citeauthor{Hanono1998}'s
...
\citeauthor{Wess1992}, \citeauthor{Hanono1998} applied a greedy heuristic that
starts from the \gls{root} \gls{node} and makes it way towards the \glspl{leaf}.
...
%
where a \gls{machine description} written in \gls!{MIMOLA}~\cite{Zimmermann1979}
...
(further examples include~\cite{Balakrishnan1986, Mahmood1990, Langevin1993}).
%
