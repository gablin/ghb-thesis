...
\refChapter{existing-isel-techniques-and-reps} is duplicated in this appendix.xk
...
\footnote{%
  ...
  2014 by \textcite{Xie:2014}, with the aim of reducing the number of writes to
  a nonvolatile \gls{register} file.
  %
...
as an extension of \gls{Olive} (see \refSection{olive}), and then ran some
experiments by targeting a \gls{MIPS}-like architecture.
%
...
of \gls{SIMD.i} \glspl{instruction} reduced code size by up to 75\% for the
...
generate near-optimal \gls{assembly code}.
%
In addition, the \glspl{block DAG} of the benchmark \glspl{program} were fairly
...
\footnote{%
  It is also possible to enforce \gls{pattern selection} through a \gls{global
    set covering constraint} developed by \textcite{MouthuyEtAl:2007}, but I
  have not seen any implementation do so.%
}
%
