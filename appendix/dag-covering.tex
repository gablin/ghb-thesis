...
operands~$x$ and $y$\hspace{-1pt}, we add two \glspl{edge}~$\mEdge{n_x}{n'_1}$
and $\mEdge{n_y}{n'_1}$ such that \mbox{$\mType(n_x) = \mType(n_y) = \mBox$}.
%
...
%
Let $\mNumChildren(n)$ denote the number of \glspl{child} of~$n$\hspace{-.8pt},
and $\mChild(i\hspace{-.8pt}, n)$ the $i$th \gls{child} of~$n$\hspace{-.8pt}.
%
We now say that a \gls{pattern}~$p$\hspace{-.8pt}, with \gls{root}
...
\end{displaymath}
%
where $\mCost(p)$ is the cost of \gls{pattern}~$p$\hspace{-.8pt}.
...
bound.
%
For each match~$m$\hspace{-.8pt}, the \gls{IP}~model contains integer
\glspl{variable} that:
%
\begin{itemize}
...
\gls{node}~$n$\hspace{-.8pt}, and \gls{variable} \mbox{$\mVar{count}_m \in
  \mSet{0, |\mCovers(m)|}$} decides how many \glspl{node} are covered by
\gls{match}~$m$\hspace{-.8pt}.
%
