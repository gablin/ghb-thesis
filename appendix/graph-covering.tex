...
%
Therefore, in this chapter we will only examine techniques that were originally
...
research has been devoted to this problem (see for example~\cite{Ullmann1976,
  ...
  Fan2011, Hino2012}).
%
In this section we will mainly look at \citeauthor{Ullmann1976}'s algorithm and
another commonly used algorithm by \citeauthor{Cordella2001}.
%
...
%
In an attempt to mitigate this issue \textcite{Arora2010} introduced a method
where the \glspl{function graph} are first normalized before \gls{pattern
...
%
\citeauthor{Ullmann1976} therefore tried to reduce the search space by
developing a procedure that eliminates some of the 1s that will never appear in
any solution.
%
But according to \textcite{Cordella2001}, even with this improvement the
...
%
For example, \citeauthor{Cordella2001} report in \cite{Cordella2001} that the
...
recently made by \textcite{Stanier2013}).
...
optimization problem (see~\cite{Loiola2007} for a recent survey) -- and was
first introduced \textcite{Scholz2002} as a means of tackling \gls{register
  allocation}.
%
...
%
For this, \citeauthor{Eckstein2003} seem to have used the Floyd-Warshall
...
also developed by \citeauthor{Eckstein2003} (we will omit details on how it
works).
...
%
According to \citeauthor{Eckstein2003}, this considerable gain in performance
...
%
Although the technique by \citeauthor{Eckstein2003} clearly mitigates these
...
the original \glsshort{PBQP} model by \citeauthor{Eckstein2003} to also support
\glspl{pattern DAG}.
%
...
%
\citeauthor{Ebner2008} solved this problem by first setting a high cost~$M$ to
...
%
In their model, \citeauthor{Ebner2008} also forbade selection of \gls{complex.r}
...
\glspl{PBQP}.
%
But unlike \citeauthor{Eckstein2003} and \citeauthor{Ebner2008},
\citeauthor{Buchwald2010} approached the task of \gls{instruction selection} as
...
\glsshort{PBQP} solver by \citeauthor{Eckstein2003}, which may fail to find a
solution in certain situations due to inadequate propagation of information.
%
However, \citeauthor{Buchwald2010} also cautioned that their own implementation
...
%
Like \gls{MSSQ} and \gls{Chess}, \citeauthor{Visser1999}'s approach is an
...
\cite{Kirkpatrick1983} for an overview).
%
...
%
which was introduced in 1994 by \citeauthor{Novack2002}~\cite{Novack1994,
  Novack2002}.
%
...
%
\citeauthor{Novack2002} compute these \glspl{mutation set} by first taking the
