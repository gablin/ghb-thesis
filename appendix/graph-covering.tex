...
example~\citeNoMbox{Ullmann1976, Cordella2001, Guo2003, Krissinel2004,
  Sorlin2004, Gallagher2006, Fan2010, Fan2011, Hino2012}).
%
In this section we
...
\combinedTextcite{Jiang1996, Jiang1998a, Jiang1998b} discovered that if the
...
%
For example, in
\refSubfigure{ssa-example-not-ssa} we see the factorial \gls{function}
(introduced in \refAppendix{introduction}), which is not in \tSSAform as
\cVar*{n} and \cVar*{f} are defined multiple times (first at lines~1
and~3, and then at lines~7 and~6, respectively).
%
We could try to rename the
variables to avoid redefinition conflicts---\cVar*{n} is renamed to
\cVar*{n}[1] at line~1 and to \cVar*{n}[2] at line~7, and \cVar*{f} is
renamed to \cVar*{f}[1] at line~3 and to \cVar*{f}[2] at line~6---but
which variable do we use for the \cCode*{return} at line~10? These situations
...
Hence, by declaring additional variables and inserting \glspl{phi
  function} at the beginning of the \cCode*{loop}~\gls{block}, the
aforementioned problem of \cVar*{f} is resolved as shown in
\refSubfigure{ssa-example-ssa}\footnote{Although this in turn introduces two new
...
example \refSubfigure{ssa-example-ssa}).
%
...
%    {Converting the function from \refSubfigure{ssa-example-not-ssa} into a
...
\refSubfigure{pdg-example-cfg}) this information is not immediately visible, but
in the \gls{CDG} (see \refSubfigure{pdg-example-cdg}) this fact becomes clear,
...
\gls{instruction selector} (see \refSection{precomputing-cost-computations})
...
%
We will explain
the \glsshort{PBQ problem} approach by building the model bottom-up, starting
...
\end{displaymath}
The model is then solved using a heuristic \glsshort{PBQ problem} solver, which
...
%
Most importantly,
their \glsshort{PBQ problem} model can only support \glspl{pattern tree} and
...
the original \glsshort{PBQ problem} model by \citeauthor{Eckstein2003} to also
...
%
The \glsshort{PBQ problem} model is
...
one from another.
%
We say that all \glsshort{base rule} and \tproxyRules belong
...
%
We can now proceed
with extending the \glsshort{PBQ problem} model.
...
element indicates \emph{selected})\footnote{A \mbox{two-element} vector is
  chosen instead of a single Boolean variable as the \glsshort{PBQ problem}
...
Hence the complete definition of $f$ in the \glsshort{PBQ problem} model by
...
\glsshort{PBQ problem} solver by \citeauthor{Eckstein2003}, which may fail to
...
control-flow information, none of the \glsshort{PBQ problem}-based techniques
...
\combinedTextcite{VanPraet1994, VanPraet2001} in their implementation of
...
introduced in 1994 by \combinedTextcite{Novack2002, Novack1994}.
%
\Gls{mutation
...
%
Another example is
multiplication by powers of~2, which can be replaced with \techTerm{shift}
...
have more extensive \gls{instruction} support compared to most \glsshort{tree
  covering} and \gls{DAG covering}-based designs.
