...
%\end{inParFigure}%
%\noindent
Let us assume that the \gls{program} is represented as a set of
\glspl{expression tree}, which we are already familiar with from
\refAppendix{macro-expansion} (see \refPage{program-trees}).
%
Let us further assume that each \gls{instruction} can be modeled similarly to a
\gls{pattern tree}.
%
When there is no risk of confusion, we will refer to these as simply
\glspl{pattern}.
%
We also say that the set of all \glspl{pattern} available during tree covering
constitute the \gls{pattern set}.
%
The task of \gls{instruction selection} can then be reduced to finding a subset
of the \gls{pattern set} such that every \gls{node} in the \gls{expression tree}
is \emph{covered} by at least one \gls{pattern}.
%
Here we see clearly how \gls{pattern matching} and \gls{pattern selection}
constitute two separate problems in \gls{instruction selection}.
%
In the former we need to find which \glspl{pattern} are applicable for a given
\gls{expression tree} and where they are applicable.
%
...
%
In the latter problem we then select from these \glspl{match} a subset which
results in a valid and efficient cover of the \gls{expression tree}.
%
...
\gls{assembly code}.
%
But unlike optimal \emph{\glsshort{instruction selection}} selection, whose
problematic definition was criticized in \refAppendix{introduction},
\gls{optimal.ps} \emph{\glsshort{pattern selection}} selection is less
controversial since it is clear that we will only consider the \glspl{pattern}
currently available in the \gls{pattern set} instead of all \glspl{pattern} that
could potentially be derived from the~\gls{instruction set}.

Finding the \gls{optimal.ps} solution to a \gls{tree covering} problem is not a
trivial task, and it becomes even less so if only certain combinations of
\glspl{pattern} are allowed.
%
To be sure, most of us would be hard-pressed just to come up with an efficient
method that finds all valid \glspl{match} of the entire \gls{pattern set}.
%
...
%
Unfortunately only \citeauthor{Weingart1973}'s work appears to be recognized by
other literature, even though \citeauthor{Wasilew1972}'s ideas have more in
...
\textcite{Lunell1983}, who gives a more detailed account in his doctoral
dissertation.
...
%
\citeauthor{Wasilew1972} also developed his own programming language, which is
...
matched against the \gls{program}.
%
For each line in the \gls{program}, \gls{pattern matching} is done starting at a
\gls{leaf} in the \gls{tree} corresponding to the current line.
%
...
compilation time.
%
In addition, the notations used by \citeauthor{Wasilew1972} are difficult to
read and write.

In comparison to \citeauthor{Wasilew1972}, \citeauthor{Weingart1973}'s design is
centered around a single \gls{tree} of \glspl{pattern} --
\citeauthor{Weingart1973} called this a \gls!{discrimination net} -- which is
automatically derived from a declarative \gls{machine description}.
%
Using a single \gls{tree} of \glspl{pattern}, \citeauthor{Weingart1973} argued,
...
is known that \citeauthor{Weingart1973} struggled in particular with the
\gls{PDP-11}.
%
...
%
Third, like its macro-expanding predecessors, the process immediately selects a
...
given \gls{node} in the \gls{expression tree}, the \gls{node} is compared
...
\refFigure{straight-forward-pattern-matching} -- exhibits quadratic time
...
%
Like \citeauthor{Weingart1973}, \citeauthor{Johnson1981} used heuristic
...
\gls{compiler} to never terminate for certain \glspl{program}.
%
\textcite{Reiser1981} also noted that the investigated version of \gls{PCC} only
supported unary and binary \glspl{pattern} with a maximum height of~1, thus
excluding many \glspl{instruction}, such as those with complex \glspl{addressing
  mode}.
%
...
%
In particular, \textcite{Henry1984} received his doctoral degree for his
...
%
For each \gls{nonterminal} there exists one or more \glspl{production} of the
following form:
%\begin{displaymath}
...
%
In terms of recursion, one can also think of \glspl{nonterminal} as inducing the
recursive case whereas the \glspl{terminal} provide the base case that stops the
recursion.
%
\Glspl{production} are often also called \glspl!{production rule} or just
\glspl!{rule}, and although they can typically be interchanged without causing
confusion, we will be consistent in this dissertation and only use the first
...
%
For that, \citeauthor{Glanville1978} applied an already-known technique called
\gls{LR parsing}~\cite{Knuth1965}.
%
...
\end{center}
%
such that the result of the expression ends up in a \gls{register}.
%
If \cVar*{a}, \cVar*{b}, and \cVar*{c} all are integers, then we can assume that
each \gls{node} in the \gls{expression tree} is of type \irCode*{Int},
\irCode*{$*$}, or \irCode*{$+$}.
%
These will be our \glspl{terminal}.

After transforming the \glspl{expression tree} into sequences of
...
\gls{symbol}.
%
\citeauthor{Glanville1978} also incorporated a \gls{register allocator} into
their parser, thus constituting an entire \gls{code generator}.
...
%
To solve this kind of conflict, \citeauthor{Glanville1978}'s state table
...
\footnote{%
  ...
  \citeauthor{Cattell1978} in his doctoral dissertation~\cite{Cattell1978}.%
}.
%
...
%
\citeauthor{Glanville1978} resolved these by selecting the \gls{rule} with the
longest \gls{pattern}.
%
...
order in which they appear in the \gls{grammar} (see for example state~20 in
\refFigure{graham-glanville-table}).
%
...
\citeauthor{Glanville1978} devised a clever trick to infer them automatically.
%
...
%
For instance, \textcite{Emmelmann1992b} presented one of the first methods of
proving the completeness of an \gls{machine grammar}\footnote{Note, however,
  that even though an \gls{machine grammar} has been proven to be complete, a
  greedy \gls{instruction selector} may still fail to use a necessary
  \gls{rule}.
%
Consequently, \citeauthor{Emmelmann1992b}'s checker assumes that an optimal
\gls{instruction selector} will be used for the proven \gls{machine grammar}.}.
%
The intuition behind \citeauthor{Emmelmann1992b}'s automatic prover is to find
all \glspl{expression tree} that can appear in the \gls{program} but cannot be
handled by the \gls{instruction selector}.
%
...
%
\citeauthor{Emmelmann1992b} recognized that this intersection can be computed by
creating a \gls{product automaton} which essentially implements the language
that accepts only the \glspl{tree} in this set of counterexamples.
%
...
%
\citeauthor{Glanville1978} worked around this limitation by pushing the
...
%
\citeauthor{Ganapathi1982b}~\cite{Ganapathi1980, Ganapathi1982b, Ganapathi1984,
  Ganapathi1985} addressed this problem by replacing the use of traditional,
\glspl{context-free grammar} with the use of a more powerful set of
\glspl{grammar} known as \glspl{attribute grammar}.
%
...
%
\textcite{Farrow1982} even made an attempt at deriving an entire \gls{Pascal}
...
due to the use of \gls{LR parsing}.  \textcite{Ganapathi1989} later made an
...
implementation of \citeauthor{Earley1970}'s algorithm~\cite{Earley1970}, and
...
achieve \gls{optimal.ps} selection of \glspl{pattern} while allegedly retaining
the linear time complexity of \gls{LR parsing}.
%
By incorporating a new version of \gls{LR parsing}~\cite{Shankar2000},
...
\gls{program} before committing to a decision that could turn out to be
suboptimal.
%
...
those based on \gls{LR parsing} -- all operate \emph{bottom-up}: the
\gls{instruction selector} begins to cover the \glspl{leaf} in the
\gls{expression tree}, and based on the decisions taken for the \glspl{subtree}
it then progressively works it way upwards along the \gls{tree} until it reaches
the \gls{root}, continually matching and selecting applicable \glspl{pattern}
along the way.
%
...
\gls{register} in which a result will be stored, is also different: a bottom-up
\gls{instruction selector} lets this information trickle upwards along the
\gls{expression tree} -- either via auxiliary data structures or through
...
scheme that uses top-down \gls{tree covering} to address \gls{instruction
  selection}.
%
In his 1975 doctoral dissertation, \citeauthor{Newcomer1975} proposes a design
...
%
\textcite{Cattell1977} also describes this in his survey paper, which is the
main source for the discussion of \citeauthor{Newcomer1975}'s design.
...
uses to perform necessary transformations of the \gls{program} -- its need will
...
from before (see \refFigure{straight-forward-pattern-matching} on
...
%
To guide this process, \citeauthor{Newcomer1975} applied a heuristic search
strategy known as \gls{means-end analysis}, which was introduced by
\textcite{Newell1959} in~1959.
%
...
in~\cite{Cattell1977} -- between the current state (that is, what the
...
%
First, it had little practical application, as \citeauthor{Newcomer1975}'s
...
%
Third, the process of transforming the \gls{program} could end prematurely due
...
%
Instead of performing the \gls{means-end analysis} as the \gls{program} is
...
%
But unlike \citeauthor{Glanville1978}'s and \citeauthor{Ganapathi1982b}'s
designs -- where the \glspl{grammar} were written by hand -- the
\glspl{template} in \gls{PQCC} are derived automatically from a target-specific
...
heuristic \glspl{rule} which apply a set of axioms (such as \mbox{$\neg \neg E
  \Leftrightarrow E$}, \mbox{$E + 0 \Leftrightarrow E$}, and \mbox{$\neg (E_1
...
%
It could therefore be argued that the two are interchangeable, but I regard
...
(see \refFigure{tree-covering-dilemma-invalid-coverage}).
...
\refFigure{tree-covering-dilemma-augmented-coverage}).
...
%
This was first discovered by \textcite{Karp1972} in~1972, and their ideas were
later extended by \textcite{Hoffmann1982} to form the algorithms most applied by
\gls{tree}-based \gls{instruction selection} techniques.
%
...
%
This is illustrated in the inlined table where the \gls{pattern} string
...
the last character (position~5).
%
When this fails, instead of returning to position~1 and restarting the matching
from scratch, the matcher remembers that the first three characters of the
\gls{pattern} (\cCode*{abc}) have already been matched at this point and
therefore continues to position~6, attempting to match the fourth character in
the \gls{pattern}.
%
Consequently all occurrences of the \gls{pattern} can be found in linear time.
%
We continue our discussion with \citeauthor{Aho1975}'s design as it is capable
of matching multiple \glspl{pattern} whereas the algorithm of
\citeauthor{Knuth1977} only considers a single \gls{pattern} (although it can
easily be extended to handle multiple \glspl{pattern} as well).
...
\citeauthor{Aho1975}'s algorithm relies on three functions -- $\mFunFont{goto}$,
$\mFunFont{failure}$, and $\mFunFont{output}$ -- where the first function is
implemented as a \gls{state machine} and the two latter ones are implemented as
...
%
In \refFigure{string-matching-machine} we see the corresponding functions for
...
%
In a paper from~1982, \citeauthor{Hoffmann1982} first present an
...
Hence, in our case \mbox{$\mRank(a) = 2$} and \mbox{$\mRank(b) = \mRank(c) =
  \mRank(v) = 0$}.
%
Following the terminology used in \citeauthor{Hoffmann1982}'s paper, we also
introduce the notion of a \gls!{sigma term} and define it as follows:
%
\begin{enumerate}
...
%
But \citeauthor{Hoffmann1982} recognized that, for \gls{simple.ps}
...
%
For example, if labels~1 and~2 respectively refer to the \glspl{pattern}~$b$ and
...
from~0, which will represent the labels (the order in which these are assigned
is not important).
...
\refFigure{hoffmann-odonnell-example-table-a, hoffmann-odonnell-example-table-b,
  hoffmann-odonnell-example-table-c}.
...
is also shown in \refFigure{table-compression-uncompressed}.
%
...
in the first place.
%
But \citeauthor{Chase1987} discovered that the tables can be compressed as they
are generated, thus pushing the limit on how large lookup tables can be
produced.
%
\textcite{Cai1992} later improved the asymptotic bounds of
\citeauthor{Chase1987}'s algorithm.
...
%
According to the literature, \textcite{Ripken1977} was the first to propose a
...
\citeauthor{Aho1976a} -- which was mentioned in \refAppendix{introduction} --
...
\citeauthor{Aho1989}~\cite{Aho1985, Tjiang1986, Aho1989} with the introduction
of a \gls{compiler} generator called \gls{Twig}.
...
%
The \gls{machine description} is expressed as an \gls{machine grammar} (see
...
every \gls{node} in the \gls{expression tree}\footnote{Remember that, when using
  \glspl{machine grammar}, a \gls{pattern} found in the \gls{match set} during
  \gls{pattern matching} corresponds to the right-hand side of a
  \gls{production}.} using an implementation of the Aho-Corasick string matching
algorithm~\cite{Aho1975}.
%
...
%
As we are already quite familiar with how to do \gls{tree}-based \gls{pattern
  matching}, we will focus on the cost computation and \gls{pattern selection}
algorithms.
...
%
For each \gls{node}~$n$ in the \gls{expression tree} we maintain an array for
remembering the lowest cost of reducing $n$ to a particular \gls{nonterminal},
as well as the \gls{rule} for making this \glsshort{rule reduction}.
%
These costs are found by iterating over all \glspl{rule} found in the \gls{match
  set} of $n$ and computing the cost of applying a particular \gls{rule}.
%
This cost is computed as the sum of the cost of the \gls{rule} itself and the
costs of the \glspl{nonterminal} appearing on the rule's \gls{pattern tree}.
%
Initially, all costs are set to infinity, thus prohibiting \glsplshort{rule
  reduction} using \glspl{nonterminal} which are not available.
%
Once all \glspl{rule} originating from the \gls{match set} have been checked, we
need to check all applicable \gls{chain.r} \glspl{rule} as these may make the
current \gls{node} reducible to additional \glspl{nonterminal} or to the same
\glspl{nonterminal} but at a lower cost.
%
However, since one \gls{chain.r} \gls{rule} may depend on another \gls{chain.r}
\gls{rule}, it matters in what order the \gls{chain.r} \glspl{rule} are checked
(at least until a fixpoint is reached).
%
...
After the costs have been computed for the \gls{root} of the \gls{expression
  tree}, we can find the least-cost cover that reduces the entire
\gls{expression tree} to the goal \gls{nonterminal} simply by consulting the
cost arrays (the algorithm is shown in
\refFigure{pattern-selection-and-code-emission-dp}).
%
...
\gls{subtree}.
%
Note that since \glspl{pattern} can have arbitrary height, this \gls{subtree}
can appear several levels down from the current \gls{node} in the
\gls{expression tree}.
%
...
optimality.
%
As explained in \refAppendix{introduction}, this is not always the case; some
solutions, whose total sum is greater compared to another set of selected
...
%
\citeauthor{Yates1988} improved the rate of \gls{pattern matching} by replacing
\gls{Twig}'s top-down approach with the faster bottom-up algorithm proposed by
\citeauthor{Hoffmann1982}, and also extended the \gls{attribute} support for
more powerful \glspl{predicate}.
%
\citeauthor{Emmelmann1989} modified the \gls{DP} algorithm to be run as the
...
overhead.
%
\citeauthor{Emmelmann1989} implemented their improvements in a system called the
\gls!{BEG}, and a modified version of this is currently used in the \gls!{CoSy}
\gls{compiler}~\cite{ACE2003}.
...
%
\citeauthor{Gough1997}~\cite{Gough1995, Gough1997} later extended the
\gls{predicate} support of \gls{IBurg} in an implementation called \gls{MBurg}.
%
...
\gls{GPBurg}~\cite{Gough2012}, which is written in \gls{CSharp}.
...
%
Experiments showed that this scheme reduced code size by about 17\% on average
compared to \gls{CoSy} for the selected target architecture and benchmark suite.
...
%
However, \citeauthor{Horspool1987} appears to have had to implement his
\gls{instruction selection} tables by hand whereas \citeauthor{Aho1989} built a
...
%
To explain this further, let us observe how most (if not all) \glspl{target
  machine} operate: between the execution of two \glspl{instruction}, the data
is synchronized by storing it in \glspl{register} or in memory.
%
...
labeling a \gls{expression tree} with \glspl{state} is exactly the same as
before (see \refFigure{hoffmann-odonnell-labeling} on
...
described in \refFigure{pattern-selection-and-code-emission-states}, more or
less identically to the algorithm used in conjunction with \glsdesc{DP} (compare
with \refFigure{pattern-selection-and-code-emission-dp} on
...
%
\citeauthor{Hatcher1986} argued that for \gls{optimal.ps} \gls{pattern
  selection} we can consider each pair of a label~$l$ and
...
\gls{pattern} appearing in the \gls{match set}, but to accommodate
\glspl{machine grammar}, \citeauthor{Hatcher1986}'s version includes one
\gls{nullary symbol} per \gls{nonterminal}.
%
...
\citeauthor{Hatcher1986}'s design.
...
%
\citeauthor{Hatcher1986} also augmented the original algorithm so that the
returned \glspl{match set} contain all \glspl{pattern} that were duplicated due
commutative operations.
%
However, if the \gls{pattern set} contains \glspl{pattern} which are truly
\gls{independent.pt}, then \citeauthor{Hatcher1986}'s design does not always
guarantee that the \glspl{expression tree} can be optimally covered.
%
...
%
In a seminal paper from~1988, \citeauthor{Pelegri-Llopart1988} prove that the
methods of \gls{tree rewriting} can always arranged such that all rewrites occur
at the \glspl{leaf} of the \gls{tree}, resulting in a \gls{BURS}.
%
...
%
An example of such a \gls{grammar} is given in
\refFigure{burs-example-rewrite-system}, and \textcite{Dold1998}, as an
extension to the work of \textcite{Zimmermann1997}, later developed a method for
proving the correctness of \glspl{BURS grammar} using abstract \glspl{state
  machine}.

Using \gls{BURS} theory \citeauthor{Pelegri-Llopart1988} developed an algorithm
that computes the tables needed for \gls{optimal.ps} \gls{pattern selection}
based on a given \gls{BURS grammar}.
%
...
\refFigure{burs-example2-tree-unbounded}.
%
...
\refFigure{burs-example2-tree-bounded}.
%
...
%
Although the work by \citeauthor{Pelegri-Llopart1988} undoubtedly led to making
\gls{offline cost analysis} an established aspect of modern \gls{instruction
  selection}, the application of \gls{BURS} theory is only \emph{one} means to
achieving \gls{optimal.ps} \gls{pattern selection} using tables.
...
lowest cost of any \gls{rule} appearing in that \gls{state} is~0.
%
...
%
In addition, \citeauthor{Balachandran1990} expressed the \glspl{instruction} as
a more traditional \gls{machine grammar} -- like those used in the
\gls{Glanville-Graham approach} -- instead of as a \gls{BURS grammar}.
...
%\end{enumerate}
%
A non\gls{normal form.g} \gls{grammar} can easily be rewritten into
\glsshort{normal form.g} by introducing new \glspl{nonterminal} and \glspl{rule}
for making the necessary transitions.
%
...
%
This also means that the \glspl{production} appearing in the \glspl{rule} become
more uniform, which greatly simplifies the task of generating the \glspl{state}.
...
%
This algorithm was also implemented by \textcite{Fraser1992b} in a renowned
\gls{code generation} system called \gls{Burg},\!%
%
\footnote{%
  The keen reader will notice that \citeauthor{Fraser1992a} also implemented the
  \gls{DP}-based system \gls{IBurg} which was introduced on \refPage{iburg}.
  %
  ...
  %
  \citeauthor{Fraser1992b} later recognized that some of the ideas for the
  testbench showed some merit in themselves, and therefore improved and extended
  them into a stand-alone generator.
  %
...
%
which since its publication in 1992 has sparked a naming convention within the
\gls{compiler} community that I have chosen to call the \gls!{BURGer
  phenomenon}.\!%
%
\footnote{%
  During my research for this dissertation, I came across the following systems,
  all with equally creative naming schemes: \gls{Burg}~\cite{Fraser1992b},
  ...
  \gls{WBurg}~\cite{Proebsting1996}.%
}
%
Although \citeauthor{Balachandran1990} were first, we will continue with
studying \citeauthor{Proebsting1992b}'s algorithm as it is better documented.
%
More details are also available in \citeauthor{Proebsting1992a}'s doctoral
dissertation~\cite{Proebsting1992a}.
...
%
Again, it is assumed that the \gls{machine grammar} is in \glsshort{normal
  form.g}.
%
The queue is first initialized with the \glspl{state} that can be generated from
all possible \glspl{leaf}.
%
A \gls{state} is then popped from the queue and used in combination with other
already-visited \glspl{state} in an attempt to produce new \glspl{state}.
%
This is done by effectively simulating what would happen if a set of
\glspl{node}, appearing as \glspl{child} to some node~$n$, were labeled with
some combination of \glspl{state}, including the \gls{state} that was just
popped.
%
If $n$ can be labeled using an already existing \gls{state}, then nothing
happens; if not, then a new appropriate \gls{state} is created and appended to
the queue, making sure that all applicable \gls{chain.r} \glspl{rule} have been
applied to the \gls{state}, as this can affect costs.
%
This is checked for every possible combination of \glspl{state} and operator
\glspl{symbol}, and the algorithm terminates when the queue becomes empty,
indicating that all \glspl{state} necessary for the \gls{machine grammar} have
been generated.
...
%
According to \textcite{Proebsting1992b}, the first attempts to do this were made
by \textcite{Henry1989}, whose methods were later improved and generalized by
\citeauthor{Proebsting1992b}~\cite{Proebsting1992b, Proebsting1995a}.
%
...
%
This technique was later improved by \citeauthor{Kang1995}~\cite{Kang1995,
  Kang2004}, who exploited properties of common \glspl{machine description} to
decrease the amount of redundant \gls{state} testing.
...
those that dynamically compute the costs as the \gls{program} is compiled, and
...
actually appear in the \gls{program}.
...
%
In brief terms \citeauthor{Geigerich1988} reformulated the \gls{instruction
  selection} problem into a ``problem of a hierarchic derivor,'' which
essentially entails the specification and implementation of a mechanism
%
\begin{displaymath}
...
%
In their 1994~paper, \citeauthor{Ferdinand1994} demonstrate how \glspl{finite
  tree automaton} can be used to solve both \gls{pattern matching} and
\gls{pattern selection} -- greedily as well as \gls{optimal.ps}[ly] -- and also
...
%
Similar designs were later proposed by \textcite{Borchardt2004} and
\textcite{Janousek2014}, who made use of \glspl{tree series transducer} (see for
example~\cite{Engelfriet2001} for an overview) and \glspl{pushdown automaton},
respectively.
...
%
Through a system called \gls!{Stratego}~\cite{Visser2001}, a \gls{machine
...
the \gls{pattern selector} to be tailored to that particular \gls{target
  machine}.
%
\citeauthor{Bravenboer2002} refer to this as providing a \gls{rewriting
...
%
In their paper, \citeauthor{Bravenboer2002} argue that this setup allows several
\gls{pattern selection} techniques to be combined, but they do not provide an
...
example \cite{Reeves2010} for an overview).\!%
%
\footnote{%
  On a related note, \textcite{Wu2006} applied \gls{ant colony optimization} --
  a meta-heuristic inspired by the shortest-path searching behavior of various
  ant species \cite{Dorigo2010} -- to improve overall code size by alternating
  between \glspl{instruction set} on a per-function basis.%
}
%
...
an \mbox{$\mBigO(nm)$} \gls{pattern matcher}, \citeauthor{Shu1996} formulated
each \gls{chromosome} as a binary bit string where a~1 indicates the selection
of a particular \gls{pattern}.
%
Likewise, a~0 indicates that the \gls{pattern} is not used in the \gls{tree
  covering}.
%
...
number of \glspl{node} in $c$ which are covered by more than one \gls{pattern}.
%
...
appendix is a rather unusual design by \citeauthor{Wess1992}~\cite{Wess1992,
  Wess1995}.
%
Specifically targeting \glsdesc{DSP}[s], \citeauthor{Wess1992}'s design
...
%
For example, let us assume a \gls{target machine} with two \glspl{register}
\cVar*{r}[1] and \cVar*{r}[2] yields the following \gls{OVA}
%
\def\mTL{\text{TL}}
\def\mRA{\text{RA}}
%
...
%
This can be generalized to \mbox{$n$-ary} operations, and a complete example is
given in \refFigure{trellis-example}.
...
%
This problem was mitigated by \textcite{Frohlich1999}, who augmented the
