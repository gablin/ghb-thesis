...
\refFigure{glanville-graham-instruction-grammar}, is given in
\refFigure{glanville-graham-table}, and a walk-through of executing an
\gls{instruction selector} with this state table using an \gls{LR parser} is
provided in \refFigure{glanville-graham-execution}.

The subscripts that appear in some of the \glspl{production} in
\refFigure{glanville-graham-instruction-grammar} are \glspl{semantic qualifier},
  ...
  \begin{minipage}{66mm}%
    ...
    \caption[Machine grammar example]%
            {%
              ...
              qualifiers.
              %
              All rules have the same unit cost~\cite{GlanvilleGraham:1978}%
            }
  ...
  \begin{minipage}{55mm}
    ...
    \caption[%
            ...
            {%
              ...
              blank entry indicates an error~\cite{GlanvilleGraham:1978}%
            }
...
\begin{figure}
    ...
    \begin{minipage}{170mm}
      \begin{tabular}{rll@{}rl}
        ...
        \midrule
           ...
           & 0 1 4
           & \cCode{$=$ $+$}
           & \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld $+$} \cVar{c}[b] \cCode{ld} $\mNT{R}[7]$ \cCode{ld}
             \cVar{c}[c] \cCode{\$}
           & shift to 12\\
             4
           & 0 1 4 12
           & \cCode{$=$ $+$} \cVar{c}[a]
           & $\mNT{R}[7]$ \cCode{$+$ ld $+$} \cVar{c}[b] \cCode{ld} $\mNT{R}[7]$ \cCode{ld} \cVar{c}[c]
             \cCode{\$}
           & shift to 23\\
             5
           & 0 1 4 12 23
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$
           & \cCode{$+$ ld $+$} \cVar{c}[b] \cCode{ld} $\mNT{R}[7]$ \cCode{ld} \cVar{c}[c] \cCode{\$}
           & shift to 8\\
             6
           & 0 1 4 12 23 8
           ...
           & 0 1 4 12 23 8 13
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld}
           & \cCode{$+$} \cVar{c}[b] \cCode{ld} $\mNT{R}[7]$ \cCode{ld} \cVar{c}[c]\cCode{\$}
           & shift to 25\\
             8
           & 0 1 4 12 23 8 13 25
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld $+$}
           & \cVar{c}[b] \cCode{ld} $\mNT{R}[7]$ \cCode{ld} \cVar{c}[c] \cCode{\$}
           & shift to 34\\
             9
           & 0 1 4 12 23 8 13 25 34
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld $+$} \cVar{c}[b]
           & \cCode{ld} $\mNT{R}[7]$ \cCode{ld} \cVar{c}[c] \cCode{\$}
           & shift to 9\\
             10
           & 0 1 4 12 23 8 13 25 34 9
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld $+$} \cVar{c}[b] \cCode{ld}
           ...
           & reduce rule 15 ($\mNT{R}[2]$ $\rightarrow$ \cCode{ld} $\mNT{R}[1]$)\\
           & & & & assign result to r8\\
           & & & & emit \instrCode{``load r8,*r7''}\\
           & & & & shift to 38\\
             12
           ...
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld $+$} \cVar{c}[b] $\mNT{R}[8]$
             \cCode{ld}
           & \cVar{c}[c] \cCode{\$}
           & shift to 18\\
             14
           ...
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld $+$} \cVar{c}[b] $\mNT{R}[8]$
             \cCode{ld} \cVar{c}[c]
           & \cCode{\$}
           & reduce rule 16 ($\mNT{R}$ $\rightarrow$ \cCode{ld C})\\
           & & & & assign result to r9\\
           ...
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ \cCode{$+$ ld $+$} \cVar{c}[b] $\mNT{R}[8]$
             $\mNT{R}[9]$
           & \cCode{\$}
           & reduce rule 1 ($\mNT{R}[2]$ $\rightarrow$ \cCode{$+$ ld $+$ C} $\mNT{R}[1]$
                            $\mNT{R}[2]$)\\
           & & & & emit \instrCode{``add r9,B,r8''}\\
           & & & & shift to 32\\
             16
           & 0 1 4 12 23 32
           & \cCode{$=$ $+$} \cVar{c}[a] $\mNT{R}[7]$ $\mNT{R}[2]$
           & \cCode{\$}
           & reduce rule 8 (\quad $\rightarrow$ \cCode{$=$ $+$ C} $\mNT{R}[1]$ $\mNT{R}[2]$)\\
           & & & & emit \instrCode{``store r9,A,r7''}\\
             17
           & 0
           &
           & \cCode{\$}
      ...
      \caption[Execution walk-through of the Glanville-Graham approach]%
        {%
          ...
          selector on an expression \irCode*{a = b + c}, where~\irVar*{a},
          \irVar*{b} and \irVar*{c} are constants residing in memory, which
          yielded the IR~code \mbox{\cCode*{$=$ $+$} \cCode*{C}[a] $\mNT{R}[7]$ \cCode*{$+$}
            \cCode*{ld $+$} \cCode*{C}[b] \cCode*{ld} $\mNT{R}[7]$ \cCode*{ld} \cCode*{C}[c]}, where
          $\mNT{R}[7]$ is the base address register.
  %
  The execution is based on the
          precomputed table from \refFigure{glanville-graham-table}.
  %
  The
  ...
  First, a reduce is executed using
          rule~15, which pops \mbox{\cCode{ld} $\mNT{R}[7]$} from the symbol
          stack.
  %
  ...
  In this case, input symbol $\mNT{R}[8]$ at state~34 leads
          to a shift to state~38~\cite{GlanvilleGraham:1978}}%
...
\refFigure{glanville-graham-instruction-grammar}, \glspl{rule}~5 and~6 are two
such \glspl{rule}).
%
...
%          & \gA{EMIT} \cVar{``addb2 \gInhAttr{a},\gInhAttr{r}''}\\
%        5 & \gByteSynAttr{r}
%          & \cCode{$+$} \gByteSynAttr{a} \gByteSynAttr{b}
%          &
...
%      3 & \mNT{reg}[B]
%        & \mNT{reg}[B]\hspace{1.5mm}\cCode{$+$}\hspace{1.5mm}\mNT{reg}[B]\\
%    \end{tabular}
%  \end{framedBoxWI}
%\end{center}
%
...
%%
%Variables \cCode{A} and \cCode{B} are
...
\refSection{tc-glanville-graham}) using a language called \gls{CGL}, which as
...
%          4 & \mNT{a} & \mNT{R}\\
%          5 & \mNT{a} & \cCode{C}\\
%          6 & \mNT{a} & \cCode{$+$} \cCode{C} \mNT{R}\\
%          7 & \cCode{C}  & \cCode{0}\\
%          8 & \mNT{x} & \cCode{$+$} \mNT{x} \cCode{0}\\
%          9 & \cCode{$+$} \mNT{y} \mNT{x} & \cCode{$+$} \mNT{x} \mNT{y}\\
%         10 & \mNT{op} \mNT{x} \mNT{y} & \cCode{$+$} \mNT{x} \mNT{y}\\
%        \end{tabular}
%      \end{framedBoxWI}
%    \end{minipage}%
%  \end{subfigure}
...
%      \mbox{\cCode{$+$} \cCode{0}
%        \cCode{$+$} \cCode{C} \cCode{C}} and the grammar shown in
...
%
%\begin{enumerate}
%  \item $\mNT{lhs}$ $\rightarrow$ $\cCode{Op}$ $\mNT{n}[1]$ $\ldots$ $\mNT{n}[k]$,
...
%      % LAYOUT FIX: prevent first row from touching the line above
%      \rule{0pt}{2ex}%
%      \mNT{reg} & \cCode{Ld} \cCode{$+$} \cCode{Const} \cCode{Const} & 2
%        & & & \mNT{reg} & \cCode{Ld} \mNT{n}[1] & 2\\
%      \multicolumn{3}{l}{} & & & \mNT{n}[1] & $+$ \mNT{n}[2] \mNT{n}[2]
%        & 0\\
%      \multicolumn{3}{l}{} & & & \mNT{n}[2] & \cCode{Const} & 0\\[2ex]
