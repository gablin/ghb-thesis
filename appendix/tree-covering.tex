...
\refAppendix{macro-expansion} (see \refSection{me-expression-tree}).
%
...
\begin{enumerate*}[label=(\roman*), itemjoin={;\ }, itemjoin*={; and\ }]
  \item an increment version \cCode*{incb} (increments a \gls{register} by~1,
...
computing the \gls!{transitive closure}\labelPage{transitive-closure} -- to
yield new \glspl{pattern} (see
...
\begin{figure}
  ...
  \caption[Tree pattern matching example using Hoffmann-O'Donnell]%
          {%
            ...
            \mbox{$\mSigmaTree{a}{\mSigmaTree{a}{v, v}, b}$} are labeled 0, 1,
...
\begin{algorithm}[t]
  ...
  {%
    ...
    \ForEach{%
      $\text{subpattern $s$} = \mSigmaTree{a}{s_1, \ldots, s_m} \in S$ in
...
\begin{algorithm}[t]
  ...
  {%
    ...
    \ForEach{%
      $\text{subpattern $s$} = \mSigmaTree{a}{s_1, \ldots, s_m} \in S$
...
The design is centered around the following assumption: the cost of reducing
...
described in \refFigure{burs-rule-selection-algorithm}.
%
...
\labelSection{tc-normal-form-grammar}
