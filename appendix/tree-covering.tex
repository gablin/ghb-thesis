...
%
%      \begin{algorithmic}[1]
%        \STATE{initialize matchset as empty}
%        \FORALL{$\text{pattern} p \in P$}
%          \IF{\tAlgName{Matches}($n$, $p$)}
%            \STATE{add $p$ to matchset}
%          \ENDIF
%        \ENDFOR
%        \STATE{return matchset}
%      \end{algorithmic}
%
%      \vspace{\baselineskip}
%
...
%      \tPadded{%
%        \raisebox{0pt}[1.8ex]{\cCode{``add \dT{r}[1],\dT{r}[2]''}}%
%      }%
%    }^{\tText{action}}
%  }_{\tText{rule}}
%\end{displaymath}%
...
%        & \gA{EMIT} \dataTerm{``add \dT{r}[1],\dT{r}[2]''}\\
%      2 & \mNT{reg}
%        & \mT{$*$}\hspace{1.5mm}\mNT{reg}[1]\hspace{1.5mm}\mNT{reg}[2] & 1
%        & \gA{EMIT} \dataTerm{``mul \dT{r}[1],\dT{r}[2]''}\\
%      3 & \mNT{reg} & \mT{Int} & 1
...
%          & \gA{EMIT} \dT{``addb2 \gInhAttr{a},\gInhAttr{r}''}\\
%        4 & \gByteSynAttr{r}
%          & \mT{$+$} \gByteSynAttr{r} \gByteSynAttr{a}
%          & \mPredicate{TwoOp}(\gInhAttr{a}, \gInhAttr{r})
%          & \gA{EMIT} \dT{``addb2 \gInhAttr{a},\gInhAttr{r}''}\\
...
%          & \multicolumn{3}{c}{}
%          & \gA{EMIT} \dT{``addb3 \gInhAttr{r},\gInhAttr{a},\gInhAttr{b}''}\\
%      \end{tabular}
%    \end{framedBoxWI}
%
...
skip directly to \refSection{precomputing-cost-computations}\unskip%
...
\refSubfigure{tree-covering-dilemma-input}, which will be covered using the
...
\refSubfigure{tree-covering-dilemma-invalid-coverage}).
...
\glspl{pattern} (see
\refSubfigure{tree-covering-dilemma-combined-coverage}).
%
The third and last
...
(see \refSubfigure{tree-covering-dilemma-augmented-coverage}).
...
for example~\citeNoMbox{Karp1972, Hoffmann1982, Purdom1985, Weisgerber1988,
...
\cCode*{shis}, it is first given as argument to the $\mFunctionName{goto}$
function.
%
...
\dT{i}, however, there exists no corresponding \gls{edge} from the current state
...
\refSubfigure{hoffmann-odonnell-example-subject}, and the \glspl{matchset} are
...
%                  \st
%                  edge $\mDEdge{s_i}{s'_i} \in \mGS, \quantsep
%                        \forall 1 \leq i \leq m$}
%                \STATE{add $\mDEdge{s}{s'}$ to $\mGS$}
%              \ENDIF
%            \ENDFOR
%          \ENDFOR
%        \end{algorithmic}
...
\refSubfigure{hoffmann-odonnell-example-subsumption}\footnote{There is also a
...
algorithm given in \refSubfigure{hoffmann-odonnell-subsumption-algorithm}.
%
The
...
outlined in \refSubfigure{hoffmann-odonnell-table-generation}.
%
First, we find
...
\refSubfigure{hoffmann-odonnell-example-table-a,
...
\refSubfigure{table-compression-uncompressed}.
%
...
information, as seen in \refSubfigure{table-compression-compressed}.
%
By denoting
...
%                     \text{$r \in$ matchset of $n$ \OR $r$ is a chain rule}%
%                   }$%
%            }%
...
\refSubfigure{compute-costs-dp-algorithm}, is as follows.
%
For each
...
is shown in \refSubfigure{pattern-selection-and-code-emission-dp}).
%
Starting
...
The \tDPscheme has several advantages over those based on \gls{LR parsing}.
...
given in \refSubfigure{burs-example-rewrite-system}, and \textcite{Dold1998}, as
...
shown in \refSubfigure{burs-example-lr-graph}).
%
...
To achieve \gls{optimal.ps} \gls{pattern selection}, the \glspl{local
...
given in \refSubfigure{burs-example2-tree-unbounded}.
%
...
seen in \refSubfigure{burs-example2-tree-bounded}.
%
...
discussed on \refPage{chase-table-compression}).
...
\labelPage{linear-form-grammar}

In the same paper, \citeauthor{Balachandran1990} also introduce the idea of
\glspl{grammar} being in \emph{\glsshort{linear-form gr}}, which means that the
...
into \glsshort{linear-form gr} by introducing new
...
%        & \multicolumn{3}{c}{Linear-form grammar}\\
%    \end{tabular}%
%  \end{framedBoxWI}%
%\end{center}
...
My tentative guess is that \gls{BURG} was derived
  from the \gls{BURS} acronym and stands for \tNewName{Bottom-Up Rewrite
    Generator}.}, which since its publication in 1992 has sparked a naming
...
\gls{machine grammar} is in \glsshort{linear-form gr}.
%
The queue is
...
in memory~(\dT{m}) or in a particular \gls{register}~(\dT{r}[x]), and its value
...
\glspl{register} \dT{r}[1] and \dT{r}[2] yields the following \gls{OVA}
%
\def\mTL{\text{TL}}
\def\mRA{\text{RA}}
%
...
%
Even
\glspl{disjoint-output instruction}, where each individual operation can be
modeled as \glspl{tree}, cannot be selected because \gls{tree covering}-based
\glspl{instruction selector} can only consider a single \gls{pattern tree} at a
time.

The third disadvantage is that \glspl{expression tree} typically cannot model
control flow.
%
For example, a \mbox{\techTerm{for} loop} statement requires a
cyclic \gls{edge} between \glspl{block}, which violates the definition of
\glspl{tree}.
%
For this reason, \gls{tree}-based \glspl{instruction selector} are
limited to selecting \glspl{instruction} for a single \gls{expression tree} at a
time -- this is called \gls{local instr-sel}[ \gls{instruction selection}] -- and
handle \gls{assembly code} emission for control flow using a separate
component.
%
This in turn excludes matching and selection of \glspl{inter-block
  instruction}, whose behavior incorporates control flow.
...
\glspl{target machine}.
%
In the next chapter we will look at a more general
