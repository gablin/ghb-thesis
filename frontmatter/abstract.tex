...
\begin{abstract}
  ...
  %
  In this dissertation, I introduce a new approach called \gls!{universal
    instruction selection} that integrates \gls{global.is} \gls{instruction
    selection} with \gls{global code motion} and \gls{block ordering}.
  %
  ...
  %
  I formulate a novel model that is simpler and more flexible compared to the
  techniques used in modern \glspl{compiler} and that captures crucial features
  ignored by other combinatorial approaches.
  %
  I also proposes extensions to the model for integrating \gls{instruction
    scheduling} and \gls{register allocation}, two other important problems of
  \gls{code generation}.
  ...
  Through experimental evaluation, I demonstrate that \gls{universal instruction
    selection} can handle architectures with a rich \gls{instruction set} and
  ...
  %
  I also demonstrate that there is sufficient data parallelism to be exploited
  through selection of \glsunset{SIMD.i}\gls{SIMD.i}\glsreset{SIMD.i}
  \glspl{instruction} and that this exploitation benefits from \gls{global code
    motion}.
  %
