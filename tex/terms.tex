...
\newTerm{JIT compilation}[first={JIT (just-in-time) compilation}]
...
\newTerm{recomputation}[see={rematerialization}, user1={recompute}]
\newTerm{refactoring}
...
\newTerm{rematerialization}[see={recomputation}]
\newTerm{saturation arithmetic}
\newAcronym{SSA}{static single assignment}
\newTerm{SSA graph}
\newTerm{status flag}
\newTerm{syntactic analysis}
...
\newAcronym{RISC}%
           ...
           [firstplural={reduced instruction-set computers (RISCs)}]
\newTerm{Sparc}
...
\newTerm{graph homomorphism}[see={homomorphism}]
\newTerm{graph isomorphism}
\newTerm{homomorphism}[see={graph homomorphism}]
...
\newTerm{variable}[see={decision variable}]
...
\newAcronym{BNF}{Backus-Naur form}
...
\newTerm{predicate}
\newTerm{production}
\newTerm{production rule}[see={production}]
...
\newTerm{conflict graph}[see={interference graph}]
...
\newTerm{interference graph}[see={conflict graph}]
...
\newTerm[A* search]{A$^{*}$~search}[sort={A* search}, user1={A$^{*}$}]
...
\newTerm{anchor node}
\newTerm{answer set programming}
\newTerm{ant colony optimization}
\newTerm{base}
\newTerm{base cost}
\newTerm{base pattern}
\newTerm{binate covering}[user1={binate}, see={unate covering}]
\newTerm{box node}
...
\newTerm{chromosome}[see={gene}]
\newTerm{Click-Paleczny graph}
\newTerm[CO graph]{CO~graph}[first={connection-operation (CO) graph}]
\newTerm{combiner}
...
\newTerm{immediately subsume}[see={subsume}]
\newTerm{index map}
...
\newTerm[reverse.pn]{reverse}[parent={Polish notation}, see={postfix notation}]
\newTerm{postfix notation}[see={reverse.pn}]
...
\newTerm{strictly subsume}[see={subsume}]
...
\newTerm{subpattern}
\newTerm{subsume}[see={strictly subsume}]
...
\newTerm{unate covering}[user1={unate}, see={binate covering}]
...
\newAcronym{FBB}%
           ...
           [firstplural={functional building blocks (FBBs)}]
...
\newTerm[C--]%
        {{%
          \leavevmode{%
            \hbox{C\hskip -0.2ex\raise 0.4ex\hbox{\tiny $--$}}%
          }%
        }}%
        [sort={C--}]
\newTerm[CSharp]{C\#}
