...
\begin{tikzpicture}
  \node [state node] (st1) {};
  \node [op node, position=-45 degrees from st1] (ld) {\opLoad};
  ...
  \node [op node, position=-45 degrees from st2] (foo) {\opCall{foo}};
  \node [state node, position=45 degrees from foo] (st3) {};
  \node [value node, position=-45 degrees from foo] (t2) {\opTemp{2}};
  \node [op node, position=-45 degrees from st3] (st) {\opStore};
