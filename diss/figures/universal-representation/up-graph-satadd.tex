...
\begin{tikzpicture}[
  ...
  \node [computation node, position=- 45 degrees from d1] (phi) {\opPhi};
  \node [computation node, position=-135 degrees from d1] (cmp) {\opLT};
  \node [value node, position=135 degrees from cmp] (MAX-1) {\opVar{MAX}};
  \node [value node, position=45 degrees from phi] (MAX-2) {\opVar{MAX}};
