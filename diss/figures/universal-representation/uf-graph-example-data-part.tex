...
\begin{tikzpicture}[
  ...
  \node [computation node, position=-45 degrees from n1] (phi-n) {\opPhi};
  \node [value node, below=of phi-n] (n2) {\opVar{n}[2]\strut};
  \node [computation node, position=-135 degrees from n2] (leq) {\opLE};
  \node [value node, below=of leq] (bool) {b};
  \node [value node, position=135 degrees from leq] (leq-1) {\opVar{1}};
  \node [computation node, below right=1.5\opNodeDist and .25\opNodeDist of n2]
        (sub) {\opSub};
  ...
  \node [computation node, position=-135 degrees from f2] (mul) {\opMul};
