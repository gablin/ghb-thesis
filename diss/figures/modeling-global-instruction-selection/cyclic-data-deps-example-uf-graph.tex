...
\begin{tikzpicture}[
  ...
  % SSA
  \node [value node] (q1) {\opVar{q}[1]};
  \node [computation node, position=-45 degrees from q1] (add-q) {\opAdd};
  \node [value node, below=of add-q] (q2) {\opVar{q}[2]};
  \node [computation node, below=1.75\opNodeDist of q2]
        (store-q) {\opStore};
  \node [computation node, right=3.75\opNodeDist of add-q] (add-p) {\opAdd};
  \node [value node, position=45 degrees from add-p] (p1) {\opVar{p}[1]};
  \node [value node, below=of add-p] (p2) {\opVar{p}[2]};
  \node [computation node, below=1.75\opNodeDist of p2]
        (store-p) {\opStore};
  \coordinate (between-adds) at ($(add-q) !.5! (add-p)$);
  \node [value node] (c4) at (between-adds |- q1) {\opVar{4}};
