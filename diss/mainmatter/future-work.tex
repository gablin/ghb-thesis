...
If a \gls{datum}~$d$ is not \gls{killed.d}, then the start of a \gls{live range}
for a \gls{datum}~$d$ is determined by the \gls{match} \glsshort{define.d}[ing]
$d$\hspace{-1pt}.
%
This can be modeled as
%
\begin{equation}
  ...
  \end{array}
  \labelEquation{live-range-start}
\end{equation}
