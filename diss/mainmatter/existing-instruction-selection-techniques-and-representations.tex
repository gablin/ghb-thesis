...
\begin{equation}
  \begin{array}{rll}
    \text{minimize}
      & \multicolumn{2}{c}{
          \displaystyle\sum_{1 \leq i < j \leq n} \mVar{x}_i^T C_{ij} \, \mVar{x}_j +
          \displaystyle\sum_{1 \leq i \leq n} \mVector{c}_i^T \mVar{x}_i,
        } \\
    \text{subject to}
...
decision~$i$.
%
For each decision variable, the condition
...
%
Each \gls{complex.m} \gls{match}~$i$ in turn introduces a decision
variable~\raisebox{0pt}[\height-2pt]{$\mVar{x}_i \in \mSet{0, 1}^2$} to decide
whether $i$ is selected.
%
Because of the \raisebox{0pt}[\height-2pt]{$\mVector{1}^T \mVar{x}_i = 1$}
condition, every such variable has exactly two elements (one representing
...
decision variable~$\mVar{x}_i$ with \glspl{match} derived from \gls{proxy.r}
\glspl{rule}.
%
...
%
\begin{equation}
  \sum_{i \in N, j \in M} \mVar{x}_i^T D_{ij} \, \mVar{x}_j
\end{equation}
%
...
matrix, and $\mVar{x}$ is a set of integer decision variables.
%
...
%
\begin{equation}
  \sum_{i \in \mCovers(n)} \mVar{x}_i = 1, \forall n \in N,
\end{equation}
%
where $N$ denotes the set of nodes in a \gls{block DAG}, $\mCovers(n)$ denotes
the set of \glspl{match} covering node~$n$, and $\mVar{x}_i$ is a Boolean
decision variable indicating whether match~$i$ is selected.
...
\footnote{If there are not enough \glspl{register} to allocate for all
  \gls{program} variables, then one or more \glspl{register} must be freed.
  %
...
\mbox{$\sum_{i \in \mCovers(n)} \mVar{x}_i \leq 1$}.
...
%
Consequently, in addition to the variables that decide which \glspl{match} are
selected, their \gls{IP}~model also has decision \glspl{variable} that, for each
\gls{match}, maps \glspl{node} and \glspl{edge} in the \gls{block DAG} to
\glspl{node} and \glspl{edge} in a \gls{pattern}.
%
...
%
This is called \gls!{instruction compaction}, and as there is an abundance of
overlap between such combinations, \citeauthor{LeupersMarwedel:1995} solved this
problem using \gls{IP}.
...
\todo{discuss \cite{BashfordLeupers:1999}}

\todo{discuss \cite{Beg:2013}}

\todo{discuss \cite{FlochEtAl:2010}}

\todo{discuss \cite{MartinEtAl:2009, MartinEtAl:2012}}
