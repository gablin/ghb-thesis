...
each result.
%
\labelPage{multi-output-grammars}
%
\Glspl{pattern} in a \gls{complex.r} \gls{rule} are called \gls!{split.p}[
  \glspl{pattern}] and are matched individually.
%
After \gls{pattern matching}, \glspl{match} derived from \gls{split.p}
...
%
First assume a \gls{grammar} similar to that of
\textcite{ScharwaechterEtAl:2007} (see \refPage{multi-output-grammars}), where
multi-output \glspl{instruction} are described using \gls{complex.r}
\glspl{rule}.
%
In this context, \glspl{rule} with a single \gls{production} are called
\gls!{simple.r}[ \glspl{rule}], and \glspl{production} of \gls{complex.r}
\glspl{rule} are loosely called \gls!{proxy.r}[ \glspl{rule}].

For each combination of \glspl{match} derived \gls{proxy.r} \glspl{rule} that
can be combined into an instance of a \gls{complex.r} \gls{rule}, a
...
%
Because of the \raisebox{0pt}[\height-2pt]{$\mVar{x}_i \mVector{1}^T = 1$}
condition, every such variable has exactly two elements (one representing
\emph{on} and the other \emph{off}).

\todo{cost vectors and matrices for complex rules}

\todo{selection of complex rule requires selection of all of its split rules}
