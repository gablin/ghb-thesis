...
\refFigure{p-match-sel-example-graph} can be expressed as \mbox{$\opLoad \;
...
\paragraph{Precomputing Costs and Pattern Selection}
...
\begin{algorithm}[t]
  \DeclFunction{ComputeStates}{grammar $G$}{%
    $S$ \Assign $\emptyset$\;
    $Q$ \Assign $\emptyset$\;
    \Call{ComputeLeafStates}{}\;
    \While{$Q \neq \emptyset$}{%
      $s$ \Assign pop from $Q$\;
      \ForEach{terminal $\mathit{op} \in G$}{%
        \Call{ComputeTransitions}{$s$, $\mathit{op}$}\;
      }
    }
    \Return{$S$}\;

    \BlankLine
    \DeclFunction{ComputeTransitions}{state $s$, terminal $t$}{%
      ...\;
    }

    \BlankLine
    \DeclFunction{NormalizeCosts}{state $s$}{%
      $d$ \Assign $\min(\mSetBuilder{s[n].\text{cost}}%
                                    {\text{nonterminal} \; n \in G})$\;
      \ForEach{nonterminal $n \in G$}{%
        $s[n]$.cost \Assign $s[n].\text{cost} - d$\;
      }
      \Return{$s$}\;
    }

    \BlankLine
    \DeclFunction{ComputeClosure}{state $s$}{%
      
    }
  }

  \caption[%
            Algorithm for constructing the states for a given machine grammar%
          ]{%
            Algorithm for constructing the states for a given machine grammar
            %
            The given grammar is assumed to be in linear form%
          }
  \labelAlgorithm{proebsting-state-algorithm}
\end{algorithm}

The idea work as follows.
%
...
\glspl{state} since \glspl{expression tree} can be of arbitrary size.
%
Fortunately,
