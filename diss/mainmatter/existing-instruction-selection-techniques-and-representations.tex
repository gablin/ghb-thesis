...
%
\RefSection{ex-isel-rep-instr-chars-principles} introduces a set of
...
\glspl{constraint}, and an \gls{objective function}.
%
...
\section{Instruction Characteristics and Principles}
\labelSection{ex-isel-rep-instr-chars-principles}

\Gls{instruction selection} has been actively researched in over four decades --
the first publications began to appear at the end of the 1960s -- and advances
are continuously made as processors become more complex.
%
When surveying the existing methods, it was discovered that essentially all
apply one of four fundamental \glspl!{principle} of \gls{instruction selection}
and that their capabilities can be compared in terms of handling
\glspl{instruction} with five \glsplshort!{instruction characteristic}.
%
We first introduce the \gls{instruction characteristic} and then the
\gls{principle}, together with a discussion of how the existing methods are
categorized according to these concepts.
%
For sake of brevity, only a brief discussion is given in this chapter as a
complete comparison is superfluous for understanding the rest of the
dissertation.
%
The full discussion is given in
\refAppendixRange{macro-expansion}{graph-covering}, and a table listing all
existing methods is given in \refAppendix{list-of-approaches}.


\subsection{Instruction Characteristics}

The set of \gls{instruction} can be said to exhibit five \glsplshort{instruction
...
\subsubsection{Single-Output Instructions}
...
memory operations with complicated addressing modes).
%
...
\subsubsection{Multi-Output Instructions}
...
\subsubsection{Disjoint-Output Instructions}
...
%
This typically includes \gls{SIMD.instr} \glspl{instruction}, which execute the
same operations simultaneously on many distinct input values.
...
\subsubsection{Inter-Block Instructions}
...
\subsubsection{Interdependent Instructions}
...
\instrCode{rpt}~instruction if a particular addressing mode is used for the
\instrCode{add}~instruction.
...
\subsection{Principles of Instruction Selection}

An \gls{instruction selector} can be said to apply one of four fundamental
\glspl{principle} -- \gls{macro expansion}, \gls{tree covering}, \gls{DAG
  covering}, and \gls{graph covering} -- where \gls{DAG covering} is a
generalization of \gls{tree covering} and \gls{graph covering} is a
generalization of \gls{DAG covering}.
%
As seen in \refFigure{principles-timeline}, the trend of \gls{macro expansion}
was quickly overtaken by \gls{tree covering}, which in turn is in decline in
favor of \gls{DAG covering}. Applications of \gls{graph covering} have been
scarce until recently due to the inherent increase in complexity.

\begin{figure}
  \centering%
  \def\numTotalPublications{?}% Will be redefined in the file below
  ...
  \caption[Principle timeline diagram]%
          {%
            ...
            (\tikz{\node [nothing, fill=black, minimum height=1.25ex,
                         ...
                         {}} represent one publication)%
          }
  \labelFigure{principles-timeline}
\end{figure}

Without loss of generality, assume that the input to the \gls{instruction
  selector} consists of a single \gls{function}, which in turn consists of many
\glspl{basic block} (henceforth referred to as simply \glspl!{block}).
%
...
\begin{figure}
  ...
  \caption[Example of a macro]%
          {%
            ...
            %
            The template to match is given as argument to {\codeFont expand},
            and the procedure to run upon expansion is given as
            {\codeFont expand}'s body%
          }
...
Applications of \gls{naive.me} \gls{macro expansion} and the
\gls{Davidson-Fraser approach} are examined in \refAppendix{macro-expansion}.

\subsubsection{Tree Covering}
...
%
For example, assume that the \instrCode{mv}, \instrCode{add}, \instrCode{mul},
and \instrCode{muladd} \glspl{instruction} in \refFigure{p-match-sel-example}
all take one cycle to execute whereas the \instrCode{load} and
\instrCode{amload} \glspl{instruction} take five cycles to execute.
%
...
\Gls{tree covering} has several advantages over \gls{macro expansion}.
%
First, modeling the \glspl{instruction} as \glspl{pattern} makes it simpler to
capture the behavior of \gls{instruction} that cover multiple \glspl{operation}.
%
Second, as it separates \gls{selection} from \gls{matching}, tradeoffs in
selecting certain combinations of \glspl{match} can be considered, which
improves code quality.
%
Third, \gls{tree covering} can be based on a formal foundation which enables
proof of completeness.
%
The main limitation of \gls{tree covering} is that because both the
\glspl{function} and \glspl{instruction} must be modeled as \glspl{tree},
\glspl{instruction selector} based on this \gls{principle} effectively only
support \gls{single-output.ic} \glspl{instruction}.
%
Restricting oneself to \glspl{expression tree} may also lead to redundant code
duplication, as will be explained in
\refSection{ex-isel-rep-traditional-approaches}.

This chapter considers the traditional and most well-known applications of
\gls{tree covering}.
%
For the complete examination, see \refAppendix{tree-covering}.


\subsubsection{DAG Covering}
...
\glspl{pattern tree} or \glspl{pattern DAG}, we attain a more general principle
called \gls!{DAG covering}.
%
Unlike \gls{tree covering}, which can be solved optimally in linear time,
finding the \gls{least-cost.c} \gls{cover} of a \gls{block DAG} is NP-complete.
%
...
%
Several near-optimal approaches to \gls{DAG covering} also exist.

\Gls{DAG covering} has several advantages of \gls{tree covering}.
%
First, as will be explained shortly, the \gls{block DAG} does not need to be
decomposed into \glspl{expression tree}, which compromises code quality.
%
Second, it enables selection of \gls{multi-output.ic} \glspl{instruction}, which
must be modeled as \glspl{pattern DAG}.
%
The main disadvantage of \gls{DAG covering} is increased complexity and no
support for \gls{inter-block.ic} \glspl{instruction}.

This chapter only considers the \glspl{combinatorial approach} to \gls{DAG
  covering}.
%
For the complete examination, see \refAppendix{dag-covering}.


\subsubsection{Graph Covering}
...
This chapter only considers the \glspl{combinatorial approach} to \gls{graph
  covering}.
%
For the complete examination, see \refAppendix{graph-covering}.
...
times, most modern \glspl{compiler} either limit themselves to \gls{tree
  covering} or apply greedy, linear-time heuristics.
%
In \glspl{tree covering} one can find \gls{least-cost.c} \glspl{cover} in linear
time, but at the price of precluding optimization decisions that only become
possible when operating on \glsshort{block DAG} or \glspl{function graph}.


\subsection{Approaches Based on Tree Covering}

The most common and well known technique based on \gls{tree covering} was
...
\subsection{Approaches Based on DAG Covering}
...
\subsection{Approaches Based on Graph Covering}
...
in linear time, nearly all \glspl{combinatorial approach} are based on \gls{DAG
  covering} with one exception, which is based on \gls{graph covering}.
%
...
\gls{instruction} is decomposed into trees -- making it a
\gls{disjoint-output.ic} \gls{instruction} -- but in this approach they are used
