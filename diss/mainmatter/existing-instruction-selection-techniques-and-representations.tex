  ...
  \begin{array}{rll}
    \text{minimize}
      & \multicolumn{2}{c}{%
          ...
          \displaystyle\sum_{1 \leq i \leq n} c_i \mVar{x}_i^T\!,%
        } \\
    \text{such that}
      & \mVar{x}_i \in \mSet{0, 1}^{|c_i|}\!
      & \forall 1 \leq i \leq n, \\
    \text{and}
      & \mVar{x}_i \mVar{1}^T = 1
      & \forall 1 \leq i \leq n. \\
  \end{array}
\end{equation}
%
...
%
Then $\mVar{x}_i$ is a vector of length~$k$, where \mbox{$\mVar{x}_i[j] = 1$}
means that choice~$j$ has been selected for decision~$i$, and \mbox{$\mVar{x}_i
  \mVar{1}^T = 1$} ensures that exactly one choice is selected.
%
...
to cover \gls{node}~$i$, $c_i$ contains the cost for each such \gls{match}, and
$C_{ij}$ contains the cost of additional \glspl{instruction} that may need to be
selected due to certain combinations of \glspl{match}.
%
...
%
An example of a full \gls{PBQP} instance is shown in \refFigure{pbqp-example}.
...
\todo{discuss \cite{EcksteinEtAl:2003}}

\todo{discuss \cite{EbnerEtAl:2008}}
