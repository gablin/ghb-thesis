...
\NewDocumentCommand{\mCosts}{}{\mathit{costs}}
\NewDocumentCommand{\mRules}{}{\mathit{rules}}

\begin{algorithm}[t]
  \DeclFunction{ComputeCosts (expression tree rooted at node $n$)}{%
    \ForEach{child $m$ of $n$}{
      ComputeCosts ($m$) \cmt*{compute costs for all subtrees}
    }
    initialize array $\mCosts_n$ with $\infty$\;
    $ R \leftarrow
        \mSetBuilder{\text{rule $r$}}%
                    {\text{$r$ $\in$ match set of $n$ \Or $r$ is a chain rule}}
    $\;
    \ForEach{rule $r \in R$, in transitive reduction order}{%
      $c \leftarrow \text{cost of applying $r$ at $n$}$\;
      $l \leftarrow \text{left-hand nonterminal of $r$}$\;
      \If{$c < \mCosts_n[l]$}{%
        $\mCosts_n[l] \leftarrow c$\;
        $\mRules_n[l] \leftarrow r$\;
      }
    }
  }
