...
\begin{equation}
  \begin{array}{rl}
    \text{maximize} & \mVector{c}^T \mVar{x} \\
    \text{subject to} & A \mVar{x} \leq \mVector{b}, \\
      & \mVar{x} \geq 0, \\
    \text{and} & \mVar{x} \in \mathbb{Z}^n, \\
  \end{array}
\end{equation}
%
where $\mVector{c}$ and $\mVector{b}$ are integer vectors and $A$ is an integer
matrix.
%
...
\textcite{BednarskiKessler:2006}, whose work was later applied by
\citeauthor{ErikssonEtAl:2008}~\cite{ErikssonEtAl:2008, ErikssonKessler:2012}.
%
\todo{discuss pattern matching}
\todo{say something about the approach}
