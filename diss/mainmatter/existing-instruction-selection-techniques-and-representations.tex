...
Another method for solving combinatorial optimization problems is \gls!{CP},
which is discussed in detail in \refChapter{constraint-programming} (hence only
a brief introduction will be given here).
%
Like \gls{IP}, a \gls{CP}~model consists of a set of \glspl{decision variable},
a set of \glspl{constraint} over the \glspl{variable}, and typically also an
objective function to either minimize or maximize.
%
...
%
With these, \gls{pattern selection} can be modeled as
%
\begin{equation}
  \mCount(m, \mVar{match}, =, \mVar{count}_m), \forall m \in M.
\end{equation}
%
Because of the domain of the $\mVar{count}$~\glsplshort{decision variable}, each
match covers either no \glspl{node} or all \glspl{node} in its \gls{pattern}.
