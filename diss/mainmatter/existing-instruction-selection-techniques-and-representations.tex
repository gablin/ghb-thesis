...
\begin{algorithm}[p]
  \DeclFunction{ComputeCosts}%
               {expression tree $T$, linear-form grammar $G$}%
  {%
...
\gls{grammar}.
%
For simplicity the algorithm assumes the \gls{grammar} to be in \gls{linear
  form.g}, but it can be adapted to accept any \gls{grammar} by expanding the
