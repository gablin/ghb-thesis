...
is also available in \refSection{optimal-p-sel-np-completeness-proof}).
%
...
\cite{PalecznyEtAl:2001} first divides the \gls{function graph} into a set of
possibly overlapping \glspl{expression tree}.
%
...
in linear time, nearly all operate on \glspl{block DAG}, with one exception
operating on \glspl{function graph}.
%
Consequently, the approaches in this section are grouped according to the tools
they apply for modeling \gls{instruction selection}.
%
Most approaches also natively support \gls{DAG}-shaped \glspl{pattern} and
therefore apply generic \glshyphened{subgraph isomorphism} algorithms for
performing \gls{pattern matching}.
...
\todo{DAG- and graph-shaped patterns turns the \gls{pattern matching} problem
  into an NP-complete problem~\cite{Cook:1971}}



\subsection{M(W)IS-based Approaches}
