...
\todo{relate rule derivations with covers}
...
%
It is based on dynamic programming and relies on the following assumption: the
cost of deriving nonterminal~$s$ at node~$n$ using rule~$r$ is the cost of~$r$
plus the costs of deriving all right-hand-side nonterminals of~$r$ at the
children of~$n$.
%
The algorithm for computing this cost for an entire \gls{expression tree} is
shown in \refAlgorithm{aho-etal-cost-algorithm}, and the algorithm for selecting
the \gls{least-cost.cover} \gls{cover} is shown in
...
\begin{algorithm}[t]
  \DeclFunction{ComputeCosts (expression tree $T$, match set $M$, grammar $G$)}{
    ...
    \DeclFunction{ComputeCostsRec (root node $n$)}{
      ...
      $R$ \Assign
      $\mSetBuilder{r}%
                   {\text{$r$ matches at $n$ in $M$ \Or
                          $r$ is a chain rule in $G$}}
      $\;
    ...
    \DeclFunction{ComputeRuleCost (root node $n$, rule $r$)}{%
      ...
      \eIf{$r$ is a chain rule}{%
        ...
        \cmt*{here cost of root node is taken instead of its children}
      }{%
  ...
  \caption[%
            Algorithm for computing optimal derivation costs for an expression
            tree in linear time%
          ]{%
            Algorithm for computing optimal derivation costs for an expression
            tree in linear time. It assumes the given grammar to be in linear
            form%
          }
