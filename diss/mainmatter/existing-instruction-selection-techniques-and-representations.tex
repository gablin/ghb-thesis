...
\gls{base.r} \glspl{rule} are processed before any \gls{chain.r} \gls{rule}, and
if two \gls{chain.r} \glspl{rule}~$i$ and~$j$ are of the format \mbox{$A
  \leftarrow B$} and \mbox{$B \leftarrow C$}, then $j$ is processed before $i$
-- then each \gls{rule} need to be processed only once per node.
%
