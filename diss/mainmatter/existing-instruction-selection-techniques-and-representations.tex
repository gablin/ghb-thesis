...
$G$ denotes either a \gls{block DAG} or a \gls{function graph}.
%
...
\begin{figure}
  ...
  \hfill%
  \subcaptionbox{%
                  Corresponding expression trees, where the common subexpression
                  has been split%
...
subexpressions have to be either split or duplicated when building the \gls{IR}.
%
...
matches matches~$m_4$ and~$m_5$, resulting in a total cost is \mbox{$4 + 5 =
  9$}.
%
Duplicating the common subexpression would, in this case, yield the same
\glspl{cover}, but would have resulted in suboptimal code in cases where the
\instrCode{addmul} and \instrCode{addload} \glspl{instruction} are not
available.
...
\gls{pattern} that matches the current node, and repeat the process for
...
%
Due to its limited scope, however, it suffers from the same limitation as
\glspl{expression tree} regarding whether to split or duplicate common
subexpressions.
