...
\begin{definition}[PBQP]%
  ...
  \begin{displaymath}
    \begin{array}{rl}
      ...
      \text{subject to}
        & \forall_{\! i} \: \mVector{1}^T \mVar{x}_i = 1. \\
    \end{array}
  \end{displaymath}%
\end{definition}
...
%
The \gls{constraint}, typically referred to as $\mCount$ or $\mGCC$, constrains
the number of \glspl{decision variable} assigned a particular value (which may
also be a \gls{decision variable}).
%
Formally, the \gls{constraint} is defined as follows (see
\refChapter{constraint-programming} for explanation of notations).
%
\begin{definition}[\Gls{global cardinality constraint}]%
  ...
  %
  \begin{displaymath}
    \mCount(v, \mVar{x}_1, \ldots, \mVar{x}_k, \mVar{y})
    =
  ...
  \end{displaymath}%
  \labelDefinition{count}%
\end{definition}

For example, the constraint \mbox{$\mCount(4, \mVar{x}_1 \in \mSet{4},
  \mVar{x}_2 \in \mSet{5}, \mVar{x}_3 \in \mSet{6}, \mVar{y} \in \mSet{1})$}
holds as exactly one \glsshort{decision variable} is assigned the value~4.
...
\glspl{constraint} with \refEquation{pattern-selection-using-count}, and then
...
same \glsshort{constraint model}, and \citeauthor{Beg:2013} also applied
...
\refChapter{introduction}, \refPage{saturated-arithmetic}).
