...
\gls{instruction selection} using a model that consists of variables,
...
\begin{figure}
  ...
  \mbox{}%
  \hfill%
  \subcaptionbox{C code\labelFigure{simple-graph-ir-example-c}}%
...
%
The IR assumes the program to be in \gls!{SSA} form, in which a \gls{program}
variable is defined exactly once.
%
For example, the \gls{function} shown in \refFigure{simple-graph-ir-example-c}
is not in \gls{SSA} form as variables~\irCode{f} and~\irCode{n} are redefined
within the loop.
%
...
into \gls{SSA} form, as shown in \refFigure{simple-graph-ir-example-ssa-c}.
