...
\begin{figure}
  ...
  \subcaptionbox{%
                  ...
                  \labelFigure{simple-graph-ir-example-c}%
                }%
                [50mm]%
                {\lstinputlisting[language=c]{fact.c}}%
  \hfill%
  \subcaptionbox{Code in SSA form\labelFigure{simple-graph-ir-example-ssa-c}}%
  ...
  \subcaptionbox{Function graph\labelFigure{simple-graph-ir-example-graph}}%
                {%
                  \small%
                  \input{figures/existing-isel-techniques-and-reps/simple-graph-ir-example-graph}%
                }

  \caption[%
            Example of a function graph represented using a graph-based IR%
          ]%
          {%
            Example of a function graph represented using the IR introduced in
            \cite{ClickPaleczny:1995}.
            %
  ...
  \labelFigure{simple-graph-ir-example}
\end{figure}

\textcite{ClickPaleczny:1995} introduced a graph-based IR for representing
entire \glspl{function}, colloquially referred to as a \gls!{sea of nodes}.
%
The \gls{IR} assumes the \gls{function} to be in \gls!{SSA}
\glsEmph{form}~\cite{CytronEtAl:1991}, in which a \gls{variable} is defined
exactly once.
%
For example, the \gls{function} shown in \refFigure{simple-graph-ir-example-c}
...
\refFigure{simple-graph-ir-example-ssa-c}.
%
The corresponding \gls{function graph} is shown in
\refFigure{simple-graph-ir-example-graph}.
...
be solved in linear time which enabled \citeauthor{ScholzEckstein:2002} to
develop a greedy, linear-time solver.
...
\begin{equation}
  \begin{array}{rll}
    \text{minimize}
      & \multicolumn{2}{c}{%
          \displaystyle\sum_{1 \leq i < j \leq n} \mVector{x}_i C_{ij} \mVector{x}_j^T
          + \displaystyle\sum_{1 \leq i \leq n} c_i \mVector{x}_i^T\!,%
        } \\
    \text{such that}
      & \mVector{x}_i \in \mSet{0, 1}^{|c_i|}\!
      & \forall 1 \leq i \leq n, \\
    \text{and}
      & \mVector{x}_i \mVector{1}^T = 1
      & \forall 1 \leq i \leq n. \\
  \end{array}
\end{equation}
%
Intuitively, one can interpret the definition as follows.
%
Assume a problem consists of $n$~decisions, where each has $k$~choices.
%
Then $\mVector{x}_i$ is a vector of length~$k$, where \mbox{$\mVector{x}_i[j] =
  1$} means that choice~$j$ has been selected for decision~$i$.
%
\mbox{$\mVector{x}_i \mVector{1}^T = 1$} ensures that exactly one choice is
selected for decision~$i$.
%
The cost of selecting a particular choice for decision~$i$ is represented
through vector~$c_i$, and the cost of combining two decisions~$i$ and~$j$ are
represented through matrix~$C_{ij}$.

All \gls{PBQP}-based approaches operate on \glspl{function graph} assumed to be
represented as \glspl{SSA graph}, which will therefore be described first.



\paragraph{SSA Graphs}

\begin{figure}
  \centering%
  \subcaptionbox{Code in SSA form\labelFigure{ssa-graph-example-ssa-c}}%
                {\lstinputlisting[language=c,mathescape]{fact-ssa.c}}%
  \hfill%
  \subcaptionbox{SSA graph\labelFigure{ssa-example-graph}}%
                {%
                  \small%
                  \input{figures/existing-isel-techniques-and-reps/ssa-graph-example-graph}%
                }

  \caption{Example of an SSA graph}
  \labelFigure{ssa-graph-example}
\end{figure}

An \gls!{SSA graph} is a \gls{data-flow graph} that encompasses an entire
\gls{function}.
%
The \glspl{operation} in the \gls{function} -- which is assumed to be in
\gls{SSA}~form -- are represented as \glspl{node} and are connected using
data-flow \glspl{edge}, ignoring the fact that the \glspl{operation} may belong
to different \glspl{block}.
%
Hence the \gls{SSA graph} is identical to the \glshyphened{sea of nodes}~IR
introduced by \citeauthor{ClickPaleczny:1995} minus the control flow, as shown
in \refFigure{ssa-graph-example}.



\paragraph{Instruction Selection as a PBQP}
...
\todo{discus \cite{EcksteinEtAl:2003}}

\todo{discus \cite{EbnerEtAl:2008}}

\todo{discus \cite{BuchwaldZwinkau:2010}}
