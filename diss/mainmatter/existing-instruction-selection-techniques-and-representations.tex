...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            ...
            %
            Thin-lined nodes and solid edges denote data operations and data
            flow.
            %
            Thick-lined nodes and solid edges denote control operations and
            control flow.
            %
            Dashed edges denote optional edges, indicating to which block an
            operation belongs%
          }
...
entire \glspl{function}, colloquially referred to as a \gls!{sea-of-nodes IR}.
%
...
%
In addition, \glspl{variable} are also represented as \glspl{node}, thereby
explicitly representing the data produced and used by the \glspl{operation}.
%
These nodes are then connected using data-flow edges, ignoring the fact that the
\glspl{operation} and \glspl{variable} may belong to different \glspl{block}.
%
...
-- like with \glspl{phi-function} and returns -- then an optional \gls{edge} can
be used to connect the \gls{operation} with its original \gls{block} placement.
...
\todo{mention global code motion}
