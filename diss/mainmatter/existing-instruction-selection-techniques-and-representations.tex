...
\begin{table}[t]
  \centering%
  \begin{tabular}{r@{ $\rightarrow$ }lcl}
    \toprule
    \multicolumn{2}{c}{Production} & Cost & \multicolumn{1}{c}{Action}\\
    \midrule
    $\mNT{Reg}[1]$ & $\opLoad + \mNT{Reg}[2] \; \irCode{imm}$
      & 1
      & emit: {\instrFont load \$$\mNT{Reg}[1]$, imm(\$$\mNT{Reg}[2]$), \#imm}\\
    $\mNT{Reg}[1]$ & $\opLoad + \irCode{imm} \; \mNT{Reg}[2]$
      & 1
      & emit: {\instrFont load \$$\mNT{Reg}[1]$, imm(\$$\mNT{Reg}[2]$), \#imm}\\
    $\mNT{Reg}[1]$ & $\opLoad \; \mNT{Reg}[2]$
      & 1
      & emit: {\instrFont load \$$\mNT{Reg}[1]$, 0(\$$\mNT{Reg}[2]$), \#imm}\\
    \bottomrule
  \end{tabular}%
...
\gls{linear form.g} if every \gls{rule} in the \gls{grammar} is in one of the
following forms:
%
\begin{enumerate}
  ...
  \item \mbox{$\mNT{N} \rightarrow \irCode{t}$}, where \irCode{t} is a
    \gls{terminal}.
    %
    These rules are also called \gls{base.r} \glspl{rule}.
