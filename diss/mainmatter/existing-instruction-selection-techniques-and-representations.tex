...
\begin{figure}
  ...
  \caption[An example of the pattern matching and selection problem]%
          {%
            An example demonstrating the pattern matching and selection problem
...
\begin{figure}
  ...
  \subcaptionbox%
    ...
    {%
      \small%
      \begin{tabular}{cr@{ $\rightarrow$ }l}
        \toprule
        \tabhead \# & \multicolumn{2}{c}{\tabhead Rule}\\
        \midrule
        1 & $\mNT{Reg}$  & \irCode{const}\\
        2 & $\mNT{SReg}$ & \irCode{const}\\
        3 & $\mNT{Reg}$  & $\opAdd \; \mNT{Reg} \; \mNT{Reg}$\\
        4 & $\mNT{SReg}$ & $\opAdd \; \mNT{SReg} \; \mNT{SReg}$\\
        \bottomrule
      \end{tabular}%
      \newcolumntype{L}{@{}l@{}}%
      \begin{tabular}{@{\hspace{3em}}cr@{ $\rightarrow$ }l>{\footnotesize}c}
        \toprule
        \tabhead \#
          & \multicolumn{3}{c}{\tabhead Rule}\\
        \midrule
        5 & $\mNT{SReg}$
          & \multicolumn{2}{L}{$\opMul \; \mNT{Reg} \; \mNT{Reg}$}\\
        6 & $\mNT{Null}$ & \multicolumn{2}{L}{$\opRet \; \mNT{Reg}$}\\
        7 & $\mNT{Reg}$  & $\mNT{SReg}$ & $(r \ll 1)$\\
        8 & $\mNT{SReg}$ & $\mNT{Reg}$  & $(r \gg 1)$\\
        \bottomrule
      \end{tabular}%
    }

  \caption[An example illustrating the need for global instruction selection]%
          {%
            An example illustrating the need for global instruction selection%
          }
