...
\subsubsection{Covering $\mPhi$-Nodes}
...
Although \refAlgorithm{vf2-algorithm} supports matching of \glspl{pattern}
derived from \gls{SIMD.instr} \glspl{instruction}, doing so directly will result
in a factorial number of \glspl{match}.
%
...
\glspl{node} does not matter.

Consequently, instead of matching the full \gls{SIMD.instr} \gls{pattern} over
...
\refSection{forbidding-cyclic-data-dependencies}.
