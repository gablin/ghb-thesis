...
%
When a \gls{computation node} with side effects is created, a data-flow
\gls{edge} is also inserted from the last \gls{state node} to this \gls{node}.
%
A new \gls{state node} is then created and set as last, and another data-flow
...
data-flow \gls{edge} and a \gls{definition edge}, respectively (note that,
unlike with \glspl{value node}, the data-flow \gls{edge} must not be drawn from
...
\begin{figure}
  ...
  \subcaptionbox{%
                  UF subgraph.
                  %
                  Gray boxes represent state nodes%
