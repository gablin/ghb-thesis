...
\refRequirement{uni-rep-capture-data-control} is partially fulfilled.
%
...
%
redirect the \glspl{edge} such that the
control flow originate from these \glspl{node}.
%
For example, for the \gls{control-flow graph} shown in
\refFigure{uf-graph-example-original-cf-graph}, \todo{finish example}
%
This results in the \gls{control-flow graph} given in
\refFigure{uf-graph-example-extended-cf-graph}.

We continue with the \gls{SSA graph}, using
\refFigure{uf-graph-example-original-ssa-graph} as an example.
%
