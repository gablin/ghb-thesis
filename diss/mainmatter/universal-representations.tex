...
%
For sake of \gls{pattern matching}, which will be described in
\refChapter{pattern-matching}, every \gls{edge} has two numbers -- an
\gls!{inbound.en} \gls!{edge number} and an \gls!{outbound.en} \gls!{edge
  number} -- that allows the \glspl{edge} to be ordered \wrt to a given
\gls{node}.
...
\subsubsection{Representing Constants}
