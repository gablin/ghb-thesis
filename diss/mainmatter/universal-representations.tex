...
%
Lastly, \refSection{uni-rep-pattern-matching} describes how to perform
\gls{pattern matching} using the representation.
...
\subsubsection{Representing Constants As Single Or Multiple Nodes}
...
described in \refChapter{modeling-value-reuse}, but this also necessitates the
...
in \refChapter{modeling-data-copying}.
...
\subsubsection{Handling Side Effects}
...
%
For a given \gls{edge}~\mbox{$e = \mPair{n}{m}$} of type~$t$, the
...
\begin{figure}
  \centering%
  \input{figures/universal-representations/edge-numbers-example-graph}
...
with the same \glspl{edge number} as the data-flow \glspl{edge} connected to the
same \gls{value node}.
%
This is to be able to identify which \gls{definition edge} belongs to which
data-flow \gls{edge} when there exist multiple such \gls{edge}, which is needed
