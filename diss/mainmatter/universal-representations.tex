...
unconditional branches are inserted along the \glspl{edge} from \irBlock{entry}
to \irBlock{head} and from \irBlock{body} to \irBlock{head}.
%
...
\irBlock{inc}~\gls{block} while the subtraction should be performed in the
\irBlock{dec}~\gls{block}.
%
...
%
This results in the \gls{graph} shown in
\refFigure{uf-graph-example-full-graph}, which is called \gls!{UF graph}.
