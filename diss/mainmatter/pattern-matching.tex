...
\footnote{%
  ...
  typically small enough that the time for finding all \glspl{match} is
  negligible compared to the time for solving the \gls{constraint model}.
}
%
...
their data types must also be compatible, which will be discussed in
\refSection{pm-matching-data-types}).
%
...
\section{Matching Data Types}
\labelSection{pm-matching-data-types}
