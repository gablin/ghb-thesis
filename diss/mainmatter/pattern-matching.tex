...
%
\def\subN{N_{\textrm{\textsc{s}}}}%
\def\subE{E_{\textrm{\textsc{s}}}}%
\def\patN{N_{\textrm{\textsc{p}}}}%
\def\patE{E_{\textrm{\textsc{p}}}}%
\begin{algorithm}[t]
  \DeclFunction{FindMatches}{subject graph $\mPair{\subN}{\subE}$,
                             pattern graph $\mPair{\patN}{\patE}$}%
  {%
    $M$ \Assign $\emptyset$\;
    ...
    {%
      ...
      \Else{%
        ...
        \ForEach{$c \in C$}{%
          \If{$c$ passes syntactic and semantic checks}{%
            \Call{FindMatchRec}{$m \cup c$}\;
          }
        }
      }
    }
