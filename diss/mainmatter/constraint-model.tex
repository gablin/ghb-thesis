...
\glspl{datum}.
%
These \glspl{constraint} are jointly modeled as
%
\begin{equation}
  \begin{array}{c}
    \neg\mVar{sel}[m] \mEq
...
%
\RefEquation{data-activity} assumes that every \gls{datum}~$d$
\glsshort{define.d}[able] by some \gls{kill match} is used by at least one
non-\gls{kill match}, which is reasonable or else the \glspl{operation} defining
$d$ would be considered dead code.
