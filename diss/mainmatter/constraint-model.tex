...
%
The \gls{UP graph} of an \gls{instruction} that adds two values~\irVar{r}
and~\irVar{s} and then shifts the result by one bit is depicted in
\refFigure{copy-extending-pattern-example-before}.
%
...
%
This results in the \gls{UP graph} shown in
\refFigure{copy-extending-pattern-example-after}.
...
%
For example, assume a \gls{block}~$A$ that jumps to either of two
\glspl{block}~$B$ and~$C$ depending on whether a condition~$c$ holds.
%
Assume also that the conditional branching in $A$ is implemented using an
\gls{instruction} that jumps to $B$ if $c$ holds, otherwise it continues the
execution with the next \gls{instruction} in the \gls{assembly code}.
%
This notion of execution is called \gls!{fall-through}, and due to this, $C$
must be placed immediately after $A$ in the sequence.
...
In this dissertation, we discuss two approaches for inserting jump
...
\gls{block ordering} before introducing each approach in turn, and then present
