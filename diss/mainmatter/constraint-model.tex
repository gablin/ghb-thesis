...
%
Although these constraints can be relaxed to allow \glspl{operation} and
...
\subsubsection{Implicit Bit Conversions}

In cases where the \gls{function} contains bit conversions, such as sign or zero
extensions or truncations, depending on the hardware the \gls{constraint model}
may produce code with redundant \glspl{instruction}.
%
\begin{figure}
  \centering%
  \input{figures/constraint-model/implicit-bit-conversions-graph}

  \caption{Example illustrating implicit bit conversions}%
  \labelFigure{implicit-bit-conversion-example}
\end{figure}
%
See for example \refFigure{implicit-bit-conversions-example}, which depicts a
...
\irVar{b}, and \irVar{c} represent 8-bit values.
%
\todo{explain example}

\todo{suggest workaround with duplicate patterns}

\todo{suggest workaround with additional variables}
