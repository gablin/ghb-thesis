...
\end{equation}
%
For \glspl{datum} representing \gls{function} arguments and constants, we assume
the \gls{pattern set} has been extended with a special \gls!{null-def pattern},
with \gls{graph} structure \mbox{$\mEdge{b}{d}$} where $b$ is a \gls{entry
  block} and $d$ is a \gls{datum}, that defines $d$ at zero cost.
