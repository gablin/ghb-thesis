...
The idea of \gls!{match duplication} is easily derived from its name: for each
\gls{match}~$m$, create a new \gls{match} for each permutation of \glspl{datum}
that are \gls{copy-related.d} to the \glspl{input datum} in $m$, where an
\gls!{input datum} is a \gls{datum} whose corresponding \gls{node} in the
\gls{pattern} has no \gls{inbound.e} \glspl{data-flow edge}.
%
\begin{figure}
  \setlength{\opNodeDist}{12pt}%
...
\end{figure}
%
See for example \refFigure{match-duplication-example-before}.
%
Because values~\irVar{v}[1] and \irVar{v}[2] are \gls{copy-related.d}, the
\gls{match}~$m$ could use either value.
%
Since \irVar{v}[1] is an \gls{input datum} to the \gls{pattern} of the
match~$m$, we duplicate $m$ to instead use \irVar{v}[2], resulting in match~$m'$
(see \refFigure{match-duplication-example-after}).
