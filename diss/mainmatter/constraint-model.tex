...
The set of \glspl{variable} \mbox{$\mVar{loc}[d] \in \mLocationSet \cup
  \mSet{\mIntLocation}$} models in which location \gls{datum}~$d$ is available,
where $\mLocationSet$ denotes the set of locations provided by the \gls{target
  machine} and $\mIntLocation$ denotes a special location for values that
...
location different from \irVar{v} -- \irVar{v} may reside on the stack, for
instance -- which means that selection of $m_1$ or $m_2$ also entails emission
of a copy \gls{instruction}.
%
...
To model whether a \gls{datum} is \gls{killed.d}, we extend the location set
with a special location~$\mKilledLocation$ and require that a \gls{kill
  match}~$m$ is selected if and only if the location of the \gls{datum}
\gls{define.d}[d] by $m$ is $\mKilledLocation$.
%
This is modeled as
%
\begin{equation}
