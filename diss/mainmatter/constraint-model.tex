...
stored in memory.
%
The special location is used for \glspl!{intermediate value}, which are values
produced within an \gls{instruction} and can only be accessed by this very
\gls{instruction}.
%
...
As expected, \glspl{phi-match} require the locations of all its \glspl{datum} to
be the same.
%
...
\gls{define.d}[d] by a \gls{kill match}.
%
Non-\gls{inactive.d} \glspl{datum} are said to be \gls!{active.d}, and
non-\glspl{kill match} are only allowed to make use of \gls{active.d}
\glspl{datum}.
%
Assuming that every \gls{datum}~$d$ \glsshort{define.d}[able] by some \gls{kill
  match} is used by at least one non-\gls{kill match} -- this is reasonable or
else the \glspl{operation} defining $d$ would be considered dead code -- then
these \glspl{constraint} can be jointly modeled as
%
\begin{equation}
  \begin{array}{c}
    \neg\mVar{sel}[m_1] \mEq
    ...
    \forall d \in \mDataSet[p_1] \hspace{-.8pt},
  \end{array}
  \labelEquation{data-activity}
\end{equation}
%
...
%
However, we use a refined version introduced in \refChapter{solving-techniques}
which enables an \gls{implied.c} \gls{constraint} to be added to the
