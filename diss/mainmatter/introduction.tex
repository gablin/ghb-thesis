...
the saturated add operation used in the program shown in
\refFigure{isel-gcmotion-example}. If selected, such an \gls{instruction} would
implement both the \mbox{\irVar{MAX} \irLE{} \irVar{c}} check given in
block~\irBlock{b3} and the assignment \mbox{\irVar{c} \irAssign{} \irVar{MAX}}
given in block~\irBlock{b4}.
