...
\begin{figure}
  ...
  \subcaptionbox{%
                ...
                {%
                  \small
                  \begin{tabular}{>{\ttfamily}r@{\hspace{4pt}}%
                                  >{$}r<{$}@{ $\leftarrow$ }%
                                  >{$}l<{$}}
                    add  & r & s + t\\
                    mul  & r & s \times t\\
...
%
Hence \gls{instruction selection} is also an optimization problem.
...
\begin{figure}
  ...
  \caption[A program computing saturated sums]%
    {A program computing the saturated sums of two arrays, where \cVar{A},
      \cVar{B}, and \cVar{C} are integer arrays of equal lengths stored in
      memory, and \cVar{N} and \cVar{MAX} are integer constants representing the
      array length and the upper limit, respectively. An \cVar{int} value is
