...
\begin{figure}
  \centering%
  \hspace*{20pt}% Make figure appear more centered
...
\end{inlinelist}.
Each of the problems above is also an optimization problem, meaning there often
exist more than one solution for a given \gls{program} which may result in code
where quality differs significantly. For example, depending on the hardware the
efficiency of two sets of selected instructions may differ by as much as two
orders of magnitude~\cite{ZivojnovicEtAl:1994}. In addition, these problems
interact with one another.

\paragraph{Global Code Motion}

\todo{Explain the problem, give example}
