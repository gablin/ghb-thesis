...
\glspl{graph} (\refAppendix{graph-definitions} provides an exact definition of
\glspl{graph} and other related terms which are used throughout this
dissertation).
%
...
This dissertation proposes an approach, called \gls!{universal instruction
  selection}, where the tasks of \gls{instruction selection}, \gls{global code
  motion}, and \gls{block ordering} are combined and solved using a
combinatorial optimization technique called \gls!{CP}.
%
...
I also participated in the following publications, which are out of scope for
the dissertation:
