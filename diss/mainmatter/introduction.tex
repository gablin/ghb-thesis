...
%
Hence the \gls{instruction} would implement the following operations:
%
\begin{inlinelist}[itemjoin={, }, itemjoin*={, and}]
  \item the \mbox{\irVar{MAX} \irLE{} \irVar{c}} comparison
  \item the conditional jumps to blocks~\irBlock{b4} and~\irBlock{b5}
  \item the \mbox{\irVar{c} \irAssign{} \irVar{MAX}} assignment
...
\gls{instruction} that takes one cycle to execute. Assume also that the

As the \glspl{operation} above reside in separate \glspl{block} (\irBlock{b3}
...
Most approaches represent the \glspl{instruction} as graphs.
%
...
performing multiple add \glspl{operation} (a so-called \gls!{SIMD.instr}[
  \gls{instruction}]).
