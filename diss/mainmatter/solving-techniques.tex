...
$\mVar{loc}$~\gls{variable} will be allocated for each individual
\glsshort{state}.
%
However, since \glsplshort{state} are abstract entities used only to capture
...
%
Consequently, it makes sense to fix the location for each \glsshort{state node},
which is modeled as
%
\begin{equation}
...
\end{equation}
%
where $\mInterchDataSet[\mCopy]$ denotes the set of chains of \glspl{datum} that
...
$\mMatchSet[\mKill]$ denote the set of \glsshort{null match} and \glspl{kill
  match}, respectively, and \mbox{$\mDefSet(d) \subseteq \mMatchSet$} denotes
