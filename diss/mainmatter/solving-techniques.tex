...
\begin{algorithm}
  ...
  {%
    ...
    \For(\tcp*[f]{assign tags}){$m \in M$}{%
      \For{$p \in \textrm{$\mUses$}(m) \cup \textrm{$\mDefines$}(m)$}{%
    ...
    $L_{\textrm{\textsc{c}}}$ \Assign $\mEmptySet$\;
    \For{$g \in G$}{%
      $l_{\textrm{\textsc{c}}}$ \Assign
      $\textrm{$\mMin$}(\mSetBuilder{l}{l \in L \text{ \st } T[l] = g})$
      \tcp*{canonical location for this group of tags}
      $L_{\textrm{\textsc{c}}}$ \Assign
      $L_{\textrm{\textsc{c}}} \cup \mSet{l_{\textrm{\textsc{c}}}}$\;
    }
    \Return $L_{\textrm{\textsc{c}}}$\;
  }
  ...
  \caption[Algorithm for computing the set of canonical locations]%
          {%
            ...
            the function, then these are also assigned a tag and processed
            accordingly%
          }
  \labelAlgorithm{canonical-locs-algorithm}
\end{algorithm}

The algorithm for computing the set of \gls{canonical.l} \glspl{location} is
shown in \refAlgorithm{canonical-locs-algorithm}.
%
\todo{explain the intuition}
