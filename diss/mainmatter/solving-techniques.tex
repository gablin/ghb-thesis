...
can only be defined by \glspl{copy match}, \mbox{$\mNullCopyMatchSet \subseteq
  \mMatchSet$} denote the set of \glspl{null-copy match}, and \mbox{$\mDefSet(d)
  \subseteq \mMatchSet$} denote the set of \glspl{match} that can define a
\gls{datum}~$d$.
%
...
\begin{equation}
  \begin{array}{c}
    ...
    \forall 1 \leq i < k,
    \exists m_i \in \mDefSet(c[i]) \cap \mNullCopyMatchSet.
  \end{array}
