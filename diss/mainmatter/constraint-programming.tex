...
The optimal solution is found using a method called \gls!{branch and
  bound}~\cite{VanBeek:2006}.
%
...
%
\Gls{propagator} are often also called \glspl{filtering algorithm}.
%
...
amount of propagation they perform~\cite{Bessiere:2006}.
%
...
%
Because at most \gls{value consistency} can be achieved for inequality
\glspl{constraint}, they cannot propagate anything until at least one
\gls{variable} becomes \gls{assigned.v}.
%
In comparison, a \glshyphened{domain consistency} implementation of the
\gls{distinct constraint} can remove values~1 and~2 from the \gls{domain} of
\gls{variable}~$\mVar{z}$ without any \gls{search} as these values do not appear
in any \glspl{solution} (see \refFigure{cp-model-example-solutions}).
