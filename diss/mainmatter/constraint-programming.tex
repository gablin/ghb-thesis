...
\gls{store} is called a \gls!{propagator}~\cite{SchulteCarlsson:2006}.
%
...
%
A \gls{propagator} is \gls!{value-consistent.p} if it only propagates when one
of its \glspl{variable} becomes \gls{assigned.v}, \gls!{bound-consistent.p} if
it only reduces the bounds of a \gls{domain}, and \gls!{domain-consistent.p} if
it removes all values that do not appear in any \gls{solution} to the
\gls{constraint}.
%
\begin{figure}
  \centering%
  ...
  \subcaptionbox{%
                  Solving with $\mDistinct$ constraint%
  ...
  \caption[Example illustrating different propagation strengths]%
          {%
            Example illustrating the different propagation strengths of
            $\mDistinct$ and its binary decomposition%
          }
...
%
Because the inequality \glspl{constraint} are \gls{value-consistent.p}, they
cannot propagate anything until at least one \gls{variable} becomes
\gls{assigned.v}.
%
In comparison, a \gls{domain-consistent.p} implementation of $\mDistinct$ can
remove values~1 and~2 from the \gls{domain} of \gls{variable}~$\mVar{z}$ without
any \gls{search} as these values do not appear in any \glspl{solution} (see
\refFigure{cp-model-example-solutions}).

\todo{discuss propagators for global constraints}
