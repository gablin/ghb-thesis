...
\begin{equation}
  ...
  \labelEquation{magic-seq-implied-cons}
\end{equation}
%
\RefEquation{magic-seq-implied-cons} holds because the sum of all occurrences --
that is, the number of items in the sequence -- must be equal to the length of
the sequence.
...
\begin{figure}
  \centering%
  \input{figures/constraint-programming/square-packing-partial-sol}
...
\end{figure}
%
Let us revisit the \glsshort{constraint model} capturing the square packing
problem, and assume the partial \gls{solution} shown in
...
shrink the \gls{variable} \glspl{domain} before solving.
