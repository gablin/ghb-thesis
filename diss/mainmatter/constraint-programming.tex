...
%
\RefSection{cp-overview} gives a brief overview.
%
...
\begin{table}
  ...
  \subcaptionbox{%
                  ...
                  \labelTable{cp-prop-strengths-example-distinct}%
                }{%
                  \begin{tabular}{%
                                   l%
                                   C@{}C@{ }C@{}C%
  ...
  \caption[Example illustrating propagation]%
          {%
            ...
            in \refTable{cp-model-example}, one using the distinct constraint
...
(see \refTable{cp-model-example-solutions}).
%
...
\begin{figure}
  ...
  \caption[Example of a search tree for an optimization problem]%
          {%
            Example of a search tree for the model given in
            \refFigure{cp-model-example} with the additional requirement that
...
which are discussed in detail by \textcite{Smith:2006},
...
\gls!{magic sequence problem}, which is defined as finding a sequence of
integers $x_0, \ldots, x_{n-1}$ such that for all \mbox{$0 \leq i < n$}, the
number~$i$ appears exactly $x_i$ times in the sequence.
%
Using the \gls{global cardinality constraint} and $n$~\glspl{variable}, this can
be modeled as
%
\begin{equation}
  \mGCC(i, \mVar{x}_0, \ldots, \mVar{x}_{n-1}, \mVar{x}_i), \forall 0 \leq i < n.
\end{equation}
%
...
reduce the \gls{variable} \glspl{domain} before solving.
%
For example, assume a \glsshort{constraint model} capturing the \gls{pattern
  selection} problem for entire \glspl{function}.
%
Solving this problem optimally is NP-complete, but finding a suboptimal
\gls{solution} can be done in linear time, as seen in
\refChapter{existing-isel-techniques-and-reps}.
%
The cost of such \glspl{solution} can used as an upper bound on the \gls{cost
  variable}, thereby pruning away the parts of the \gls{search space} containing
\glspl{solution} that are worse.
