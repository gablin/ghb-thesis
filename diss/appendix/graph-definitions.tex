...
%
In a \gls{directed.g} \gls{graph}, an \gls{edge}~$e$ from a \gls{node}~$n$ to
another \gls{node}~$m$ is written \mbox{$\mPair{n}{m}$} and we say that $e$ is
\gls!{outgoing.e} (or \gls!{outbound.e}) with respect to~$n$, and
\gls!{ingoing.e} (or \gls!{inbound.e}) with respect to~$m$.
%
...
%
More formally, we define a \gls{path} between two \glspl{node}~$n$ and $m$ as an
ordered sequence of \glspl{edge}~\mbox{$p = \mSequence{e_1, \ldots, e_l}$} such
that for a \gls{directed.g} \gls{graph}~\mbox{$\mPair{N}{E}$}, \mbox{$e_i \in E,
...
\gls{graph}~\mbox{$G' = \mPair{N'}{E'}$}, also written as \mbox{$G \subseteq
  G'$}, iff \mbox{$N \subseteq N'$} and \mbox{$E \subseteq E'$}.
%
...
%
If there exists such a mapping then we say that $G$ is an \emph!{isomorphic
  \gls{subgraph}} of~$G'$, and an example of this is given in
\refFigure{graph-example}.
%
If $f$ is a bijection, whose inverse function is also a \gls{graph
  homomorphism}, then $f$ is also called a \gls!{graph isomorphism}.
