...
%
\Glspl{edge} with a direction are written $\mPair{n}{m}$, and \glspl{edge}
without are written $\mSet{n, m}$.
%
In a \gls{directed.g} \gls{graph}, we say that an \gls{edge}~\mbox{$e =
  \mPair{n}{m}$} $e$ is \gls!{outgoing.e} (or \gls!{outbound.e}) with respect
