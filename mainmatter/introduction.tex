...
\gls{instruction selection} techniques that were used in the 1980s.
...
This dissertation introduces \gls!{universal instruction selection},\!%
%
\footnote{%
...
The \gls!{frontend} performs syntactic and semantic analysis on the
\gls{program} under compilation, making sure that the \gls{program} is
...
The \gls!{backend} performs \gls{code generation}, where three tasks are most
promiment:%
%
\begin{inlinelist}[itemjoin={; }, itemjoin*={; and}]
...
\end{inlinelist}.
%
Among other things, the \gls{backend} then performs \gls{block ordering} in
order to minimize the number of jump \glspl{instruction}.
%
After these steps the \gls{program} has been fully transformed into
\gls{assembly code}, which can then be translated into machine code to be
...
To evaluate a \gls{constraint model}, we implement it using \mbox{\gls{MiniZinc}
  2.1.6}~\cite{NethercoteEtAl:2007}, which is a high-level \gls{constraint}
modeling language.
%
The model is solved using \gls{Chuffed}~\cite{Chu:2011} -- a \glsdesc{LCG}-based
\glsshort{constraint solver} included in \gls{MiniZinc} -- on a Linux machine
with an \gls{Intel} Xeon W3530 at \SI{2.80}{\GHz} and \SI{16}{\giga\byte} main
memory.
%
...
%
Due to insuffiicent support in the current toolchain, we remove all
...
%
A value greater than \num{1} means $a$ is faster than $b$, a value less than
\num{1} means $b$ is faster than $a$, and a value of exactly \num{1} means $a$
and $b$ take the same time to solve a given \gls{function}.
...
from \num{1} as when $b$ is twice as fast as $a$.
%
