...
Assume that the \gls{target machine} also has a
\glsunset{SIMD.instr}\gls{SIMD.instr} \gls{instruction} for addition.\!%
%
\footnote{%
  A \glsreset{SIMD.instr}\gls!{SIMD.instr}[ \gls{instruction}] is an
  \gls{instruction} that executes the same \gls{operation} over multiple sets of
  input data.%
}
%
...
\gls{LCG}-based \glsshort{constraint solver}, which is included in
...
\begin{description}
  ...
  \item[Ending]
    ...
    \item \refChapter{proposed-model-extensions} proposes
      ...
      \item \refChapter{conclusions-future-work} delivers the conclusions and
