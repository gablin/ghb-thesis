...
the possible \gls{block} sequence and vice verse.
%
...
\begin{inlinelist}[itemjoin={; }, itemjoin*={; and}]
  ...
  \item a \mbox{{\instrFont*jmp} $b$} \gls{instruction}, which unconditionally
    branches to block~$b$\hspace{-1pt}
\end{inlinelist}.
%
...
\gls{baseline} \glsshort{constraint model}~$b$\hspace{-1pt}.
%
A value greater than \num{1} means $a$ is faster than $b$\hspace{-1pt}, a value
less than \num{1} means $b$ is faster than $a$\hspace{-.8pt}, and a value of
...
from \num{1} as when $b$ is twice as fast as $a$\hspace{-.8pt}.
%
...
where $b$ is twice as fast as $a$\hspace{-.8pt}.
%
\begin{figure}
