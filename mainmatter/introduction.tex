...
This dissertation addresses these problems by introducing a new approach, called
\gls!{universal instruction selection}, which is based on methods from
...
\end{figure}
%
The approach, outlined in \refFigure{approach-overview}, relies on a novel model
that is simpler and more flexible compared to non-\glspl{combinatorial approach}
and captures crucial features ignored by other, existing combinatorial
optimization models for \gls{instruction selection}.
%
...
We begin with a thorough literature review to identify the strengths and
limitations of existing \gls{instruction selection} techniques and common
...
techniques, one reason being lack of appropriate \gls{program} and
\gls{instruction} representations.

Having established the need for new representations, we then identify a set of
representation requirements and build a new representation by starting from
well-established, existing representations and then augmenting these as needed
...
\gls{solution} to this task and then add the appropriate \glspl{variable} and
\glspl{constraint} to enforce such \glspl{solution}.
%
If more than one design choice exist for integrating the same task, then we
implement both as separate \glsplshort{constraint model} and evaluate which is
better before proceeding.
%
The same goes for each individual solving technique introduced.
%
Picking the best design decisions and solving techniques, we then evaluate the
significance of the \glsshort{constraint model} by comparing its
  ...
  \item[Background]
    ...
    \begin{inlinelist}[itemjoin={; }, itemjoin*={; and}]
      \item \refChapter{constraint-programming} describes \glsdesc{CP}, which is
        a combinatorial optimizaion method on which our approach is based
