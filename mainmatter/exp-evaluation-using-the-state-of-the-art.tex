...
\cCode*{gpk\_open}), for example, our approach is able to lift computations, in
...
\begin{patternList}
  \item \labelPattern{no-simd}
    one derived from \gls{Hexagon} without \gls{SIMD.instr} \glspl{instruction}
  \item \labelPattern{with-simd}
    one with such \glspl{instruction}
\end{patternList}.
