...
capable of capturing the problems described in \refSection{intro-motivation}.
%
...
expected to be produced in a particular \gls{block}, which are captured by the
\glspl{phi-function}.
%
...
%
Hence, if an \gls{operation}~$o$ implicitly depends on another
...
that $e$ is the $i$th \gls{ingoing.e} \gls{edge} of type~$t$ connected to $m$,
where \mbox{$i = \mInEdgeNr(e)$}.
%
...
To curb the number of \glspl{match}, we prune away all \gls{SIMD.i}
...
\gls{instruction} to be preceded by a copy \gls{instruction}, thereby nullifying
the benefit of the \gls{SIMD.i} \gls{instruction}.
