...
%
This forces $v$ to be produced in $b$, which in turn prevents the
\gls{operation} producing $v$ from being moved out of $b$.
%
...
%
For a given \gls{edge}~\mbox{$e = \mEdge{n_1}{n_2}$} of type~$t$, where $t$
represents either \glslong{control-flow edge}, \glslong{data-flow edge}, or
\glslong{state-flow edge}, the \gls!{inbound.en}[ \gls{edge number}], denoted
\mbox{$\mInEdgeNr(e) = i$}, indicates that $e$ is the $i$th \gls{ingoing.e}
\gls{edge} of type~$t$ connected to $n_2$.
%
Similarly, the \gls!{outbound.en}[ \gls{edge number}], denoted
\mbox{$\mOutEdgeNr(e) = i$}, indicates that $e$ is the $i$th \gls{outgoing.e}
\gls{edge} of type~$t$ connected to $n_1$.
%
