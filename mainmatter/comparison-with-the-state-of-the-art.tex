...
%
In three cases (\codeInText{alloc\_save\_spac}, \codeInText{gp\_enumerate\_fi},
and \codeInText{gpk\_open}), for example, our approach is able to lift
...
the cost.
%
In \codeInText{alloc\_save\_spac}, our approach is also able to move an addition
...
In three other cases (\codeInText{jpeg\_read\_header},
\codeInText{gl\_TexImage3DEX}, and \codeInText{gl\_EnableClient}), our approach
...
significantly lesser cost than those yielded by \glsshort{pattern
  set}~\patternSetA.
%
The five cases with lesser cost (\codeInText{debug\_print\_str},
\codeInText{gl\_read\_alpha}, \codeInText{gx\_curve\_cursor},
\codeInText{mp\_dmul}, and \codeInText{zero}), our approach is able to combine
pairs of additions or subtractions into \num{2}-way \gls{SIMD.instr}
\glspl{instruction}.
%
In addition, in one of these cases (\codeInText{gl\_read\_alpha}) the additions
