...
introduced.
%
Through experimental evaluation, we have seen that these techniques collectively
...
%
We discovered that two solving techniques
...
We have experimentally evaluated the approach by comparing the code quality it
...
%
Hence the approach can handle hardware architectures with a rich
\gls{instruction set} and generates code of equal or better quality compared to
the state of the art.

We have also experimented with generating code with and without \gls{SIMD.instr}
\glspl{instruction}.
%
When such \glspl{instruction} are available, the approach improves code quality
up to~\printSpeedup{%
...
\gls{SIMD.instr} \glspl{instruction}, and this exploitation is benefitted from
