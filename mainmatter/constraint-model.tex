...
of \glspl{match} covering~$o$\hspace{-.8pt}.
%
...
\glsshort{define.d}[ing]~$d$\hspace{-.8pt}.
...
\glspl{datum} produced by \gls{match}~$n$\hspace{-.8pt}.
%
...
some selected \gls{match} placed in a \gls{block}~$b$\hspace{-1pt}, and every
non-\gls{phi-match} \glsshort{use.d}[ing] $d$ is placed in a \gls{block}
\gls{dominate.b}[d] by~$b$\hspace{-1pt}.
%
...
\glspl{operation} covered by \gls{match}~$m$\hspace{-.8pt}, then this
...
or a set containing only the \gls{entry block} of match~$m$ (when the \gls{UP
  graph} has such a \gls{node}), then this \gls{constraint} is modeled as
%
\begin{equation}
  \forall m \in \mMatchSet,
...
respectively \gls{use.d}[d] by \gls{match}~$m$\hspace{-.8pt}.
%
Let also \mbox{$\mDom(b) \subseteq \mBlockSet$} denote the set of \glspl{block}
\gls{dominate.b}[d] by \gls{block}~$b$\hspace{-1pt}.
%
...
\gls{span.b}[ned] by \gls{match}~$m$\hspace{-.8pt}, then this constraint is
modeled as
%
\begin{equation}
  \begin{array}{c}
...
\gls{consume.b}[d] by \gls{match}~$m$\hspace{-.8pt}, then this constraint is
modeled as
%
\begin{equation}
  \begin{array}{c}
...
\glspl{datum} (that is, \mbox{$\mStores(m_1, d) \cap \mStores(m_2, d) =
  \mEmptySet$}).
%
Henceselection of such \glspl{match} is prevented.
...
%
For each \gls{data-flow edge}~$\mEdge{v}{o}$\hspace{-.8pt}, where $v$ is a
\gls{value node} and $o$ is an \gls{operation}, we remove this \gls{edge} and
...
$m$\hspace{-.8pt}, where an \gls!{input datum} is a \gls{datum} \gls{use.d}[d]
but not \gls{define.d}[d] by~$m$\hspace{-.8pt}.
%
...
%
Because \irVar*{v}[1] is an \gls{input datum} in~$m$\hspace{-.8pt}, we duplicate
$m$ to instead use \irVar*{v}[2], resulting in match~$m'$
...
The idea is as follows.
%
For each \gls{match}~$m$\hspace{-.8pt}, let every \glspl{input datum}~$d$ in $m$
be mapped to any \gls{datum} that is \gls{copy-related.d} to~$d$\hspace{-.8pt}.
%
...
%
Because \irVar*{v}[1] is an \gls{input datum} in~$m$\hspace{-.8pt}, we extend
the mapping from $n$ to \irVar*{v}[1] to include \irVar*{v}[2], where $n$ is the
corresponding \gls{value node} in the \gls{UP graph} of~$m$
(\refFigure{alt-values-example-after}).
%
In this context, \irVar*{v}[1] and \irVar*{v}[2] are said to be
\glspl!{alternative value} to~$m$\hspace{-.8pt}.
%
...
%
These invariants can be achieved by transformating the \gls{function} as needed
...
must be placed immediately after $b$ in $s$\hspace{-.8pt}.
...
condition holds, otherwise they branch to block~\irBlock*{C}
...
successor of block~$b$\hspace{-1pt}.
%
...
\gls{block}~$b$\hspace{-1pt}, then the \gls{constraint} can naively be modeled
as \mbox{$\mVar{sel}[m] \mImp \mVar{succ}[\mEntry(m)] = b$}\hspace{-1pt}.
%
However, this \gls{constraint} is too limiting as it disallows \gls{empty.b}
\glspl{block} to be placed between $\mEntry(m)$ and~$b$\hspace{-1pt}, thus
...
\glspl{block}.
%
Let $\mFallThroughSet$ denote a set of pairs~$\mPair{m\hspace{-.8pt}}{b}$, where
$m$ is a \gls{match} and $b$ is a \gls{block} through which $m$ will fall if
selected, and let $\mNullMatchSet$ denote the set of \glspl{null match}.
%
...
new\gls{control node}~$c'$\hspace{-1pt}, and new \glspl{control-flow edge} such
that \mbox{$\mEdge{c}{\mEdge{b'}{\mEdge{c'}{b}}}$}\hspace{-1pt}.
%
...
jump \gls{instruction} that falls through to a given block~$b$\hspace{-1pt}, we
add a new \gls{pattern} to $S$.
%
...
%
Hence, for each \gls{data-flow edge}~$\mEdge{v}{o}$\hspace{-.8pt}, where $v$ is
a \gls{value node} and $o$ is an \gls{operation}, we remove this \gls{edge} and
insert a new \gls{extension node}~$e$\hspace{-.8pt}, a new\gls{value
  node}~$v'$\!, and new \glspl{data-flow edge} such that
