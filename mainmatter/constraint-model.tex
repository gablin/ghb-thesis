...
We evaluate the two methods of \gls{value reuse} by comparing the time it takes
...
\subsubsection{Computing Speedup}

\newcommand{\speedupCase}[1]{\textrm{\textsc{#1}}}

To compare the solving time exhibited by two \glsplshort{constraint model}, we
compute the \gls{speedup}.
%
The \gls{speedup} attained by \glsshort{constraint model}~$a$ over another
\glsshort{constraint model}~$b$ is typically computed as
%
\begin{equation}
  S = \frac{t_b}{t_a}
...
%
With \refEquation{traditional-speedup}, however, this is not the case.
%
...
\begin{figure}
  ...
  \subcaptionbox{%
                  Speedup values computed with
                  \refEquation{traditional-speedup}%
  ...
  \subcaptionbox{%
                  Speedup values computed with
                  \refEquation{normalized-speedup}%
...
\end{equation}
%
which, as seen in \refFigure{speedup-comparison-normalized}, results in
\gls{speedup} values that are centered around $0$ and for which the values
computed in cases \speedupCase{i} and \speedupCase{ii} are equally far apart.
%
\todo{what source to cite?}

The same method is used when comparing \gls{solution} costs as they represent an
estimate of the time it would take to execute the code yielded by the
\glspl{solution}.
...
\todo{value reuse matters (as expected)}
...
\todo{match dup is worse than alt values (as expected}
...
\todo{results match our expected results (branch extension is worse)}
