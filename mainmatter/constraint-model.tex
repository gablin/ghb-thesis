...
the set of \glspl{datum} in $\mUFGraph$ and \mbox{$\mMatchSet[d] \subseteq
  \mMatchSet$} denotes the set of \glspl{match} \glsshort{define.d}[ing]~$d$.
...
respectively \gls{use.d}[d] by \gls{match}~$m$.
%
...
%
Let \mbox{$\mStores(m, d) \subseteq \mLocationSet \cup \mSet{\mIntLocation}$}
denote the set of compatible \glspl{location} (including the special
\gls{location} for intermediate values) for a \gls{datum}~$d$ \gls{define.d}[d]
or \gls{use.d}[d] by a \gls{match}~$m$.
%
...
\gls{copy node}~$c$, a new \gls{value node}~$v'$, and new \glspl{data-flow edge}
such that \mbox{$\mEdge{v}{\mEdge{c}{\mEdge{v'}{o}}}$}.
%
...
with \gls{graph} structure \mbox{$\mEdge{v}{\mEdge{c}{v'}}$}, where $v$ and $v'$
are \glspl{value node} and $c$ is a \gls{copy node}.
%
...
remove this \gls{edge} and insert a new \gls{block node}~$b'$, a new\gls{control
  node}~$c'$, and new \glspl{control-flow edge} such that
\mbox{$\mEdge{c}{\mEdge{b'}{\mEdge{c'}{b}}}$}.
%
...
new \gls{extension node}~$e$, a new\gls{value node}~$v'$, and new
\glspl{data-flow edge} such that \mbox{$\mEdge{v}{\mEdge{e}{\mEdge{v'}{o}}}$}.
%
