...
solving times (including the time for presolving) for the two \glspl{constraint
...
%
From the plot, we see clearly that the \glsplshort{constraint model} based on
...
values for which \glspl{alternative value} yield \num{94}~\glspl{match} and
\gls{match duplication} yields \num{537}~\glspl{match}.
%
...
%
From the plot, we see clearly that \glsplshort{constraint model} supporting
\gls{value reuse} produce \glspl{solution} with considerably lower cost than
those without.
%
...
shows the normalized solving times (including the time for presolving) and
