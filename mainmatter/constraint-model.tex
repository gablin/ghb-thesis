...
We first evaluate the two methods for \gls{value reuse} by comparing the solving
time exhibited by two versions of the \gls{constraint model}: one based on
\gls{match duplication}, and another based on \glspl{alternative value}.
%
Based on the results for the experiment above, we then evaluate the impact of
\gls{value reuse} by comparing the cost of the optimal \gls{solution} produced
by the better of the \glsplshort{constraint model} above with the cost of the
optimal \gls{solution} produced by a \gls{constraint model} without \gls{value
  reuse}.
...
As problem instances, we take a subset of the \glspl{function} in
...
\subsubsection{Attaining Statistically Significant Results}
...
%
To account for this uncertainty, we compute the \num{95}\textsuperscript{th}
\gls{CI}~\cite{Neyman:1937} for the \gls{GMI} value.
%
Hence, if the \gls{CI} is greater than or less than~\num{1}, then we can
statistically conclude with \SI{95}{\%} confidence that the result is positive
respectively negative.
%
If the \gls{CI} contains~\num{1}, then the result is inconclusive.

For the experiments described in this dissertation, we compute the \gls{CI}
using \gls{percentile bootstrapping} with
...
time exhibited by two \glsplshort{constraint model}: one based on \gls{match
  duplication}, and another based on \glspl{alternative value}.
%
...
\glsshort{constraint model}~\modelB{} to perform better than
...
solving times (including \gls{presolving} time) for the two \glspl{constraint
  model} described above, with \glsshort{constraint model}~\modelA{} as
\gls{baseline} and \glsshort{constraint model}~\modelB{} as \gls{subject}.
%
...
\num{537}~\glspl{match}.
%
Hence we conclude that, when implementing \gls{value reuse}, \glspl{alternative
  value} is a better design choice over \gls{match duplication}.
...
the optimal \glspl{solution} produced by two \glsplshort{constraint model}: one
with \gls{value reuse} support (based on \glspl{alternative value}), and another
...
we expect \glsshort{constraint model}~\modelB{} to produce \glspl{solution} with
less cost compared with \glsshort{constraint model}~\modelA.
...
As the \gls{GMI} is \printGMI{%
  ...
  \AltValuesVsWithoutCyclesSpeedupCyclesRegularSpeedupCiMax%
}, we see that \glsshort{constraint model}~\modelB{} produces \glspl{solution}
with significantly lesser cost than those produced by \glsshort{constraint
  model}~\modelA.
%
...
\glspl{solution} produced by two versions of the \gls{constraint model}: one
based on \gls{branch extension}, and another based on \glspl{DTB pattern}.
%
We refer to these \glsplshort{constraint model} as \modelA{} and \modelB,
respectively.
%
...
redundant jump \glspl{instruction}, we expect \glsshort{constraint
  model}~\modelB{} perform better, both in terms of solving time and cost, than
\glsshort{constraint model}~\modelA.
...
\RefFigureList{%
  dual-target-branch-patterns-vs-branch-extension-solving-time-plot,
  dual-target-branch-patterns-vs-branch-extension-cycles-plot%
} show the normalized solving times (including \gls{presolving} time) and
...
and \glsshort{constraint model}~\modelB{} as \gls{subject}.
%
...
\glspl{instruction} increases -- up to \num{13}, in the case of {\codeFont
  nextkeypacket} -- the \gls{search space} for \glsshort{constraint
  model}~\modelA{} grows faster than for \glsshort{constraint model}~\modelB.
%
In addition, as the \gls{GMI} for cost is \printGMI{%
  ...
  \DualTargetPatternsVsBranchExtCyclesSpeedupCyclesRegularSpeedupCiMin%
    ...
    \DualTargetPatternsVsBranchExtCyclesSpeedupCyclesRegularSpeedupCiMax%
}, we see that, in some cases, \glsshort{constraint model}~\modelB{} produces
optimal \glspl{solution} with slightly lower cost compared those produced by
\glsshort{constraint model}~\modelA.
%
Hence we conclude that, when implementing insertion of jump \glspl{instruction},
\glspl{DTB pattern} is a better design choice over \gls{branch extension}.
