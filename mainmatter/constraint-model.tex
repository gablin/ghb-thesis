...
to solve two versions of the \gls{constraint model} -- one based on \gls{match
  duplication} and another based on \glspl{alternative value} -- over a set of
\glspl{function}.
%
...
in this dissertation, it is called \gls!{zero-centered normalization} -- which
is computed as
%
\begin{equation}
  S =
...
code yielded by the \glspl{solution}.
...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            Relative solving times for two constraint models that supports
            value reuse, one based on match duplication (baseline) and another
            based on alternative values%
          }
...
described above, with the \glsshort{constraint model} based on \gls{match
...
%
As can be clearly seen from the data, \glsplshort{constraint model} based on
...
\begin{figure}
  ...
  \caption[%
            Comparing optimal costs for two constraint models with and without
            value reuse%
          ]%
          {%
            Relative optimal costs for two constraint models, one with value
            reuse support and another without (baseline)%
          }
...
\RefFigure{alt-values-vs-without-cycles-plot} shows the normalized costs for the
two \glspl{constraint model} describe above, with the \glsshort{constraint
  model} without \gls{value reuse} support serving as \gls{baseline}.
%
All \glspl{function} are solved to optimality.
%
...
%
As can be clearly seen from the plots, \glsplshort{constraint model} supporting
...
\gls{branch extension} and another based on \glspl{dual-target branch pattern}
-- over a set of \glspl{function}.
...
\glspl{dual-target branch pattern}, many of which will be empty and thereby
...
\begin{figure}
  ...
  \caption[%
            Comparing solving times for two constraint models supporting
            jump instruction insertion%
          ]%
          {%
            Relative solving times for two constraint models that supports jump
            insertion, one based on branch extension (baseline) and another
            based on dual-target branch patterns%
          }
  \labelFigure{%
...
\begin{figure}
  ...
  \caption[%
            Comparing optimal costs for two constraint models supporing
            jump instruction insertion%
          ]%
          {%
            Relative optimal costs for two constraint models that supports jump
            insertion, one based on branch extension (baseline) and another
            based on dual-target branch patterns%
          }
...
shows the normalized solving times (including presolving) and \gls{solution}
costs, respectively, for the two \glspl{constraint model} described above, with
the \glsshort{constraint model} based on \gls{branch extension} serving as
\gls{baseline}.
%
...
%
For the \gls{baseline}, the solving times range from
\SI{\DualTargetPatternsVsBranchExtPrePlusSolvingTimeSpeedupBaselineSolvingTimeAvgMin}{\s}
to
\SI[round-precision=0]{\DualTargetPatternsVsBranchExtPrePlusSolvingTimeSpeedupBaselineSolvingTimeAvgMax}{\s},
and the costs range from \num[round-precision=0]{%
  \DualTargetPatternsVsBranchExtCyclesSpeedupBaselineCyclesAvgMin}~cycles to
\num[round-precision=0]{%
  \DualTargetPatternsVsBranchExtCyclesSpeedupBaselineCyclesAvgMax}~cycles.
%
As can be clearly seen from the plots, \glsplshort{constraint model} based on
\glspl{dual-target branch pattern} yield significantly lower solving times than
those based on \gls{branch extension}.
%
Cases where \glspl{dual-target branch pattern} result in longer solving times
(\todo{select functions}) are \glspl{function} consisting of few \glspl{block}
for which \gls{branch extension} yields a smaller \gls{search space}.
%
When the number of conditional branch \glspl{instruction} increases -- up to
\num{13}, in the case of {\codeFont nextkeypacket} -- the \gls{search space} for
\gls{branch extension} grows faster than for \glspl{dual-target branch pattern}.
%
In addition, in some cases \glspl{dual-target branch pattern} result in optimal
\glspl{solution} with slightly lower cost compared with \gls{branch extension}.
%
Hence, when implementing insertion of jump \glspl{instruction},
\glspl{dual-target branch pattern} is a better design choice over \gls{branch
  extension}.
