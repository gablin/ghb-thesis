...
\glspl{function}.
%
To evalute the impact of \gls{value reuse}, we compare the quality of the code
produced by one of the \glsplshort{constraint model} above (since both methods
are \gls{solution}-preserving, it does not matter which is selected) with the
quality of the code produced by a \gls{constraint model} without \gls{value
  reuse}.
...
%
When summarizing ratios such as speedup -- we take the geometric
mean~\cite{FlemingWallace:1986}.


\subsubsection{Computing Speedup}

\todo{describe how we compute speedup}

\todo{describe function for computing speedup (give example)}
