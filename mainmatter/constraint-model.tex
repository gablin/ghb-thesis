...
memory \glspl{operation} and a \gls{target machine} where the address can be
computed as part of the memory \glspl{instruction}.
%
A \gls{solution} to the latter problem would therefore only need two
\glspl{instruction}, whereas the former problem would require three
\glspl{instruction} (one to compute the sum and two to perform the memory
\glspl{operation}, since the addition is not allowed to be covered by both
memory \glspl{instruction}).
%
In certain conditions, however, an add \gls{instruction} may still be required.
%
...
requires an add \gls{instruction} to compute the sum.
%
Due to the increased complexity of the relaxed version of the problem, we model
exact \gls{cover}[age] in this dissertation.

Similarly, every value and state must be produced by exactly one selected
\gls{match}.
%
...
if there exists an \gls{inbound.e} \glsshort{state-flow edge} or \gls{data-flow
  edge} to $d$ in the \gls{UP graph} from which $m$ was derived.
%
Similarly, $m$ \gls!{use.d}[s] $d$ if there exists an \gls{outbound.e}
...
\glspl{datum} produced by \gls{match}~$n$.
%
...
in the \gls{cycle}.
%
The \glspl{cycle} are found using \citeauthor{Johnson:1975}'s
algorithm~\cite{Johnson:1975}.
...
some selected \gls{match} that is placed in a \gls{block}~$b$, and every
...
sophisticated \gls{addressing mode} is produced within the pipeline and thus
cannot be reused by other \glspl{instruction}.
%
...
In this dissertation, we discuss two methods for reusing values:
%
...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            ...
            constraint models that supports value reuse:
            %
            \begin{inlinelist}[itemjoin={, }, itemjoin*={, and}]
...
%
In one case (\cCode*{reg2rsaref}), for example, the solving time differs by a
factor
of~\printSpeedup{%
...
%
In one case (\cCode*{putseqdispext}), for example, the cycle count is reduced
from \num{256}~cycles to \num{208}~cycles (a speedup of \printSpeedup{%
  \AltValuesVsWithoutCyclesSpeedupCyclesRegularSpeedupMax%
}).
%
...
jump \gls{instruction} that falls through to a given block~$b$, we add a new
\gls{pattern} add to $S$.
%
...
%
Moreover, many of the \glspl{blocks} incurred by \gls{branch extension} will be
...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            Normalized solving times (incl.\ presolving time) for two
            constraint models that supports jump insertion:
            %
            \begin{inlinelist}[itemjoin={, }, itemjoin*={, and}]
...
zero cost provided that \mbox{$(\mVar{sext}[v] \mOr \mNot\mVar{sext}[v']) \mAnd
  (\mVar{zext}[v] \mOr \mNot\mVar{zext}[v'])$} holds.
%
