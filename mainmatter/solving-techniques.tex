...
}, thus defending our claim that \glsplshort{constraint model} based on the
refined \gls{objective function} yield better code quality than those based on
the naive \gls{objective function}.
%
...
First, we evalute the impact of all \gls{implied.c} \gls{constraint}
(\refEquationRange{%
  impl-cons-defs-dominate-defs%
}{%
  impl-cons-fix-fall-throughs%
}) by comparing the solving times exhibited by two versions of the
\gls{constraint model} -- one with all \gls{implied.c} \glspl{constraint} and
another without -- for the set of sampled \glspl{function}.
%
We expect the collection of \gls{implied.c} \glspl{constraint} to lead to an
overall reduction in solving time -- hence the \gls{GMI} should be greater
than~\num{1}.
%
Second, we evaluate the impact of each \gls{implied.c} \gls{constraint} by
comparing the solving times exhibited by two versions of the \gls{constraint
  model} -- one with a given \gls{implied.c} \gls{constraint} and another
without the \gls{constraint} -- for the set of sampled \glspl{function}.
%
...
Eqs.\thinspace\refEquation*{impl-cons-defs-in-spanned-blocks} and
\refEquation*{impl-cons-spanned-input} to have no impact on solving time.
%
...
%
Third, we evaluate synergy effects among the \gls{implied.c} \glspl{constraint}
by comparing the solving times exhibited by two versions of the
\glsshort{constraint model} -- one with all \gls{implied.c} \glspl{constraint}
and another with only those who individually have a non-negative effect on
solving time -- for the set of sampled \glspl{function}.
%
No hypothesis is given for this evaluation.

\begin{figure}
  \centering%
  \insertSolvTechSpeedupPlot{all-implied-cons}{1}

  \caption[%
            Plot for evaluating the implied constraints' impact on
            solving time%
          ]%
          {%
            Normalized solving times (incl.\ presolving time) for two
            constraint models: one with all implied constraints (subject) and
            another with no such constraints (baseline)%
          }
  \labelFigure{impl-cons-vs-no-cons-solving-time-plot}
\end{figure}

\input{%
  \expDir/solv-tech-disable-all-implied-cons-pre+solving-time-speedup.stats%
}

\RefFigure{impl-cons-vs-no-cons-solving-time-plot} shows the normalized solving
times (including presolving time) for two \glspl{constraint model}: one with all
\gls{implied.c} \glspl{constraint}, serving as \gls{subject}; and another with
no such \glspl{constraint}, serving as \gls{baseline}.
%
The solving times range from
\SIMinOf{
  \SolvTechDisableAllImpliedConsPrePlusSolvingTimeSpeedupAllPrePlusSolvingTimeAvgMin,
  \SolvTechDisableAllImpliedConsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeAvgMin
}{\s}
to
\SIMinOf{
  \SolvTechDisableAllImpliedConsPrePlusSolvingTimeSpeedupAllPrePlusSolvingTimeAvgMax,
  \SolvTechDisableAllImpliedConsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeAvgMax
}{\s}
with a maximum coefficient of variation of
\numMaxOf{
  \SolvTechDisableAllImpliedConsPrePlusSolvingTimeSpeedupAllPrePlusSolvingTimeCvMax,
...
}, thus defending our claim that \glsplshort{constraint model} with the
\gls{implied.c} \glspl{constraint} yield significantly lower solving times than
those without such \glspl{constraint}.

\def\mkFigureCaption{%
  \caption[%
            Set of plots for evaluating each implied constraint's impact on
            solving time%
          ]%
          {%
            Normalized solving times (incl.\ presolving time) for two
            constraint models: one with a given implied constraint (subject) and
            another without the constraint (baseline)%
          }
}
...
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-defs-dominate-defs-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-defs-in-spanned-blocks-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-identical-entry-blocks-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-defs-dominate-entry-blocks-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-place-phi-ops-same-as-def-edges-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-no-span-uses-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-no-span-defs-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-no-span-use-defs-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-spanned-input-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-used-data-must-be-available-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-exterior-data-must-be-available-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-locs-of-uses-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-locs-of-defs-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-fix-fall-throughs-pre+solving-time-speedup.stats%
}
...
with a given \gls{implied.c} \gls{constraint}, serving as \gls{subject}; and
another without the \gls{constraint}, serving as \gls{baseline}.
%
The solving times range from
\SIMinOf{
...
%
\def\colsep{\hspace{.37em}}%
%
\begin{table}
  \centering%
  \figureFontSize\smaller%
  \begin{tabular}{p{8mm}*{7}{@{\colsep}c@{\colsep}}}
    \toprule
      & \multicolumn{7}{c}{\tabhead equation} \\
    \cmidrule(lr){2-8}%
      & \tabhead\refEquation*{impl-cons-defs-dominate-defs}
      & \tabhead\refEquation*{impl-cons-defs-in-spanned-blocks}
      & \tabhead\refEquation*{impl-cons-identical-entry-blocks}
      & \tabhead\refEquation*{impl-cons-defs-dominate-entry-blocks}
      & \tabhead\refEquation*{impl-cons-place-phi-ops-same-as-def-edges}
      & \tabhead\refEquation*{impl-cons-no-span-uses}
      & \tabhead\refEquation*{impl-cons-no-span-defs} \\
    \midrule
        \tabhead GMI
      & \printGMI{\SolvTechDisableImplConsDefsDominateDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsDefsInSpannedBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsIdenticalEntryBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsDefsDominateEntryBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsPlacePhiOpsSameAsDefEdgesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsNoSpanUsesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsNoSpanDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean} \\
        \tabhead CI
      & \printGMICI{%
          \SolvTechDisableImplConsDefsDominateDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsDefsDominateDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsDefsInSpannedBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsDefsInSpannedBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsIdenticalEntryBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsIdenticalEntryBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsDefsDominateEntryBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsDefsDominateEntryBlocksPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsPlacePhiOpsSameAsDefEdgesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsPlacePhiOpsSameAsDefEdgesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsNoSpanUsesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsNoSpanUsesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsNoSpanDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsNoSpanDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        } \\
    \midrule
      & \tabhead\refEquation*{impl-cons-no-span-use-defs}
      & \tabhead\refEquation*{impl-cons-spanned-input}
      & \tabhead\refEquation*{impl-cons-used-data-must-be-available}
      & \tabhead\refEquation*{impl-cons-exterior-data-must-be-available}
      & \tabhead\refEquation*{impl-cons-locs-of-uses}
      & \tabhead\refEquation*{impl-cons-locs-of-defs}
      & \tabhead\refEquation*{impl-cons-fix-fall-throughs} \\
    \midrule
        \tabhead GMI
      & \printGMI{\SolvTechDisableImplConsNoSpanUseDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsSpannedInputPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsUsedDataMustBeAvailablePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsExteriorDataMustBeAvailablePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsLocsOfUsesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsLocsOfDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsFixFallThroughsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean} \\
        \tabhead CI
      & \printGMICI{%
          \SolvTechDisableImplConsNoSpanUseDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsNoSpanUseDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsSpannedInputPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsSpannedInputPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsUsedDataMustBeAvailablePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsUsedDataMustBeAvailablePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsExteriorDataMustBeAvailablePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsExteriorDataMustBeAvailablePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsLocsOfUsesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsLocsOfUsesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsLocsOfDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsLocsOfDefsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsFixFallThroughsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsFixFallThroughsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        } \\
    \bottomrule
  \end{tabular}

  \caption[%
            Geometric mean improvements on solving time attained by a given
            implied constraint%
          ]%
          {%
            GMIs on the solving time attained by a given implied constraint%
          }
  \labelTable{single-impl-con-vs-no-con-solving-time-geom-mean-improvements}
\end{table}
%
Due to the \glspl{GMI} and \glspl{CI} given in
\refTable{single-impl-con-vs-no-con-solving-time-geom-mean-improvements}, defend
our claim that Eqs.\thinspace\refEquation*{impl-cons-defs-in-spanned-blocks} and
\refEquation*{impl-cons-spanned-input} have no impact on solving time and that
some of the \gls{implied.c} \glspl{constraint}
(Eqs.\thinspace\refEquation*{impl-cons-defs-dominate-defs},
\refEquation*{impl-cons-identical-entry-blocks},
\refEquation*{impl-cons-no-span-defs},
\refEquation*{impl-cons-used-data-must-be-available},
\refEquation*{impl-cons-exterior-data-must-be-available}, and
\refEquation*{impl-cons-fix-fall-throughs}) lead to a reduction in solving time
while others have a negative effect
(Eqs.\thinspace\refEquation*{impl-cons-defs-dominate-entry-blocks},
\refEquation*{impl-cons-no-span-uses},
\refEquation*{impl-cons-no-span-use-defs},
\refEquation*{impl-cons-locs-of-uses}, and
\refEquation*{impl-cons-locs-of-defs}).
%
\refEquation{impl-cons-place-phi-ops-same-as-def-edges} appears to have no
impact on solving time.

\begin{figure}
  \centering%
  \insertSolvTechSpeedupPlot{bad-implied-cons}{1}

  \caption[%
            Plot for evaluating ?? impact on
            solving time%
          ]%
          {%
            Normalized solving times (incl.\ presolving time) for two constraint
            models: one with all implied constraints (baseline) and another with
            only the constraints who individually have a non-negative effect on
            solving time (subject)%
          }
  \labelFigure{no-bad-impl-cons-vs-all-cons-solving-time-plot}
\end{figure}

\input{%
  \expDir/solv-tech-disable-bad-implied-cons-pre+solving-time-speedup.stats%
}

\RefFigure{no-bad-impl-cons-vs-all-cons-solving-time-plot} shows the normalized
solving times (including presolving time) for two \glspl{constraint model}: one
with all \gls{implied.c} \glspl{constraint}, serving as \gls{baseline}; and
another with only the \glspl{constraint} who individually have a non-negative
effect on solving time, serving as \gls{subject}.
%
The solving times range from
\SIMinOf{
  \SolvTechDisableBadImpliedConsPrePlusSolvingTimeSpeedupAllPrePlusSolvingTimeAvgMin,
  \SolvTechDisableBadImpliedConsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeAvgMin
}{\s}
to
\SIMinOf{
  \SolvTechDisableBadImpliedConsPrePlusSolvingTimeSpeedupAllPrePlusSolvingTimeAvgMax,
  \SolvTechDisableBadImpliedConsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeAvgMax
}{\s}
with a maximum coefficient of variation of
\numMaxOf{
  \SolvTechDisableBadImpliedConsPrePlusSolvingTimeSpeedupAllPrePlusSolvingTimeCvMax,
...
\begin{figure}
  ...
  \caption{TODO}
  \labelFigure{dom-cons-vs-no-cons-solving-time-plots}
\end{figure}

\input{%
  \expDir/%
  solv-tech-disable-dom-cons-locs-of-states-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-dom-cons-operands-of-non-selected-matches-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-impl-cons-input-operands-not-taking-min-value-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-dom-cons-interch-data-chains-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-dom-cons-null-copy-match-selection-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-dom-cons-kill-match-selection-pre+solving-time-speedup.stats%
}

\begin{table}
  \centering%
  \figureFontSize\smaller%
  \begin{tabular}{p{8mm}*{6}{@{\colsep}c@{\colsep}}}
    \toprule
      & \multicolumn{6}{c}{\tabhead equation} \\
    \cmidrule(lr){2-7}%
      & \tabhead\refEquation*{dom-cons-locs-of-states}
      & \tabhead\refEquation*{dom-cons-operands-of-non-selected-matches}
      & \tabhead\refEquation*{impl-cons-input-operands-not-taking-min-value}
      & \tabhead\refEquation*{dom-cons-interch-data-chains}
      & \tabhead\refEquation*{dom-cons-null-copy-match-selection}
      & \tabhead\refEquation*{dom-cons-kill-match-selection} \\
    \midrule
        \tabhead GMI
      & \printGMI{\SolvTechDisableDomConsLocsOfStatesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableDomConsOperandsOfNonSelectedMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableImplConsInputOperandsNotTakingMinValuePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableDomConsInterchDataChainsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableDomConsNullCopyMatchSelectionPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableDomConsKillMatchSelectionPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean} \\
        \tabhead CI
      & \printGMICI{%
          \SolvTechDisableDomConsLocsOfStatesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableDomConsLocsOfStatesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableDomConsOperandsOfNonSelectedMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableDomConsOperandsOfNonSelectedMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableImplConsInputOperandsNotTakingMinValuePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableImplConsInputOperandsNotTakingMinValuePrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableDomConsInterchDataChainsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableDomConsInterchDataChainsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableDomConsNullCopyMatchSelectionPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableDomConsNullCopyMatchSelectionPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableDomConsKillMatchSelectionPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableDomConsKillMatchSelectionPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        } \\
    \bottomrule
  \end{tabular}
  ...
  \caption[%
            Geometric mean improvements on solving time attained by a given
            symmetry or dominance breaking constraint%
          ]%
          {%
            GMIs on the solving time attained by a given symmetry or dominance
            breaking constraint%
          }
  \labelTable{dom-cons-vs-no-cons-solving-time-geom-mean-improvements}
\end{table}
...
\begin{figure}
  ...
  \caption{TODO}
  \labelFigure{presolving-vs-no-pre-solving-time-plots}
\end{figure}

\input{%
  \expDir/%
  solv-tech-disable-dom-matches-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-illegal-matches-uncovered-ops-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-illegal-matches-undefined-data-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-illegal-matches-kills-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-illegal-matches-def-locs-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-illegal-matches-use-locs-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-redun-kills-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-redun-non-null-copy-matches-pre+solving-time-speedup.stats%
}
\input{%
  \expDir/%
  solv-tech-disable-canonical-locs-pre+solving-time-speedup.stats%
}
...
\begin{table}
  ...
  \begin{tabular}{p{8mm}*{7}{@{\colsep}c@{\colsep}}}
    \toprule
      & \multicolumn{7}{c}{\tabhead equation} \\
    \cmidrule(r){2-8}%
      & \tabhead\refEquation*{illegal-matches-uncovered-ops}
      & \tabhead\refEquation*{illegal-matches-undefined-data}
      & \tabhead\refEquation*{illegal-matches-kills}
      & \tabhead\refEquation*{illegal-matches-def-locs}
      & \tabhead\refEquation*{illegal-matches-use-locs}
      & \tabhead\refEquation*{redun-kills}
      & \tabhead\refEquation*{redun-non-null-copy-matches} \\
    \midrule
        \tabhead GMI
      & \printGMI{\SolvTechDisableIllegalMatchesUncoveredOpsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableIllegalMatchesUndefinedDataPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableIllegalMatchesKillsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableIllegalMatchesDefLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableIllegalMatchesUseLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableRedunKillsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}
      & \printGMI{\SolvTechDisableRedunNonNullCopyMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean} \\
        \tabhead CI
      & \printGMICI{%
          \SolvTechDisableIllegalMatchesUncoveredOpsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableIllegalMatchesUncoveredOpsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableIllegalMatchesUndefinedDataPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableIllegalMatchesUndefinedDataPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableIllegalMatchesKillsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableIllegalMatchesKillsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableIllegalMatchesDefLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableIllegalMatchesDefLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableIllegalMatchesUseLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableIllegalMatchesUseLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableRedunKillsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableRedunKillsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        }
      & \printGMICI{%
          \SolvTechDisableRedunNonNullCopyMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
        }{%
          \SolvTechDisableRedunNonNullCopyMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
        } \\
    \midrule
      & \multicolumn{3}{c}{%
          \tabhead dominated matches
        }
      &
      & \multicolumn{3}{c}{%
          \tabhead canonical locations
        } \\
    \midrule
        \tabhead GMI
      & \multicolumn{3}{c}{%
          \printGMI{\SolvTechDisableDomMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}%
        }
      & \tabhead CI
      & \multicolumn{3}{c}{%
          \printGMI{\SolvTechDisableCanonicalLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupGmean}%
        } \\
      & \multicolumn{3}{c}{%
          \printGMICI{%
            \SolvTechDisableDomMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
          }{%
            \SolvTechDisableDomMatchesPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
          }%
        }
      &
      & \multicolumn{3}{c}{%
          \printGMICI{%
            \SolvTechDisableCanonicalLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMin%
          }{%
            \SolvTechDisableCanonicalLocsPrePlusSolvingTimeSpeedupPrePlusSolvingTimeRegularSpeedupCiMax%
          }%
        } \\
    \bottomrule
  \end{tabular}
  ...
  \caption[%
            Geometric mean improvements on solving time attained by a given
            presolving technique%
          ]%
          {%
            GMIs on the solving time attained by a given presolving technique%
          }
  \labelTable{presolving-vs-no-pre-solving-time-geom-mean-improvements}
\end{table}
