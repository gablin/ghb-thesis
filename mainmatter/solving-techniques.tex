...
among the \gls{implied.c} \glspl{constraint} by comparing the solving times
exhibited by four versions of the \glsshort{constraint model}: one with no
\gls{implied.c} \glspl{constraint} whatsoever, one with all \glspl{constraint},
one with only those who individually lead to an overall reduction in solving
time, and another without those who individually lead to an overall increase in
solving time.
%
...
no \gls{implied.c} \glspl{constraint}.
%
...
\def\mkFigureCaption{%
  \caption[%
          ...
          {%
            ...
            (baseline) and another with all such constraints (subject)%
          }
}
...
solving time, that
Eqs.\thinspace\refEquation*{impl-cons-defs-dominate-entry-blocks} and
\refEquation*{impl-cons-no-span-use-defs} lead to an overall increase in solving
time, and that Eqs.\thinspace\refEquation*{impl-cons-defs-in-spanned-blocks},
\refEquation*{impl-cons-spanned-input}, and
\refEquation*{impl-cons-place-phi-ops-same-as-def-edges} has no impact on
solving time.
%
For Eqs.\thinspace\refEquation*{impl-cons-defs-dominate-defs},
\refEquation*{impl-cons-identical-entry-blocks},
\refEquation*{impl-cons-no-span-uses}, \refEquation*{impl-cons-no-span-defs},
...
\refEquation*{impl-cons-locs-of-defs}, the results are inconclusive.
...
\begin{figure}
  ...
  \mkSolvTechSubfigure{bad-implied-cons}%
                      [%
                        ...
                        \refEquation*{impl-cons-defs-dominate-entry-blocks} and
  ...
  \caption[%
          ...
          {%
            ...
            another with all implied constraints (subject)%
          }
  \labelFigure{diff-impl-cons-conf-solving-time-plot}
\end{figure}
...
\RefFigure{diff-impl-cons-conf-solving-time-plot} shows the normalized solving
...
\gls{subject}; and another with all \gls{implied.c} \glspl{constraint}, also
serving as \gls{subject}.
%
...
To begin with, we observe that with the \gls{GMI} for \glsshort{constraint
...
%
This means that adding an \gls{implied.c} \gls{constraint} to a
\glsshort{constraint model} with other \gls{implied.c} \glspl{constraint} is
more beneficial than adding an \gls{implied.c} \gls{constraint} to a
\glsshort{constraint model} with no such \glspl{constraint}.
%
...
parts.
%
Hence we conclude that the \gls{implied.c} \glspl{constraint} jointly lead to an
overall reduction in solving time.
