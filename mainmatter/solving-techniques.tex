...
We begin in \refSectionList{st-refining-define-before-use-constraint,
  st-refined-objective-function} with refining the \gls{constraint model}
introduced in \refChapter{constraint-model} in order to strengthen
\gls{propagation}.
%
...
\refSection{st-cost-bounds} and describe the \glspl{branching strategy} applied
in our approach in \refSection{st-branching-strategies}.
%
...
\refSection{st-experimental-evaluation} the impact of these techniques, both
individually and as groups.
%
Lastly, a summary is given in\refSection{st-summary}.

The material presented in
\refSectionList{st-refining-define-before-use-constraint, st-impl-constraints,
  st-dom-breaking-constraints, st-branching-strategies, st-pre-dom-matches,
  st-canonical-locations} is based on ideas conceived by Mats~Carlsson which
have been improved by the author of this dissertation.
%
...
smallest value in the \gls{domain} of~$v$.
%
...
the solving techniques introduced in this chapter.
...
results in significantly shorter solving times than \glsshort{constraint
  model}~\refModel{obj-fun-mul-div}.
%
In one case (checksum\codeInText{build\_ycc\_rgb\_t}), for example, the solving
time is two orders of magnitude shorter, and in three cases
(\codeInText{alloc\_save\_spac}, \codeInText{gpk\_open}, and
\codeInText{trueRandAccum}) the solving time is more than one order of magnitude
shorter.
%
Although \glsshort{constraint model}~\refModel{obj-fun-mul-div} outperforms
\glsshort{constraint model}~\refModel{obj-fun-div-mul} in three other cases
(\codeInText{alloc\_name\_is\_s}, \codeInText{debug\_dump\_byte}, and
\codeInText{write\_file\_trai}), the loss in performance for these
\glspl{function} is overshadowed by the gain in performance for other
\glspl{function}.
%
...
can be solved optimally by five versions of the \gls{constraint model}:
%
\begin{modelList}[resume]
  ...
  \item \labelModel{obj-fun-naive-ub}
    another naive \glsshort{constraint model} but with upper bound
...
for the \gls{cost variable}, we expect \glsplshort{constraint
  model}~\refModel{obj-fun-refined-no-b}, \refModel{obj-fun-refined-ub},
and~\refModel{obj-fun-refined-lb-ub}{} to find a greater number of optimal
\glspl{solution} compared with \glsplshort{constraint
  model}~\refModel{obj-fun-naive-no-b} and~\refModel{obj-fun-naive-ub}.
%
Due to further tightening of the cost bounds, we expect \glsshort{constraint
  model}~\refModel{obj-fun-naive-ub} to outperform \glsshort{constraint
  model}~\refModel{obj-fun-naive-no-b}, \glsplshort{constraint
  model}~\refModel{obj-fun-refined-ub}{} and~\refModel{obj-fun-refined-lb-ub}{}
to outperform \glsshort{constraint model}~\refModel{obj-fun-refined-no-b}, and
\glsshort{constraint model}~\refModel{obj-fun-refined-lb-ub} to outperform
\glsshort{constraint model}~\refModel{obj-fun-refined-ub}.
...
\refModel{obj-fun-refined-ub}, and~\refModel{obj-fun-refined-lb-ub} clearly
outperform \glsplshort{constraint model}~\refModel{obj-fun-naive-no-b}
and~\refModel{obj-fun-naive-ub}.
%
...
fact, \glsshort{constraint model}~\refModel{obj-fun-refined-lb-ub} fails to find
the optimal \gls{solution} for one \gls{function} whereas \glsshort{constraint
  model}~\refModel{obj-fun-refined-ub} manages to find the optimal
...
\gls{solution} found within the time limit by \glsplshort{constraint
  model}~\refModel{obj-fun-naive-ub} (naive \glsshort{constraint model} with
upper bound) and~\refModel{obj-fun-refined-ub} (refined \glsshort{constraint
  model} with upper bound).
%
For the same reason as with the second experiment, we expect the
\glspl{solution} produced by \glsshort{constraint
  model}~\refModel{obj-fun-refined-ub} to be of significantly better quality
compared with those produced by \glsshort{constraint
  model}~\refModel{obj-fun-naive-ub}.
...
third experiment, with \glsshort{constraint model}~\refModel{obj-fun-naive-ub}
as \gls{baseline} and \glsshort{constraint model}~\refModel{obj-fun-refined-ub}
as \gls{subject}.
%
...
We see clearly that \glsplshort{constraint model} based on the refined
...
Based on the results from these experiments, we conclude:
%
...
%
Based on the results, we then arrange them into groups to evaluate synergy
effects.
%
To curb experiment runtimes, we include all other solving techniques in the
\glsplshort{constraint model} because, without them, a very long time out would
...
\end{modelList}.
%
For both \glsplshort{constraint model}, we include all other solving techniques
...
We see that \refEquation{impl-cons-fix-fall-throughs} leads to an overall
reduction in solving time, that
...
We evaluate synergy effects among the \gls{implied.c} \glspl{constraint} by
comparing the solving times exhibited by \glspl{constraint model} with and
without a given combination of \glspl{constraint}.
%
...
versions of the \glsshort{constraint model}:
%
\begin{modelList}[resume]
  ...
  \item \labelModel{impl-ex-only-good-cons}
    ...
    time
  \item \labelModel{impl-ex-no-bad-cons}
...
\refModel{impl-ex-no-bad-cons}, and~\refModel{impl-ex-all-cons} to all perform
better than \glsplshort{constraint model}~\refModel{impl-ex-no-cons}.
%
...
\refModel{impl-ex-no-bad-cons}, and~\refModel{impl-ex-all-cons}.
...
described above in the second experiment, with \glsshort{constraint
  ...
  model}~\refModel{impl-ex-only-good-cons}, \refModel{impl-ex-no-bad-cons},
and~\refModel{impl-ex-all-cons} as \glspl{subject}.
%
The solving times range from
\printMinSolvingTime{
...
To begin with, we observe that the \gls{GMI} for \glsshort{constraint
  ...
  model}~\refModel{impl-ex-no-cons}
...
In comparison, we observe that the \gls{GMI} for \glsshort{constraint
  model}~\refModel{impl-ex-no-bad-cons} over \glsshort{constraint
  model}~\refModel{impl-ex-no-cons}
...
%
Since \glsshort{constraint model}~\refModel{impl-ex-all-cons} contains
\gls{implied.c} \glspl{constraint} whose individual impact on solving time is
statistically negative yet still outperforms \glsshort{constraint
  model}~\refModel{impl-ex-no-bad-cons}, this means that there are synergy
...
Based on the observations above, we conclude that the \gls{implied.c}
\glspl{constraint} have strong enough synergy effects among each other that it
appears to be most beneficial to include all such \glspl{constraint} in the
\glsshort{constraint model}.
...
\begin{enumerate*}[label=(\roman*), itemjoin={;\ }, itemjoin*={; but\ }]
  \item that some of the \gls{implied.c} \glspl{constraint} have a positive
    impact on solving time while others have a negative impact
  \item due to strong synergy effects, it appears to be most beneficial to
    include all \gls{implied.c} \glspl{constraint} in the \glsshort{constraint
      model}
\end{enumerate*}.
...
We observe that \refEquationList{dom-cons-null-copy-match-selection,
...
We evaluate synergy effects among the \glsshort{symmetry breaking.c} and
\gls{dominance breaking.c} \glspl{constraint} by comparing the solving times
exhibited by \glspl{constraint model} with and without a given combination of
\glspl{constraint}.
%
...
\gls{implied.c} \glspl{constraint}
  \item \labelModel{dom-ex-only-good-cons}
    one with only those who individually lead to an overall reduction in solving
    time
  \item \labelModel{dom-ex-no-bad-cons}
    one without those who individually lead to an overall increase in solving
    time
...
\refModel{dom-ex-no-bad-cons}, and~\refModel{dom-ex-all-cons} to all perform
better than \glsplshort{constraint model}~\refModel{dom-ex-no-cons}.
%
No hypothesis is attempted regarding the relative performance between
\glsplshort{constraint model}~\refModel{dom-ex-only-good-cons},
\refModel{dom-ex-no-bad-cons}, and~\refModel{dom-ex-all-cons}.
...
times (including \gls{presolving} time) for three of the \glspl{constraint
  ...
  model}~\refModel{dom-ex-no-cons} as \gls{baseline} and \glsplshort{constraint
  model}~\refModel{dom-ex-only-good-cons} and~\refModel{dom-ex-all-cons} as
\glspl{subject} (\glsshort{constraint model}~\refModel{dom-ex-no-bad-cons} is
not evaluated since no \glsshort{symmetry breaking.c} or \gls{dominance
...
We observe that the \gls{GMI} for \glsshort{constraint
...
In comparison, we see that \glsshort{constraint
  model}~\refModel{dom-ex-all-cons} yields a greater reduction in solving time
for a larger number of \glspl{function} than \glsshort{constraint
  model}~\refModel{dom-ex-only-good-cons} but also a considerable increase in
...
Based on the observations above, we conclude that the \glsshort{symmetry
  breaking.c} and \gls{dominance breaking.c} \glspl{constraint} in the
\glsshort{constraint model} do not have strong enough synergy effects among each
other that it appears to be most beneficial to only include
\refEquationList{dom-cons-null-copy-match-selection,
...
\begin{enumerate*}[label=(\roman*), itemjoin={;\ }, itemjoin*={; and\ }]
  \item that some of the \glsshort{symmetry breaking.c} and \gls{dominance
    breaking.c} \glspl{constraint} have a positive impact on solving time
  \item that, due to weak synergy effects, it appears to be most beneficial to
    only include those \glsshort{symmetry breaking.c} and \gls{dominance
      breaking.c} \glspl{constraint} in the \glsshort{constraint model}
\end{enumerate*}.
...
We observe that \refEquationList{redun-kills, redun-non-null-copy-matches} lead
to an overall reduction in solving time, that
...
We evaluate synergy effects among the \gls{presolving} techniques by comparing
the solving times exhibited by \glspl{constraint model} with and without a given
...
better than \glsplshort{constraint model}~\refModel{pre-ex-no-techs}.
%
No hypothesis is attempted regarding the relative performance between
...
To begin with, we observe that the \gls{GMI} for \glsshort{constraint
  ...
  model}~\refModel{pre-ex-no-techs}
...
Next, we observe that the \gls{GMI} for \glsshort{constraint
...
}, which is still better than \glsshort{constraint
  model}~\refModel{pre-ex-only-good-techs} but worse than \glsshort{constraint
  model}~\refModel{pre-ex-no-bad-techs}.
%
Looking at the \gls{CI}, we see that the spread of improvement for each
\gls{function} is wider.
%
In other words, while \glsshort{constraint model}~\refModel{pre-ex-all-techs}
improves solving time for some \glspl{function} over \glsshort{constraint
  model}~\refModel{pre-ex-no-bad-techs}, it degrades solving time for a larger
set of \glspl{function}, which is reflected in the \gls{GMI}.

Based on the observations above, we conclude that the \gls{presolving}
techniques have strong synergy effects within some groups but not within other
groups.
%
Hence it appears to be most beneficial to exclude
...
\begin{enumerate*}[label=(\roman*), itemjoin={;\ }, itemjoin*={; and\ }]
  ...
  \item that, due to weak synergy effects among these techniques, it appears to
    be most beneficial to exclude the techniques that have a negative impact
    from the \glsshort{constraint model}
\end{enumerate*}.
...
\begin{modelList}
  ...
  \item \labelModel{st-ex-best-combos}
    one with with the \gls{implied.c}, \glsshort{symmetry breaking.c} and
...
\refModel{st-ex-no-bad-techs}, \refModel{st-ex-all-techs},
and~\refModel{st-ex-best-combos} to all perform better than
\glsplshort{constraint model}~\refModel{st-ex-no-techs}.
%
We also expect \glsshort{constraint model}~\refModel{st-ex-best-combos} to
outperform \glsplshort{constraint model}~\refModel{st-ex-only-good-techs},
\refModel{st-ex-no-bad-techs}, and~\refModel{st-ex-all-techs} as it contains the
...
\refModel{st-ex-all-techs}, and~\refModel{st-ex-best-combos} as \glspl{subject}.
%
The solving times range from
\printMinSolvingTime{
...
We see clearly that all \glsplshort{constraint model} significantly improve
solving time over \glsshort{constraint model}~\refModel{st-ex-no-techs}.
%
...
We also observe that, counter to our expectation, \glsshort{constraint
  model}~\refModel{st-ex-no-bad-techs} (\refFigure{solv-tech-disable-all-bad})
yields the largest improvement over \glsshort{constraint
  model}~\refModel{st-ex-no-techs}, closely followed by \glsplshort{constraint
  model}~\refModel{st-ex-all-techs} (\refFigure{solv-tech-disable-all}) and
\refModel{st-ex-best-combos} (\refFigure{solv-tech-best-combos}), which in turn
both considerably outperform \glsshort{constraint
...
Moreover, because \glsshort{constraint model}~\refModel{st-ex-best-combos}
performs worse than both \glsplshort{constraint
  model}~\refModel{st-ex-no-bad-techs} and~\refModel{st-ex-all-techs}, it means
...
another category (such as \glsshort{symmetry breaking.c} and \gls{dominance
...
counterintuitive ways, which is further magnified when \gls{LCG} is involved.

Regarding \glsshort{constraint model}~\refModel{st-ex-no-bad-techs} and
\glsshort{constraint model}~\refModel{st-ex-all-techs}, we observe when
comparing \refFigureList{solv-tech-disable-all-bad, solv-tech-disable-all} that
both \glsplshort{constraint model} have very similar performance characteristics
...
%
While \glsshort{constraint model}~\refModel{st-ex-all-techs} yields considerable
solving time improvement for one case (second from the left), it is slightly
worse than \glsshort{constraint model}~\refModel{st-ex-no-bad-techs} on several
...
\begin{enumerate*}[label=(\roman*), itemjoin={;\ }, itemjoin*={; and\ }]
  \item that the solving techniques introduced in this chapter are crucial for
    scalability
  \item that picking only solving techniques that have a statistically positive
    effect on solving time when evaluated individually is too conservative
  \item that rejecting solving techniques that have an statistically negative
    effect on solving time when evaluated individually yields a \gls{constraint
