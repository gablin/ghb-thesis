...
Assume a \gls{UF graph} containing two \gls{copy-related.d} values,
\irVar*{v}[1] and~\irVar*{v}[2], that may both be connected to two
...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            ...
            multiply-then-divide method (baseline), and one based on the
...
%
In one case (checksum\cCode*{build\_ycc\_rgb\_t}), for example, the solving time
is two orders of magnitude shorter, and in three cases
(\cCode*{alloc\_save\_spac}, \cCode*{gpk\_open}, and \cCode*{trueRandAccum}) the
...
(\cCode*{alloc\_name\_is\_s}, \cCode*{debug\_dump\_byte}, and
\cCode*{write\_file\_trai}), the loss in performance for these \glspl{function}
is overshadowed by the gain in performance for other \glspl{function}.
%
...
fact, using a lower bound causes \glsplshort{constraint
...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            ...
            implementing the naive objective function (baseline), and one
            ...
            %
            Both models uses an upper bound computed by LLVM and a lower bound
            computed using a relaxed model.
            %
...
\NewDocumentCommand{\mkFigureCaption}{s}{%
  ...
  \def\capText{%
    ...
    one without a particular implied constraint (baseline) and one with all
...
We see that \refEquationList{impl-cons-defs-dominate-defs,
  impl-cons-defs-dominate-entry-blocks} lead to an overall reduction in solving
time, and that \refEquationList{impl-cons-defs-in-spanned-blocks,
...
\glsplshort{constraint model}~\refModel{impl-ex-no-cons}.
%
  ...
  \caption[%
          ...
          {%
            ...
            \begin{modelList}
              ...
              \item one with all such \glspl{constraint} (subject)
            \end{modelList}%
          }
...
Hence combining \refEquationList{impl-cons-defs-dominate-defs,
  impl-cons-defs-dominate-entry-blocks} yields a greater overall reduction in
solving time than when picked individually.
%
However, we note that this combination yields a considerable performance
degradation for one \gls{function}.
...
}, which is a greater improvement compared to \glsshort{constraint
  model}~\refModel{impl-ex-only-good-cons}.
%
We also see that this combination yields no considerable performance degradation
for any \gls{function}.
%
...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            ...
            breaking constraint (baseline) and one with all such
            constraints (subject)%
          }
...
reduction in solving time and that
...
and~\refModel{dom-ex-all-cons} to all perform better than \glsplshort{constraint
...
In comparison, we observe that the \gls{GMI} for \glsshort{constraint
  ...
  model}~\refModel{dom-ex-no-cons} (\refFigure{solv-tech-disable-all-dom-cons})
is \printGMI{%
...
%
In addition, this combination yields no considerable performance degradation for
any \gls{function}.
%
Hence it is most beneficial to include all such \glspl{constraint} in the
\glsshort{constraint model}.
...
\begin{figure}
  ...
  \caption[%
          ...
          {%
            ...
            (baseline) and one with all such techniques (subject)%
          }
...
\gls{canonical.l} \glspl{location} lead to an overall reduction in solving time,
that \refEquationList{illegal-matches-def-locs, illegal-matches-use-locs} lead
to an overall increase in solving time, and that
  ...
  \caption[%
          ...
          {%
            ...
            \begin{modelList}
              ...
              \item one with only those who individually lead to an overall
                reduction in solving time (\refEquationList{redun-kills,
...
better than \glsplshort{constraint model}~\refModel{st-ex-no-techs}.
%
No hypothesis is attempted regarding the relative performance between
  ...
  \caption[%
          ...
          {%
            ...
            \begin{modelList}
              ...
              \item one with only those who individually lead to an overall
                reduction in solving time (subject)
                (\refEquationList{impl-cons-defs-dominate-defs,
            ...
            \end{modelList}.
            %
            \Glspl{function} marked with \barValueNoBaselineSolution{} are those
            for which model~\refModel{st-ex-no-techs} fails to produce any
            solution%
          }
...
significantly improve solving time over \glsshort{constraint
  model}~\refModel{st-ex-no-techs} and do not degrade performance for any
\glspl{function}.
%
...
(\refFigure{solv-tech-disable-all-bad}) yields the largest improvement over
\glsshort{constraint model}~\refModel{st-ex-no-techs}, closely followed by
\glsshort{constraint model}~\refModel{st-ex-all-techs}
(\refFigure{solv-tech-disable-all}), which in turn considerably outperforms
\glsshort{constraint model}~\refModel{st-ex-only-good-techs}
...
%
Also, rejecting solving techniques that have an overall negative effect on
solving time when evaluated individually yields the \gls{constraint model} with
best performance.
...
\begin{enumerate*}[label=(\roman*), itemjoin={;\ }, itemjoin*={; and\ }]
  ...
  \item that picking only solving techniques that have a statistically
    ...
    too conservative
  \item that rejecting solving techniques that have an statistically
    significant, negative effect on solving time when evaluated individually
    yields a \gls{constraint model} with best performance
\end{enumerate*}.
