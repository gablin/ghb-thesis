...
\end{figure}
%
Assume the \gls{UF graph} shown in \refFigure{cost-example-graph}, which can be
...
%
An example of this cost matrix is given in \refFigure{cost-example}.
%
From this example, we can deduce that the cost of covering
\glspl{operation}~$o_1$ and~$o_2$ is between~\num{3} and~\num{40} respectively
between~\num{1} and~\num{20} (\refFigure{cost-example-matrix}).
%
...
%
In this case, the relaxed \glsshort{constraint model} consists of only the
...
\end{figure}
%
For sake of discussion, each storage requirement has been labeled with a tag.
%
...
%
\mbox{$\mSet{\mReg{1}, \ldots, \mReg{4}}$} due to tags~\num{1} and~\num{2},
%
\mbox{$\mSet{\mReg{5}}$} due to tag~\num{1},
%
$\mSet{\mReg{6}}$ due to tag~\num{1} and~\num{3},
%
\mbox{$\mSet{\mReg{7}, \mReg{8}}$} due to tags~\num{3} and~\num{4}, and
%
$\mSet{\mReg{9}}$ due to tag~\num{4}.
%
...
\NewDocumentCommand{\mkFigureCaption}{s}{%
  ...
  \def\capText{%
    ...
    \begin{inlinelist}[itemjoin={, }, itemjoin*={, and}]
      ...
      \item one with all such constraints (subject)
    \end{inlinelist}%
  }%
  \IfBooleanTF{#1}{%
    \caption*{\capText}%
  ...
  \caption[%
          ...
          {%
            ...
            \begin{inlinelist}[itemjoin={, }, itemjoin*={, and}]
              ...
              \item one with all such techniques (subject)
            \end{inlinelist}%
          }
