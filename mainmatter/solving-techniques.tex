...
\gls{constraint model} -- one with the \gls{constraint} and another without --
for the set of sampled \glspl{function}.
%
...
%
Second, we evaluate synergy effects among the \gls{implied.c} \glspl{constraint}
by comparing the solving times exhibited by two versions of the
\glsshort{constraint model} -- one using all \gls{implied.c} \glspl{constraint}
and another using only those who individually have a non-negative effect on
...
times (including presolving time) for two \glspl{constraint model}: one with a
given \gls{implied.c} \gls{constraint}, serving as \gls{subject}; and another
without the \gls{constraint}, serving as \gls{baseline}.
%
The solving times range from
\SIMinOf{
...
\begin{table}
  ...
  \caption[%
          ...
          {%
            ...
            %
            See also \refFigure{impl-cons-vs-no-cons-solving-time-plots}%
          }
  \labelTable{impl-cons-vs-no-cons-solving-time-geom-mean-improvements}
\end{table}
%
The \glspl{geometric mean improvement}, given in
\refTable{impl-cons-vs-no-cons-solving-time-geom-mean-improvements}, defend our
claim that Eqs.\thinspace\refEquation*{impl-cons-defs-in-spanned-blocks} and
