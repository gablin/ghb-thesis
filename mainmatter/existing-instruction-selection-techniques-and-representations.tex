...
\glspl{instruction} requires the \gls{function} to be written directly in
\gls{assembly code}.
...
\begin{inlinelist}[itemjoin={, }, itemjoin*={, and}]
  ...
  \item an \gls!{expand procedure} to be executed upon the part of the
    \gls{function} that was matched (see \refFigure{macro-example} for an
    example)
\end{inlinelist}.
%
...
attempts one \gls{macro} after another, typically in the order they are
declared.
%
...
and often only expand one \gls{IR}~\gls{operation} at at time, which results in
poor code quality.
%
...
correctness -- that is, the simple, single-\gls{operation} \gls{macro} which
ensure that code can always be produced -- and those used for efficiency, which
makes for a simpler and more incremental retargeting effort.
%
...
Because this \gls{principle} has little relevance for our approach, we will not
...
where $G$ denotes a \gls{data-flow graph} derived from the \gls{function}.
%
...
\begin{figure}
  ...
  \caption[Example of the pattern matching and selection problem]%
          {%
            ...
            %
            Variable assignments need not be explicitly represented as nodes as
...
be described in a machine-readable format, which we call the \gls!{machine
  description}.
%
...
use of index subscripts for the \glspl{node} representing the input arguments.
...
of overlap between such combinations, \citeauthor{LeupersMarwedel:1995} solve
...
positive (non-negated) literal, and \gls{IP}~models built using \glspl{Horn
...
%
Like \gls{IP}, a \gls{constraint model} consists of a set of \glspl{variable}, a
set of \glspl{constraint} over the \glspl{variable}, and typically also an
\gls{objective function} to either minimize or maximize.
%
...
\end{equation}
%
which offers stronger \gls{propagation} than
\refEquation{pattern-selection-in-ip} and thus reduces solving
time~\cite{FlochEtAl:2010}.
...
\glspl{graph} (one such example is given in \refChapter{introduction} on
...
problems described in \refSection{intro-motivation} and, to that end, we need a
more powerful means of representing \glspl{function} and \glspl{instruction}.
