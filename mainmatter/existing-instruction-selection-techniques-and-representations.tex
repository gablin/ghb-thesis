...
\begin{definition}[IP]
  ...
  integer matrix, and $\mVar{x}$ be a vector of integer \glspl{decision
    variable}.
  %
  Then
  %
  \begin{displaymath}
    \begin{array}{rl}
      ...
      \text{or minimize} & \\
      \text{subject to} & \mMatrix{A} \mVar{x} \leq \mVector{b}, \\
        & \mMatrix{A} \mVar{x} \in \mathbb{Z}^{n \times n}\!, \\
...
selecting \gls{SIMD.i} \glspl{instruction}.
%
    ...
    \begin{array}{rl}
      \text{minimize}
        & \displaystyle
          ...
          \mVar{x}_i^{\transp} \mMatrix{C}_{ij} \, \mVar{x}_j +
