...
\gls!{state}.
%
For a given labeled \gls{node}~$n$\hspace{-.8pt}, the \gls{state} essentially
holds enough information to optimally reduce the \gls{expression tree} rooted
at~$n$ to any \gls{nonterminal}.
%
...
\gls{nonterminal}~$\mNT{N}$\hspace{-1pt}, then $\mNT{N}$ can be shared between
the \glspl{rule} whose \glsplshort{rule pattern} all contain
$\mNT{N}$\hspace{-1pt}.
%
...
match~$m$\hspace{-.8pt}, and $\mVector{\mVar{x}}[m]$ is a Boolean
(\mbox{$\mSet{0, 1}$}) \gls!{decision variable} indicating whether match~$m$ is
selected.
%
...
many \glspl{node} are covered by \gls{match}~$m$\hspace{-.8pt}.
%
...
%
This is achieved by first extending, for each node~$i$\hspace{-.8pt}, the domain
of its \gls{variable}~$\mVector{\mVar{x}}_i$ with \glspl{match} derived from
...
set to a \gls{proxy.r} \gls{rule} associated with $j$\hspace{-1pt}.
%
...
\begin{definition}[Mapping Candidate Set]
  Let \mbox{$\fG = \mPair{\fN}{\fE}$} be a \gls{function graph}, \mbox{$\pG =
    \mPair{\pN}{\pE}$} be a \gls{pattern graph}, and $s$ be a set of mappings
  from \glspl{node} in $\fG$ to \glspl{node} in $\pG$.
  %
  Let $\fN(s)$ and $\pN(s)$ denote the sets of \glspl{node} in $\fG$ and $\pG$,
  respectively, that appear in $s$\hspace{-.8pt}.
  %
