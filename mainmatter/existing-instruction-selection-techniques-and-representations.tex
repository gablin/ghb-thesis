...
%
Then, for each node~$n$ to be covered, \gls{variable} \mbox{$\mVar{match}_n \in
  \mSetBuilder{m}{m \in M, i \in \mCovers(m)}$} decides which \gls{match}
covers~$n$.
%
Also, for each match~$m$, \gls{variable} \mbox{$\mVar{count}_m \in \mSet{0,
    |\mCovers(m)|}$} decides how many \glspl{node} are covered by $m$.
%
...
%
Then $\mVector{\mVar{x}}_i$ is a \gls!{variable} with $k$~elements, where
\mbox{$\mVector{\mVar{x}}_i[j] = 1$} means that choice~$j$ has been selected for
decision~$i$.
%
