...
\begin{definition}[$\mGCC$]
  ...
  \begin{displaymath}
    ...
    \equiv
    \big|
      \mSetBuilder{\mVar{x}_i}{\forall 1 \leq i \leq k : \mVar{x}_i = v}
    \big| = \mVar{y}.
  \end{displaymath}
  \labelDefinition{gcc}
\end{definition}

For example, \mbox{$\mGCC(3, \mVar{x}_1 \in \mSet{2}, \mVar{x}_2 \in \mSet{3},
  \mVar{y} \in \mSet{1})$} holds because exactly one of the
$\mVar{x}$~\glspl{variable} is assigned value~$3$.
%
Similarly, \mbox{$\mGCC(3, \mVar{x}_1 \in \mSet{2}, \mVar{x}_2 \in \mSet{3},
  \mVar{y} \in \mSet{0, 2})$} does not holds because either none or both of the
$\mVar{x}$~\glspl{variable} must be assigned value~$3$.
...
$\MCumulative$ is used in \refChapter{future-work} to model \gls{instruction
  scheduling}.
...
$\MNoOverlap$ is used in \refChapter{future-work} to model \gls{register
  allocation}.
...
%
\footnote{%
  In \cite{FeydyStuckey:2009}, it is described how to re-engineer the original
...
To begin with, every \glspl{variable} has two dual representations: one based on
Booleans; and one based on integers, which is the \gls{domain} typically used in
\gls{CP}.
%
...
\mbox{$\mVar{x} \geq k$} are equivalent to \mbox{$\mBoolRepVar{\mVar{x} \leq k -
    1}$}, \mbox{$\neg\mBoolRepVar{\mVar{x} \leq k - 1}$}, and
\mbox{$\neg\mBoolRepVar{\mVar{x} \leq k - 1}$}.
...
assigned value~$2$, then it will produce two clauses \mbox{$\neg\mBoolRepVar{x =
    2} \mOr \mBoolRepVar{y = 1} \mOr \mBoolRepVar{y = 3}$} and
\mbox{$\neg\mBoolRepVar{y = 2} \mOr \mBoolRepVar{x = 1} \mOr \mBoolRepVar{x =
...
For example, if we are given a clause~\mbox{$\mBoolRepVar{\mVar{x} = 1} \mOr
  \neg\mBoolRepVar{\mVar{y} = 4}$} and $\mVar{x} = 2$, then the Boolean
...
derive an explanation for why a \gls{constraint} \glsshort{failure}, which can
be captured as a clause, called \gls!{no-good}, and added to the existing body
of clauses.
%
This effectively cuts away that part of the \gls{search space} and prevents the
\glsshort{constraint solver} from making the same mistake again.
%
