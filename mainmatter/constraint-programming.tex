...
\subsubsection{The All-Different Constraint}
...
\subsubsection{The Global Cardinality Constraint}
...
\subsubsection{The Circuit Constraint}

Another relevant example is the \gls!{circuit constraint}~\cite{Lauriere:1978},
which enforces that the \glspl{variable} representing adjacency forms a cycle.
%
...
As will be seen in \refChapter{constraint-model}, $\mCircuit$ can be used to
model \gls{block ordering}.


\subsubsection{The Table Constraint}
...
refine the \gls{objective function} in order to improve solving.


\subsubsection{The Value-Precede-Chain Constraint}
\labelSection{cp-vpc}

Another \gls{global.c} \gls{constraint} we need is the \gls!{value-precede-chain
  constraint}~\cite{LawLee:2004}, which requires a sequence of \glspl{variable}
to be sorted according to a given chain of values.
%
...
As will be seen in \refChapter{solving-techniques}, $\mValuePrec$ can be used
for implementing a \gls{dominance breaking.c} \gls{constraint}.


\subsubsection{The Increasing Constraint}
\labelSection{cp-increasing}

To this end, we make use of the \gls!{increasing constraint}~\todo{[ref]}, which
requires a sequence of \glspl{variable} to be assigned values in a non-strictly
increasing order.
%
We refer to this \gls{constraint} as $\mIncreasing$, which is defined as
follows.

\begin{definition}[$\mIncreasing$]
  Let \mbox{$\mVar{x}_1, \ldots, \mVar{x}_k$} be a permutation of
  \glspl{variable}.
  %
  Then
...
For example, \mbox{$\mIncreasing(\mVar{x}_1 \in \mSet{1}, \mVar{x}_2 \in
  \mSet{2}, \mVar{x}_3 \in \mSet{2}, \mVar{x}_4 \in \mSet{5})$} holds because
the assignment forms a sequence \mbox{$\mTuple{1, 2, 2, 4}$} which is in
non-strictly increasing order.
%
However, \mbox{$\mIncreasing(\mVar{x}_1 \in \mSet{4}, \mVar{x}_2 \in \mSet{3})$}
does not hold the corresponding sequence \mbox{$\mTuple{4, 3}$} is clearly not
ordered.

As will be seen in \refChapter{solving-techniques}, $\mIncreasing$ can be used
for implementing two \gls{dominance breaking.c} \glspl{constraint}.
