...
of a \gls{match}~$m$\hspace{-.8pt}, and \mbox{$\mResourceUse(m, r) \in
...
$m$ is selected and placed in $b$\hspace{-.8pt}.
%
This can be modeled as
%
\begin{equation}
  ...
  \mVar{sched}[m, b]
  \mEq
  \mVar{sel}[m] \mAnd \mBlockOf(m) = b \hspace{-.8pt}.
...
\gls{live range} for a \gls{datum}~$d$\hspace{-.8pt}.
%
...
\gls{match}~$m$ uses \gls{datum}~$d$\hspace{-.8pt}.
%
...
Intuitively, given a \gls{datum}~$d$ and a \gls{block}~$b$\hspace{-.8pt}, if $d$
...
\end{equation}
%
Moreover, \refEquationList{global-live-range-end-def-and-int-blocks,
